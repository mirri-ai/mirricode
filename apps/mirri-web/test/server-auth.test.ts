// apps/mirri-web/test/server-auth.test.ts
...
const STORAGE_KEY = 'mirri-web.server-credential';
...
describe('credential persistence', () => {
  ...
  it('drops a legacy session credential when localStorage migration fails', async () => {
    sessionStore.setItem(STORAGE_KEY, 'legacy-tok');
    localStore.setItem = () => {
    ...
    // Mirri-web's loadStored() does not mark migrationRecorded when the
    // sessionStorage.removeItem succeeds — the persistCredential call already
    // failed, so the credential is not safe to use.
...
describe('cross-tab credential clearing', () => {
  ...
  it('keeps a newer token another tab persisted when this tab is stale', async () => {
    ...
    // Another tab stores a fresh token (e.g. after rotation + `mirri web`).
