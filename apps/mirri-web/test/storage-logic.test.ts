...
describe('draftStorageKey', () => {
  it('uses the session id when present', () => {
    expect(draftStorageKey('abc')).toBe('mirri-web.draft.abc');
  });

  it('falls back to __new__ when sid is empty/undefined', () => {
    expect(draftStorageKey(undefined)).toBe('mirri-web.draft.__new__');
    expect(draftStorageKey('')).toBe('mirri-web.draft.__new__');
  });
});
