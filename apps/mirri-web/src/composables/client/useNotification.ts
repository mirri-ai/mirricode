// apps/mirri-web/src/composables/client/useNotification.ts
...
function maybeNotifyCompletion(sid: string, ctx: NotifyCompletionCtx): void {
  maybeNotify(
    ...
    `mirri-complete-${sid}-${ctx.promptId ?? Date.now()}`,
  );
}
