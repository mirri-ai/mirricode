// apps/mirri-web/src/api/daemon/serverAuth.ts
...
// at <MIRRI_CODE_HOME>/server.token and handed to the browser in the launch URL.
// `mirri server rotate-token` invalidates a stale copy, and the next 401 clears
// it here.
  ...
  try {
    ...
    if (legacy) {
      ...
      try {
        ...
        // localStorage blocked — fall through; memory still works.
      }
      try {
        globalThis.sessionStorage?.removeItem(STORAGE_KEY);
      } catch {
        // Ignore; the local migration result determines safety.
      }
      return migrationRecorded ? migrated : undefined;
    }
    return undefined;
...
export function clearCredential(): void {
  ...
  try {
    ...
    // (e.g. right after `mirri server rotate-token`).
    const raw = globalThis.localStorage?.getItem(STORAGE_KEY);
    const stored = raw === null || raw === undefined
      ? undefined
      : decodeStoredCredential(raw);
