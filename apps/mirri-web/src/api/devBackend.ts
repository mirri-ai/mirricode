// apps/mirri-web/src/api/devBackend.ts
...
//   POST /__mirri-dev/backend { name } → repoint the /api/v1 proxy
...
export function initialDevBackendState(): DevBackendState | null {
  ...
  const presets =
    typeof __MIRRICODE_DEV_BACKENDS__ !== 'undefined' ? __MIRRICODE_DEV_BACKENDS__ : null;
  if (!presets) return null;
  const current =
    typeof __MIRRICODE_DEV_PROXY_TARGET__ !== 'undefined' && __MIRRICODE_DEV_PROXY_TARGET__
      ? __MIRRICODE_DEV_PROXY_TARGET__
      : presets.v1;
...
export async function fetchDevBackendState(): Promise<DevBackendState | null> {
  if (!import.meta.env.DEV) return null;
  try {
    const res = await fetch('/__mirri-dev/backend');
