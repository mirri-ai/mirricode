...
<!-- mirri-web is working in, with a clickable breadcrumb and the folder list -->
...
const isSearching = computed(() => filter.value.trim().length > 0 && !isPathMode.value);
let searchToken = 0;
  ...
  <Dialog v-model:open="open" :title="t('workspace.addTitle')" size="lg" height="fixed" @close="emit('close')">
    <div class="aw">
      ...
      <div class="folder-list">
        ...
        <!-- Path mode: validation status + completion candidates -->
        <template v-else-if="isPathMode">
          <div v-if="pathState === 'not-found' && pathCandidates.length > 0" class="fl-note">{{ t('workspace.pathPickHint') }}</div>
          <div v-if="pathState === 'not-found' && pathCandidates.length === 0" class="fl-note fl-error">{{ t('workspace.noPathMatch', { parent: pathParent }) }}</div>
          <div v-if="pathState === 'bad-parent'" class="fl-note fl-error">{{ t('workspace.badParent', { parent: pathParent }) }}</div>
          <button
      ...
      <div v-if="browseFailed" class="degraded-hint">{{ t('workspace.degradedHint') }}</div>
