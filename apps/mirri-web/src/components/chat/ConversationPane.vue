...
function onPanesPointerDown(event: PointerEvent): void {
  ...
  // Only stop following when the click lands on the scrollbar track: the
  // pointer X is beyond the visible content width (accounting for the
  // overlay-scrollbar inset on macOS).
  const pane = panesRef.value;
  if (!pane) return;
  const inset = pane.offsetWidth - pane.clientWidth;
  if (event.clientX < pane.getBoundingClientRect().right - inset - 2) return;
  stopFollowingForUserIntent();
}
