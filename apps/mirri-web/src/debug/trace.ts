// apps/mirri-web/src/debug/trace.ts
...
// `localStorage["mirri-web.debug"]="1"`; key lifecycle metadata is always on.
...
export function downloadTraceLog(list: readonly TraceEntry[] = entries): void {
  ...
  try {
    ...
    anchor.href = url;
    anchor.download = `mirri-web-log-${new Date().toISOString().replaceAll(/[:.]/g, '-')}.jsonl`;
...
/** Serialize the session-export metadata ring as JSONL. Only the always-on
 * key lifecycle events are included — full REST/WS/console diagnostics are
