...
function bundle(): Record<string, string> {
  return {
    'manifest.json': JSON.stringify({ sessionId: 'session_orig', mirriCodeVersion: '0.20.2', workspaceDir: '/w/proj', title: 'demo' }),
    ...
    'logs/mirri-code.log': '2026-06-01T00:00:00.000Z INFO  boot  step=0\n2026-06-01T00:00:01.000Z ERROR  oops  code=500\n',
  };
}
...
describe('imports + logs routes', () => {
  ...
  it('imports a zip and surfaces it in the session list tagged imported', async () => {
    ...
    const body = (await res.json()) as { sessions: { sessionId: string; imported: boolean; importMeta: { manifest: { mirriCodeVersion: string } | null } | null }[] };
    const imported = body.sessions.find((s) => s.sessionId === importId);
    expect(imported).toBeDefined();
    expect(imported!.imported).toBe(true);
    expect(imported!.importMeta?.manifest?.mirriCodeVersion).toBe('0.20.2');
  });
  ...
  it('sanitizes type-corrupt manifest fields so the session list cannot crash', async () => {
    ...
    const corrupt: Record<string, string> = {
      // workspaceDir / mirriCodeVersion are the wrong type — must not reach workDir.
      'manifest.json': JSON.stringify({ sessionId: 'session_orig', workspaceDir: 123, mirriCodeVersion: 7, title: 'demo' }),
    ...
    const body = (await (await sessionsRoute(home).request('/')).json()) as {
      sessions: { sessionId: string; workDir: unknown; importMeta: { manifest: { workspaceDir?: unknown; mirriCodeVersion?: unknown; sessionId?: unknown } | null } | null }[];
    };
    ...
    expect(s.importMeta?.manifest?.mirriCodeVersion).toBeUndefined(); // dropped
