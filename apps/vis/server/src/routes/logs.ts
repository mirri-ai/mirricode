...
import { MIRRICODE_HOME } from '../config';
import { discoverLogFiles, readLogs } from '../lib/log-reader';
import { readSessionDetail } from '../lib/session-store';

const SESSION_LOG_REL = ['logs', 'mirri-code.log'] as const;
const GLOBAL_LOG_REL = ['logs', 'global', 'mirri-code.log'] as const;
const HOME_GLOBAL_LOG_REL = ['logs', 'mirri-code.log'] as const;

export function logsRoute(home: string = MIRRICODE_HOME): Hono {
  const r = new Hono();
  r.get('/:id/logs', async (c) => {
    ...
    // it is captured under the session dir (logs/global/mirri-code.log); for a
    // live local session it lives at <MIRRICODE_HOME>/logs/mirri-code.log
    ...
    // Either log may have rotated (mirri-code.log.1, .2, …); discover the active
