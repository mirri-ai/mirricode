...
function buildTarget(): string {
  return process.env['MIRRICODE_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __MIRRICODE_VERSION__: JSON.stringify(packageJson.version),
    __MIRRICODE_CHANNEL__: JSON.stringify(process.env['MIRRICODE_CHANNEL'] ?? ''),
    __MIRRICODE_COMMIT__: JSON.stringify(process.env['MIRRICODE_COMMIT'] ?? ''),
    __MIRRICODE_BUILD_TARGET__: JSON.stringify(buildTarget()),
    __MIRRICODE_NATIVE_BUNDLE__: 'true',
  },
  deps: {
