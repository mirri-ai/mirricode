...
import {
  ...
  MIRRICODE_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
describe('loadPluginMarketplace', () => {
  ...
  it('loads the default CDN marketplace with injectable fetch', async () => {
    ...
    const marketplace = await loadPluginMarketplace({
      workDir: '/tmp/work',
      source: MIRRICODE_PLUGIN_MARKETPLACE_URL,
      fetchImpl,
    });
  ...
  it('falls back to the source checkout marketplace when the default CDN cannot be fetched', async () => {
    const previous = process.env[MIRRICODE_PLUGIN_MARKETPLACE_URL_ENV];
    delete process.env[MIRRICODE_PLUGIN_MARKETPLACE_URL_ENV];
    const fetchImpl = vi.fn(async () => {
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env[MIRRICODE_PLUGIN_MARKETPLACE_URL_ENV] = previous;
      }
    }
  });
  ...
  it('accepts legacy marketplace type aliases as normal plugins', async () => {
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'mirri-webbridge',
        source: join(dir, 'mirri-webbridge'),
      }),
    );
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
