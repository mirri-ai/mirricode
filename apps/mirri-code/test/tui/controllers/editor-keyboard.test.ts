...
import { DOUBLE_ESC_WINDOW_MS } from '#/tui/constant/mirri-tui';
import {
...
describe('EditorKeyboardController btw panel priority', () => {
  it('closes the btw panel first on Esc while compacting, without cancelling compaction', () => {
  ...
  it('cancels compaction on the next Esc press once the btw panel is gone', () => {
    const { editor, btwCloseOrCancel, cancelCompaction } = createHarness({ isCompacting: true });
  ...
  it('cancels compaction directly on Esc when no btw panel is open', () => {
    const { editor, btwCloseOrCancel, cancelCompaction } = createHarness({ isCompacting: true });
  ...
  it('cancels a running btw question first on Ctrl+C while compacting', () => {
  ...
  it('closes an idle btw panel on Ctrl+C while compacting, without cancelling compaction', () => {
    const { editor, btwCloseOrCancel, cancelCompaction } = createHarness({ isCompacting: true });
    btwCloseOrCancel.mockReturnValue(true);
  ...
  it('cancels compaction on Ctrl+C when no btw panel is open', () => {
