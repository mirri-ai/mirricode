...
import type { ApprovalRequest, ApprovalResponse, Event } from '@mirri-ai/mirri-code-sdk';
import { afterEach, describe, expect, it, vi } from 'vitest';

import { ApprovalPanelComponent } from '#/tui/components/dialogs/approval-panel';
import { EffortSelectorComponent } from '#/tui/components/dialogs/effort-selector';
import { MIRRICODE_PLUGIN_MARKETPLACE_URL } from '#/constant/app';
...
function makeSession(overrides: Record<string, unknown> = {}) {
  ...
  return {
    ...
    setModel: vi.fn(async (model: string) => { currentModel = model; }),
    setThinking: vi.fn(async (effort: string) => { currentEffort = effort; }),
...
async function makeTempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'mirri-code-tui-'));
  tempDirs.push(dir);
  return dir;
}

async function makeExportedSessionZip(content = 'session zip'): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'mirri-code-feedback-export-'));
  tempDirs.push(dir);
...
afterEach(async () => {
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['MIRRICODE_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['MIRRICODE_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('MirriTUI message flow', () => {
  ...
  it('tracks theme changes from slash commands', async () => {
    process.env['MIRRICODE_HOME'] = await makeTempHome();
    const { driver, harness } = await makeDriver();
    harness.track.mockClear();
...
`,
  ...
  it('removes debug timing status from undone turns', async () => {
    const { driver, session } = await makeDriver();
    const previousDebug = process.env['MIRRICODE_DEBUG'];
    process.env['MIRRICODE_DEBUG'] = '1';
    try {
      driver.handleUserInput('hello');
      driver.sessionEventHandler.handleEvent(
        {
      ...
      if (previousDebug === undefined) {
        delete process.env['MIRRICODE_DEBUG'];
      } else {
        process.env['MIRRICODE_DEBUG'] = previousDebug;
      }
    }
  });
  ...
  it('shows an error instead of throwing when skill media materialization fails', async () => {
    ...
    const missing = imageStore.addVideo('video/quicktime', '/tmp/mirri-missing-source.mov');
  ...
  it('shows an error instead of throwing when plugin command media materialization fails', async () => {
    ...
    const { driver } = await makeDriver(session);
    const imageStore = (driver as unknown as { imageStore: ImageAttachmentStore }).imageStore;
    const missing = imageStore.addVideo('video/mp4', '/tmp/mirri-missing-source.mp4');
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ Mirri Code');
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
        ...
        plugins: [
          {
            ...
            source: 'https://mirricode.com/plugins/official/mirri-datasource.zip',
          },
        ],
      }),
      'utf8',
    );
    process.env['MIRRICODE_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        'https://mirricode.com/plugins/official/mirri-datasource.zip',
      );
    });
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
  ...
  it('removes a plugin record without auto-running any cleanup skill', async () => {
    ...
    driver.handleUserInput('/plugins remove mirri-webbridge');
    ...
    await vi.waitFor(() => {
      expect(session.removePlugin).toHaveBeenCalledWith('mirri-webbridge');
    });
    expect(session.activateSkill).not.toHaveBeenCalled();
  });
  ...
  it('shows an inline Official error when the marketplace is unreachable, keeping the panel open', async () => {
    const originalFetch = globalThis.fetch;
    process.env['MIRRICODE_PLUGIN_MARKETPLACE_URL'] = 'https://example.test/marketplace.json';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/Kimi K2\s+Mirri Code ← current/);
    expect(pickerOutput).toMatch(/❯ Kimi Turbo\s+Mirri Code/);
  ...
  it('refreshes only OAuth provider models before opening /model picker', async () => {
    ...
    const refreshOAuthProviderModels = vi.fn(async () => {
      ...
      return { changed: [], unchanged: ['managed:mirri-code'], failed: [] };
    });
    (
