import type { ModelAlias } from '@mirri-ai/mirri-code-sdk';
import { visibleWidth } from '@mirri-ai/pi-tui';
...
describe('ModelSelectorComponent', () => {
  it('lays out the provider as a right column and marks the current model', () => {
    ...
    expect(out).toMatch(/❯ Kimi K2\s+Mirri Code ← current/);
    // Provider is no longer inlined in parentheses next to the name.
    expect(out).not.toContain('Kimi K2 (Mirri Code)');
  });
...
describe('ModelSelectorComponent effort list', () => {
  it('uses support_efforts for selectable efforts', () => {
    const picker = new ModelSelectorComponent({
      models: {
        kimi: effortModel('Kimi K2', ['low', 'high'], 'high'),
      },
      currentValue: 'kimi',
      currentThinkingEffort: 'high',
      onSelect: vi.fn(),
