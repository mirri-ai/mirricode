import type { ExperimentalFeatureState } from '@mirri-ai/mirri-code-sdk';
...
describe('ExperimentsSelectorComponent', () => {
  it('renders searchable feature toggles with source details', () => {
    ...
    expect(out).toContain('    id micro_compaction · config · MIRRICODE_EXPERIMENTAL_MICRO_COMPACTION');
  ...
  it('drafts changes with Space and applies them with Enter', () => {
    ...
    expect(text(selector)).toContain(
      '    id micro_compaction · default · MIRRICODE_EXPERIMENTAL_MICRO_COMPACTION · modified',
    );
