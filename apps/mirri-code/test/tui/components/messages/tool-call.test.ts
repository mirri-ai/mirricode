import { visibleWidth, type TUI } from '@mirri-ai/pi-tui';
...
describe('ToolCallComponent', () => {
  ...
  it('should chip Auto-approved in the header when ExitPlanMode was auto-approved without user review', () => {
    const component = new ToolCallComponent(
      {
        id: 'call_exit_auto',
        name: 'ExitPlanMode',
  ...
  it('truncates a long file path from the head so the filename stays visible', () => {
    const longPath =
      'apps/mirri-code/src/tui/components/messages/tool-renderers/long-path/example/final-file.ts';
    const component = new ToolCallComponent(
      {
        id: 'call_long_path',
        name: 'Read',
    ...
    expect(out).not.toContain('apps/mirri-code/src/tui/components/messages/tool-renderers/long-pa…');
  });
  ...
  it('shows Read paths relative to the active workspace', () => {
    const component = new ToolCallComponent(
      {
        ...
        name: 'Read',
        args: { path: '/tmp/proj-a/apps/mirri-code/src/main.ts' },
      },
      {
    ...
    const expectedReadPath =
      process.platform === 'win32' ? 'apps\\mirri-code\\src\\main.ts' : 'apps/mirri-code/src/main.ts';
  ...
  it('renders a single foreground subagent without the generic Agent tool header', () => {
    ...
    component.appendSubToolCall({
      ...
      name: 'Read',
      args: { path: 'apps/mirri-code/src/tui/utils/background-agent-status.ts' },
    });

    out = strip(component.render(120).join('\n'));
    expect(out).toContain('Explore Agent Running (explore project xxx) · 1 tool · 10s');
    expect(out).toContain('Using Read (apps/mirri-code/src/tui/utils/background-agent-status.ts)');
