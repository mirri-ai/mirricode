...
function makePlan(over: Partial<MigrationPlan> = {}): MigrationPlan {
  return {
    ...
    hasMcp: true,
    hasUserHistory: true,
    oauthCredentials: ['mirri-code.json'],
    workdirs: [],
    detectedPlugins: [],
...
describe('MigrationScreenComponent — result phase', () => {
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedHooks: 0,
            siblingContents: {
              providers: ['openai', 'managed:mirri-code'],
              models: ['gpt4'],
              hooks: 3,
            },
          },
  ...
  it('lists kept config settings on the result screen when kimi-cli differed', () => {
      ...
      makeReport(
        {},
        {
          config: {
            ...
            droppedModels: [],
            droppedKeys: [],
            configConflicts: ['default_model', 'providers.mirricode'],
            wroteSiblingDueToConflict: false,
            wroteTuiSibling: false,
            migratedHooks: 0,
            droppedHooks: 0,
    ...
    expect(out).toContain('default_model · providers.mirricode');
  });
