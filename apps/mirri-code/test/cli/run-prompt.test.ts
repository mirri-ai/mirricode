...
vi.mock('@mirri-ai/mirri-code-oauth', async () => {
  const actual = await vi.importActual<typeof import('@mirri-ai/mirri-code-oauth')>(
...
describe('runPrompt', () => {
  ...
  it('uses the CLI model override when creating a fresh prompt session', async () => {
    await runPrompt(opts({ model: 'mirri-code/k2.5' }), '1.2.3-test', {
    ...
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'mirri-code/k2.5' }),
    );
  });
  ...
  it('applies the CLI model override to resumed prompt sessions', async () => {
    await runPrompt(opts({ session: 'ses_existing', model: 'mirri-code/k2.5' }), '1.2.3-test', {
      stdout: { write: vi.fn(() => true) },
      stderr: { write: vi.fn(() => true) },
    });

    expect(mocks.harnessResumeSession).toHaveBeenCalledWith({ id: 'ses_existing' });
    expect(mocks.session.setModel).toHaveBeenCalledWith('mirri-code/k2.5');
    expect(mocks.initializeTelemetry).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'mirri-code/k2.5' }),
    );
  });
