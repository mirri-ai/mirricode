...
describe('runUpdatePreflight', () => {
  ...
  it('skips all update work when MIRRICODE_NO_AUTO_UPDATE is set', async () => {
    vi.stubEnv('MIRRICODE_NO_AUTO_UPDATE', '1');
  ...
  it('npm-global: prompts and spawns npm install -g when automatic updates are disabled', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        installCommand: 'npm install -g @mirri-ai/mirri-code@0.5.0',
    ...
    expect(stdout.join('')).toContain('Updated @mirri-ai/mirri-code to 0.5.0');
  });
  ...
  it('refreshes a stale cached target before showing the foreground install prompt', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        installCommand: 'npm install -g @mirri-ai/mirri-code@0.7.0',
      }),
    );
    expect(mocks.spawn).toHaveBeenCalledWith(
      expect.stringMatching(/^npm(\.cmd)?$/),
      ['install', '-g', '@mirri-ai/mirri-code@0.7.0'],
      { stdio: 'inherit' },
    );
    expect(stdout.join('')).toContain('Updated @mirri-ai/mirri-code to 0.7.0');
  });
    ...
    try {
      ...
      expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          installCommand: 'npm install -g @mirri-ai/mirri-code@0.6.0',
        }),
      );
    } finally {
      vi.useRealTimers();
  ...
  it('yarn-global: spawns yarn global add', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      ['global', 'add', '@mirri-ai/mirri-code@0.5.0'],
      { stdio: 'inherit' },
    );
  });
  ...
  it('homebrew: prints manual brew upgrade command, does not spawn', async () => {
    ...
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('brew upgrade mirri-code');
    expect(promptForInstallChoice).not.toHaveBeenCalled();
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('native on darwin: spawns bash -c with pipefail-guarded curl|bash', async () => {
    ...
    try {
      ...
      expect(script).toContain('curl -fsSL https://mirricode.com/install.sh');
  ...
  it('native on win32: prints manual powershell command, does not spawn', async () => {
    ...
    try {
      const { stdout, options } = captureOutput();
      await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
      expect(stdout.join('')).toContain('irm https://mirricode.com/install.ps1 | iex');
      expect(promptForInstallChoice).not.toHaveBeenCalled();
      expect(mocks.spawn).not.toHaveBeenCalled();
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform });
  ...
  it('unsupported: prints fallback npm command', async () => {
    ...
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('npm install -g @mirri-ai/mirri-code@0.5.0');
    expect(mocks.spawn).not.toHaveBeenCalled();
  });
  ...
  it('warns and continues when spawn exits non-zero, without claiming success', async () => {
    ...
    expect(stdout.join('')).not.toContain('Updated @mirri-ai/mirri-code');
  });
  ...
  it('infers a background update success notice when the active install version is now running', async () => {
    ...
    expect(stdout.join('')).toContain('Mirri Code updated to v0.5.0');
    expect(writeUpdateInstallState).toHaveBeenCalledWith(expect.objectContaining({
      active: null,
      lastFailure: null,
      lastSuccess: expect.objectContaining({
  ...
  describe('rollout gating', () => {
    ...
    it('MIRRICODE_EXPERIMENTAL_FLAG bypasses the rollout: held devices still update', async () => {
      vi.stubEnv('MIRRICODE_EXPERIMENTAL_FLAG', '1');
      const held = cacheWithManifest(heldForEveryone('0.5.0'));
      mocks.readUpdateCache.mockResolvedValue(held);
      mocks.refreshUpdateCache.mockResolvedValue(held);
      mocks.detectInstallSource.mockResolvedValue('npm-global');
    ...
    it('MIRRICODE_NO_AUTO_UPDATE still wins over the experimental flag', async () => {
      vi.stubEnv('MIRRICODE_EXPERIMENTAL_FLAG', '1');
      vi.stubEnv('MIRRICODE_NO_AUTO_UPDATE', '1');
