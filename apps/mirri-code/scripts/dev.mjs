...
// the whole repo instead of just apps/mirri-code.
const REPO_ROOT = resolve(APP_ROOT, '../..');
// Runtime variable the CLI reads to locate the marketplace JSON.
const MARKETPLACE_ENV = 'MIRRICODE_PLUGIN_MARKETPLACE_URL';
// Opt-in for dev: point this run at an external marketplace instead of a local one.
const EXTERNAL_MARKETPLACE_ENV = 'MIRRICODE_DEV_MARKETPLACE_URL';
...
child.on('error', async (error) => {
  console.error(`Failed to start Mirri Code dev CLI: ${error.message}`);
