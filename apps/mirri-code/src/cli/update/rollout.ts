...
export type PassiveUpdateReason =
  ...
  | 'eligible'
  /** MIRRICODE_EXPERIMENTAL_FLAG is on: rollout skipped, newest always visible. */
  | 'experimental';
 ...
 *
 * `mirri upgrade` must NOT go through this gate — it selects directly from the
 * raw latest version.
 */
 ...
 * truthy values as `MIRRICODE_NO_AUTO_UPDATE`) — the update preflight runs
 * before the harness exists, so the core flag registry is not consulted.
 * `MIRRICODE_NO_AUTO_UPDATE` still wins: disabling updates beats opting in.
 */
export function isRolloutBypassedByExperimentalEnv(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  const value = (env['MIRRICODE_EXPERIMENTAL_FLAG'] ?? '').trim().toLowerCase();
