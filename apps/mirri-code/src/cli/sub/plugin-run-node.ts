...
export async function runPluginNodeEntry(entry: string, args: readonly string[]): Promise<void> {
  const pluginRoot = process.env['MIRRICODE_PLUGIN_ROOT'];
  if (pluginRoot === undefined || pluginRoot.trim().length === 0) {
    throw new Error('MIRRICODE_PLUGIN_ROOT is required to run a plugin node entry.');
  }
  ...
  if (!isWithin(entryReal, rootReal)) {
    throw new Error(`Plugin node entry must be inside MIRRICODE_PLUGIN_ROOT: ${entry}`);
  }
