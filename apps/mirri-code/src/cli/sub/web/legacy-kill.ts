/**
 * `mirri server kill` — deprecated; only stops a server started by an old
 * (pre-`mirri web`, i.e. before 0.28.0) build.
 *
 ...
 * the `mirri web` command tree could leave a background daemon behind; those
 * recorded themselves in the legacy single-instance lock at
 * `<MIRRICODE_HOME>/server/lock`, which the instance registry never sees.
...
/**
 * The first release whose servers run in the foreground (`mirri web`) and
...
/** Deprecation notice printed on every `mirri server kill` run. */
export const DEPRECATED_KILL_NOTICE =
  '`mirri server kill` is deprecated: it only stops servers started by a version before 0.28.0. Servers started by `mirri web` run in the foreground — stop them with Ctrl+C.\n';
...
export function registerLegacyKillCommand(server: Command): void {
  server
    .command('kill')
    .description(
      'Deprecated — stop a server started by a version before 0.28.0 (recorded in the legacy server lock). Servers started by `mirri web` run in the foreground — stop them with Ctrl+C.',
    )
    // Swallow legacy argument shapes (`mirri server kill <serverId>`, flags):
...
export async function handleLegacyKillCommand(deps: LegacyKillDeps): Promise<void> {
  ...
  if (lock === undefined) {
    deps.stdout.write('No running legacy Mirri server.\n');
    return;
  }
  ...
  await deps.removeLock().catch(() => {});
  deps.stdout.write(`Legacy Mirri server (pid ${String(lock.pid)}) ${outcome}.\n`);
}
...
async function killLegacyServer(
  ...
  throw new Error(
    `Failed to stop legacy Mirri server (pid ${String(pid)}); insufficient permissions?`,
  );
}
