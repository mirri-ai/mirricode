/**
 * `mirri web rotate-token` — generate a new persistent server token.
 *
 * Rewrites `<MIRRICODE_HOME>/server.token` (0600, atomic). The previous token
...
import { getLiveServerInstance, rotateServerToken } from '@mirri-ai/kap-server';
