/**
 * `mirri web` — run the local server in the foreground and open the web UI.
 *
...
import { createServerLogger, startServer, type ServerHostIdentity, type ServerLogger } from '@mirri-ai/kap-server';
import { shutdownTelemetry, track } from '@mirri-ai/mirri-telemetry';
import chalk from 'chalk';
import { type Command } from 'commander';

import { CLI_SHUTDOWN_TIMEOUT_MS, CLI_PLATFORM, CLI_USER_AGENT_PRODUCT, WEB_USER_AGENT_SUFFIX } from '#/constant/app';
...
/**
 * `mirri web` — runs the local server in-process, attached to the current
 ...
 * (`MIRRICODE_DEV_SERVER=1`, set by the repo's `dev:server` / `dev:kap-server*`
 * scripts) a missing `dist-web` build is tolerated: the server starts API-only
 * and the web UI is expected to come from the mirri-web Vite dev server.
...
export function serverWebAssetsDir(
  ...
  if (env['MIRRICODE_DEV_SERVER'] === '1' && !existsSync(join(dir, 'index.html'))) {
    return undefined;
  }
  return dir;
}
...
export function formatReadyBanner(
  ...
  // Borderless header: the Mirri sprite sits next to the title, keeping the
  // brand without the enclosing box.
  const logo = ['▐█▛█▛█▌', '▐█████▌'] as const;
  const lines: string[] = [
    '',
