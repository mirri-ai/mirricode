...
export function initializeCliTelemetry(options: InitializeCliTelemetryOptions): void {
  initializeTelemetry({
    ...
    getAccessToken: async () =>
      (await options.harness.auth.getCachedAccessToken(MIRRICODE_PROVIDER_NAME)) ?? null,
  });
...
/**
 * Bootstrap telemetry for the `mirri web` / `mirri server run` host.
 *
...
export function initializeServerTelemetry(
  ...
  initializeTelemetry({
    ...
    getAccessToken: async () => (await auth.getCachedAccessToken(MIRRICODE_PROVIDER_NAME)) ?? null,
  });
