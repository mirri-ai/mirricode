...
import { MIRRI_BUILD_INFO } from '#/cli/build-info';
...
function currentTarget(): string {
  return MIRRI_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  const cacheDirEnv = optionalEnvValue(env, 'MIRRICODE_CACHE_DIR');
  if (cacheDirEnv !== null) return cacheDirEnv;

  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'mirri-code');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'mirri-code')
      : pathWin32.join(home, 'AppData', 'Local', 'mirri-code', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'mirri-code');
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? MIRRI_BUILD_INFO.version ?? 'dev');
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.mirri-native-entry.cjs');
  ensureFile(
    entryPath,
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = MIRRI_BUILD_INFO.version ?? 'dev';
