...
import { detectMigration, type MigrationPlan } from '@mirri-ai/migration-legacy';
...
export async function detectPendingMigration(
  ...
  if (input.ignoreMarker !== true) {
    if (migrationAlreadyTargeted(join(sourceHome, '.migrated-to-mirri-code'), targetHome)) {
      return null;
    }
  ...
  // only data is `credentials/*.json` has nothing to offer — mirri-code's own
...
/**
 * True when the legacy `.migrated-to-mirri-code` marker records a migration
 * into *this* target home. A marker written for a different `MIRRICODE_HOME`
