 ...
 * conventions in `apps/mirri-code/src/tui/components/dialogs/choice-picker.ts`.
 *
 ...
 */
import { Container, matchesKey, Key, truncateToWidth, type Focusable } from '@mirri-ai/pi-tui';
...
} from '@mirri-ai/migration-legacy';
...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  private renderResult(width: number): string[] {
    ...
    if (r !== undefined) {
      ...
      // safely be held by two installs at once). mirri-code's normal auth flow
      ...
      lines.push('');
      lines.push(
        chalk.hex(colors.textMuted)(' Old data kept at ~/.mirricode/ — kimi-cli still works.'),
      );
    }
    lines.push('');
    lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to mirri-code'));
...
function stepFor(phase: Phase, plan: MigrationPlan): StepDef {
  if (phase === 'ask1') {
    return {
      title: 'Migrate this data to mirri-code?',
      options: [
