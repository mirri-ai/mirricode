...
import { truncateToWidth, visibleWidth } from '@mirri-ai/pi-tui';
import chalk from 'chalk';

import { effectiveModelAlias } from '@mirri-ai/mirri-code-sdk';
...
export class WelcomeComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (safeWidth < 24) {
      const title = chalk.bold.hex(currentTheme.palette.primary)('Welcome to Mirri Code!');
    ...
    const rightRow0 = truncateToWidth(
      chalk.bold.hex(currentTheme.palette.primary)('Welcome to Mirri Code!'),
      textWidth,
      '…',
    );
