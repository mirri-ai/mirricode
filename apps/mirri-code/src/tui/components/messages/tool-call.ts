...
import { Container, Spacer, Text, truncateToWidth, visibleWidth } from '@mirri-ai/pi-tui';
import type { Component, TUI } from '@mirri-ai/pi-tui';
...
import type { TokenUsage } from '@mirri-ai/mirri-code-sdk';
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `MirriTUI` calls `setPlanInfo` with
  ...
  // when MirriTUI routes a `subagent.event` with this tool call
   ...
   * this: plan-file mode leaves `args.plan` empty, so `MirriTUI` fetches
  ...
  // ── Subagent API (called by MirriTUI event routing) ───────────────
