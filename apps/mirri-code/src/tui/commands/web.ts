 ...
 *
 * Equivalent to `mirri server run` (ensures the background daemon is up) plus
 * `mirri web` (opens the browser), but deep-linked to the active session and
...
export async function handleWebCommand(host: SlashCommandHost): Promise<void> {
  ...
  // the `#token=` fragment — matching the `mirri web` subcommand. Show the URL
