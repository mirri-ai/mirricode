# apps/mirri-code Development Guide

This file only contains rules local to `apps/mirri-code`. For cross-repo rules, see the root `AGENTS.md`.
...
- `apps/mirri-code` may only use core capabilities through `@mirri-ai/mirri-code-sdk`. Do not import `@mirri-ai/agent-core` directly in app code.
