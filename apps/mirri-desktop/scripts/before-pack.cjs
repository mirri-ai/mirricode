...
// prebuilt Mirri SEA backend into `resources-stage/bin/<target>/` so that the
// `extraResources` rule copies exactly that one binary into the packaged app's
// resources. sea-path.ts resolves `<resources>/bin/<target>/mirri[.exe]` at
...
exports.default = async function beforePack(context) {
  ...
  const exe = platform === 'win32' ? 'mirri.exe' : 'mirri';

  const desktopRoot = resolve(__dirname, '..');
  const seaDir = resolve(desktopRoot, '..', 'mirri-code', 'dist-native', 'bin', target);
  const seaExe = join(seaDir, exe);
  if (!existsSync(seaExe)) {
    throw new Error(
      `Bundled Mirri server not found for ${target} at ${seaExe}. ` +
        `Build it for this platform first: \`pnpm -C apps/mirri-code build:native:sea\` ` +
  ...
  console.log(`[before-pack] staged Mirri server (${target}) -> ${stageDir}`);
};
