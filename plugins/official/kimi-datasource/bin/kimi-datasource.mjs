...
const VERSION = '3.2.0';
const DEFAULT_MIRRICODE_OAUTH_HOST = 'https://auth.kimi.com';
const DEFAULT_MIRRICODE_BASE_URL = 'https://api.kimi.com/coding/v1';
...
const TOOLS = [
  {
    ...
    description:
      "Dispatch a call to any registered data source's API via the Mirri Code gateway. Always call get_data_source_desc(name) first to learn that source's available APIs and required params, then construct this call with api_name and params taken from that description.",
    inputSchema: {
      type: 'object',
      properties: {
        data_source_name: {
...
function resolveKimiCodeCredentialName() {
  ...
  // Keep this in sync with packages/oauth/src/managed-mirri-code.ts.
  const digest = createHash('sha256')
    .update(JSON.stringify({ oauthHost, baseUrl }))
    .digest('hex')
    .slice(0, 16);
  return `mirri-code-env-${digest}`;
}
...
async function loadAccessToken() {
  ...
  if (!isRecord(parsed)) {
    throw new Error(`Invalid Mirri Code credentials file: ${credentialsFile}`);
  }
  const token = typeof parsed.access_token === 'string' ? parsed.access_token : '';
  if (token.length === 0) {
    throw new Error('Mirri Code credentials do not contain access_token. Run /login again.');
  }
