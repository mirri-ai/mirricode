...
本页记录 Mirri Code CLI 每个版本的变更内容。

## 1.8.0（2026-07-26）

### 新功能

- web: 在设置 > MCP 服务器中新增 MCP 服务器和工具的运行时开关。无需重启即可启用或禁用服务器和工具——被禁用的项目将对 LLM 隐藏并在执行时被阻止。
- web: 新增"模型"设置标签页，用于管理提供商和编辑模型元数据，支持基于目录的提供商发现和手动录入回退。
- web: 将设置对话框升级为全屏设置视图，左侧标签导航和返回工作区按钮，为"模型"和"配置文件"标签页提供全窗口宽度。

### 修复

- 在构建门禁中新增 DMG 打包步骤，使完整构建能生成桌面磁盘映像。
- web: 修复 macOS 标题栏包装器破坏应用网格布局导致的白屏问题。桌面标题栏现在使用网格行而非弹性包装器，恢复所有平台上的侧边栏和对话面板。

## 1.7.0（2026-07-24）

### 新功能

- 新增 `PreLlmRequest` 和 `PostLlmRequest` 钩子事件，用于 LLM 调用可观测性。在 `[[hooks]]` 中配置 `event = "PreLlmRequest"` 或 `event = "PostLlmRequest"` 以跟踪模型使用情况、Token 消耗和请求计时。

### 修复

- web: 修复后台子代理完成后在任务列表中仍显示为运行中的问题。
- 新增 `GET /api/v1/tools/all` 端点，在无需活跃会话的情况下返回内置工具描述符。Web 设置面板使用此端点替代 `GET /tools`，使会话范围的工具列表不再被配置管理所占用。

## 1.6.0（2026-07-23）

### 新功能

- web: 允许从 Web UI 覆盖内置代理配置文件的所有字段（描述、工具、模型、提示模板、角色指令），而不仅仅是默认模型。被覆盖的内置代理同时显示"内置"和"自定义"徽章。"恢复默认"按钮可移除所有覆盖并恢复原始配置文件。
- 新增自定义代理配置文件：在 `~/.mirri-code/agents/` 或 `.mirri-code/agents/` 下的 YAML 文件中定义自定义代理，支持每个代理的默认模型、启用/禁用控制，以及 Agent 和 AgentSwarm 工具上的 `model` 参数，使主代理可以选择每个子任务的模型。通过 `extra_agent_dirs` 配置发现目录，通过 `disabled_agents` 禁用代理。内置代理可被部分覆盖（例如仅覆盖 `defaultModel` 而不替换完整配置文件）。自定义代理自动对 LLM 可见且可调度。被禁用的代理对 LLM 不可见且无法生成。Web UI 包含模型选择器下拉菜单和带自动完成的标签工具编辑器。
- 新增子代理分发的 LLM 感知模型选择：`config.toml` 中带有 `description` 字段的模型现在暴露在 Agent 和 AgentSwarm 工具描述中，使 LLM 能够在覆盖默认模型时做出明智决策。代理配置文件在工具描述中显示其 `defaultModel`，无效的模型别名返回列出有效选项的可操作错误。

### 修复

- 修复自定义代理配置文件删除在配置文件缺失或已被移除时静默失败的问题。删除路由现在返回正确的错误信封而非原始 500 错误。
- web: 修复因未定义 CSS 变量导致代理配置文件工具编辑器中工具建议下拉菜单不出现的问题。同时通过添加专用 z-index 层修复删除确认对话框被设置对话框遮挡的问题。
- web: 改进代理配置文件工具建议列表布局，每项 2-3 行并带有彩色组徽章（内置、MCP 服务器、MCP 工具）。将描述和"使用场景"字段更改为多行文本区域。修复编辑现有配置文件时扩展变更自动填充工具不生效的问题。

## 1.5.5（2026-07-21）

### 修复

- web: 修复发送粘贴的图片加文字时出现重复用户消息的问题。

## 1.5.4（2026-07-20）

### 新功能

- 新增 `/copy` 斜杠命令，将最后一条助手消息复制到剪贴板。
- 将思考强度（thinking effort）限定为当前会话范围而非全局。在轮次遥测事件中添加 `thinking_effort`。
- web: 在模型切换器中添加缓存失效提示，以便用户知道切换模型后可能需要重新加载页面。

### 优化

- 默认以前台模式运行 `mirri web` 和 TUI `/web` 命令，不再后台守护进程；命令会保持连接到终端直到 Ctrl+C。传入 `--background` 可保持之前的行为。已运行的服务器会在升级时被原样复用，并在输出中提示版本不匹配。
- 统一 YOLO 和 Auto 权限模式描述：YOLO 自动批准常规工具操作但代理仍可能提问，而 Auto 完全自主从不提问。措辞现在在 CLI `--help`、TUI 斜杠命令、Web 斜杠命令列表、ACP 适配器模式描述和配置文档中保持一致。

### 修复

- 会话调试导出的默认文件名现在包含时间戳（`mirri-debug-<shortId>-<timestamp>.zip`），避免同一会话的重复导出互相覆盖。
- 修复中断模型响应后重复请求被拒绝的问题，通过记录未运行的工具调用并以中断结果关闭它们来解决。
- **安全修复：** 修复 FetchURL SSRF 绕过和 DNS 重绑定窗口。加固内置 URL 抓取工具，使精心构造的域名和重定向链无法再访问回环或内部网络服务：主机名解析后每个地址都会被检查是否属于回环/RFC1918/链路本地/CGNAT/ULA 范围（包括 IPv4 映射的 IPv6 形式），重定向手动跟随并每次跳转都重新运行安全检查，每个请求的连接绑定到已验证的 DNS 答案以防止连接时重新解析被重绑定到内部地址。
- 在 OAuth 连接错误消息中包含传输层根因。
- 修复 Esc 和 Ctrl+C 在关闭打开的 `/btw` 面板前取消正在进行的压缩的问题。`/btw` 面板堆叠在对话记录之上，现在会在任何压缩或流取消逻辑运行前先关闭。
- 隐藏 TUI 对话记录中的纯空白思考内容。
- web: 按模型记住思考强度，修复当活动模型不支持先前存储的级别时思考选择器为空且无响应的问题。
- 修复以符号链接安装的 AGENTS.md 文件被配置文件上下文加载器忽略的问题。`isFile()` 辅助函数现在向 `kaos.stat()` 传入 `{ followSymlinks: true }`，使符号链接的 AGENTS.md 文件能被解析到目标并正常加载。
- 修复通过网络访问 Web UI 时 LaTeX 公式渲染为乱码重叠文本的问题；服务器的内容安全策略现在允许数学和代码高亮所依赖的内联样式，同时严格限制脚本。

## 1.5.3（2026-07-20）

### 修复

- 支持 `mcp.json` 字符串值中的 `${VAR}` 和 `${env:VAR}` 环境变量插值，使密钥和主机名不再需要硬编码。未定义变量解析为空字符串，无效结果会被 schema 验证拒绝。

## 1.5.2（2026-07-18）

### 新功能

- web: 新增开发后端切换器和引擎徽章，用于双引擎调试。

### 优化

- 在自动权限模式下，计划退出现在在工具结果和对话记录中标记为自动批准（而非用户审核），代理不再将自动计划批准视为用户信号来开始执行。
- 优化上下文使用量显示的单位格式：从 1000 进制切换到 1024 进制（262144 → "256k"），使用向上取整的整数百分比，并在 TUI 和 Web 之间共享单一格式化器。
- web: 在加载屏幕上随机显示五个 logo 变体之一，适应系统主题（深色/浅色模式）。
- web: 用正交的工作事实（busy、main_turn_active、pending_interaction、last_turn_reason）替代五值会话状态枚举，防止状态转换在流式传输期间丢失或重复的竞态条件。

### 修复

- 修复子代理完成被标记为会话轮次完成的问题，这会在主轮次仍在运行时触发过早的完成通知、提示音和未读标记。
- web: 刷新页面后恢复 swarm 成员列表。
- web: 在 v2 引擎上保持上下文使用量实时更新。
- web: 让工作区选择器适配其内容。
- web: 在内容安全策略中允许图片源。
...
- 在 `Read`、`WebSearch` 和 `FetchURL` 内置工具上声明 `code.read`、`web.search` 和 `web.fetch` 能力，以便通过 `integrations.yaml` 被 MCP 工具替换或增强。

## 1.5.0（2026-07-15）

### 新功能

- 允许 explore 和 plan 子代理通过能力注册表发现 `integrations.yaml` MCP 工具。在 `integrations.yaml` 中配置 MCP 工具的能力，使其自动在子代理中可用。
- 新增工具能力抽象层，通过分层 `integrations.yaml`（用户全局 `~/.mirri-code/integrations.yaml` + 项目本地 `<project>/.mirri-code/integrations.yaml`，项目覆盖用户）使外部 MCP 工具可以优先于内置工具使用。声明 MCP 工具能力（如 `code.explore`）可在系统提示中将其排在内置 Grep/Glob 之前。提供商排序现在完全由 `preferOver` 驱动；之前硬编码的 MCP 优先级断已移除（已有配置中已声明 `preferOver` 的不受影响）。
- 新增 `background.print_background_mode`（`exit`/`drain`/`steer`）用于 `mirri -p`：在 `steer` 模式下，完成的后台任务（包括 `Bash(run_in_background=true)`）的行为类似后台子代理——它注入一条合成用户消息，引导主代理进入新的轮次以处理结果。受 `print_wait_ceiling_s` 和新的 `print_max_turns` 约束。

### 优化

- 新增可配置的子代理超时，通过 `[subagent] timeout_ms` 配置，将默认值从 30 分钟提升至 2 小时。
- 导入自定义注册表时识别 `support_efforts` 和 `default_effort` 字段，使思考强度级别对这些模型可用。
- web: 修复会话过多时侧边栏卡顿（避免重复扫描会话列表）；在模型选择器下拉菜单中按 id 匹配当前模型，避免同名但不同提供商的模型出现错误勾选；切换到支持思考强度的模型时自动启用默认思考强度；将服务器访问令牌在标签页关闭和浏览器重启后保留最多 7 天；当服务器暂时不可达时保持连接启动画面并重试首次加载的认证检查。

### 修复

- 将文档 URL 从 `docs.mirricode.com` 更新为 `mirricode.com`。
- web: 切换到支持思考强度的模型时自动启用默认思考强度。
- web: 修复会话在重连后卡在发送状态的问题，使工作指示器停止且下一条消息在轮次完成时正常发送。
- web: 当服务器暂时不可达时保持连接启动画面并重试首次加载的认证检查。
- web: 宽 Markdown 表格在表格内水平滚动并在桌面上增长到 1040px 超出阅读列，对话大纲在表格经过时临时隐藏。
- web: 修复浏览对话记录时聊天视图向下跳动的问题。
- 修复 WebP 解码器无法打包到独立二进制文件中的问题，该问题导致 `pnpm build:native:sea` 和发布版单文件 CLI 失败。
...
- 修复 `mirri -p` 会丢弃启动较晚或运行时间较长的后台子 Agent、导致结果无法返回主 Agent 的问题。
...
- Anthropic 供应商（Claude 与 Kimi 的 Anthropic 兼容模式）现在默认保留历史轮次的思考内容，与 Mirri 默认行为一致；可通过 `[thinking] keep = "off"` 或 `MIRRICODE_MODEL_THINKING_KEEP=off` 关闭。
...
- Mirri 模型开启 Thinking 时默认跨轮次保留推理，可设置 `[thinking] keep = "off"` 关闭。

## 0.22.3（2026-07-04）

### 修复

- `mirri -p` 会在后台子 Agent 完成并返回结果后再退出，避免提前结束本轮。
- web: 修复 web 聊天中已上传视频无法播放的问题。
- 回退近期 TUI 对话渲染改动，恢复上游原始行为，修复相关渲染问题。

### 优化

- `mirri server run` 新增 `--dangerous-bypass-auth` 与 `--keep-alive` 选项，可在可信网络中跳过 token 校验运行服务器，并突破空闲超时保持存活。
...
- 修复 Windows 上 `mirri upgrade` 在安装新版本时因 spawn 错误而失败的问题。
...
- 启动时从用户登录 shell 补充 PATH，使 shell 命令能找到用户自行安装的工具（如 Homebrew 的 `gh`），即使 mirri-code 启动时未继承完整的 profile PATH。
...
- 强制退出无头运行（`mirri -p`），以免运行残留的引用句柄让已完成的运行一直存活到外部超时；同时为 prompt 清理加上时限，避免某个卡住的关闭步骤拖挂整个关闭流程。
...
- Mirri Code 现支持 Anthropic 兼容协议，并支持视频输入。
- web UI 新增完成提示音与问题通知，并在设置中分别提供完成通知、问题通知和提示音的开关。问题通知默认关闭，仅在用户主动开启后才会将问题文本发送到桌面。
- 新增 `MIRRICODE_CUSTOM_HEADERS` 环境变量，用于自定义出站 LLM 请求头，并向非 Mirri 供应商发送 `User-Agent` 请求头。将 `MIRRICODE_CUSTOM_HEADERS` 设为由换行分隔的 `Name: Value` 行。
...
- 将 Anthropic 兼容协议上的 Mirri Code 模型改走 beta Messages API。
...
- 插件现支持在 `mirri.plugin.json` 中声明生命周期 hooks，在指定阶段运行脚本。详见[插件 Hooks](../customization/plugins.md#插件中的-hooks)。
- `/feedback` 现支持附加诊断日志与代码库上下文。
- 新增 `mirri update` 命令，等价于 `mirri upgrade`，可用于升级到最新版本。
...
- 修复 Windows 上 mirri server 首次运行后无法启动的问题。
...
- CLI 新增 `--host` 选项，可通过 `mirri web --host` 将服务器暴露到互联网，并加固 token 鉴权、限流等安全措施。
- Web UI 支持渲染 LaTeX 行间公式（`$$…$$`）。

### 修复

- 修复 Linux 上由未处理的原生剪贴板错误导致的启动崩溃。
- 修复当 CLI 通过 npm/pnpm 安装或从源码运行时，`mirri web` 和 `/web` 在 Windows 上因 `spawn EFTYPE` 无法启动后台服务器守护进程的问题。官方单二进制安装脚本不受影响。
...
- 将 `/plugins` 重新设计为单个标签页面板：**Installed**（管理已安装插件——切换、移除、MCP、详情、重新加载）、**Official**（Mirri 维护的 marketplace 插件）、**Third-party**（来自其他发布者的 marketplace 插件）以及 **Custom**（直接从 GitHub URL、zip URL 或本地路径安装）。使用 `Tab` / `Shift-Tab` 切换标签页。
...
- 新增添加额外工作区目录的能力：
  ...
  - 使用 `mirri --add-dir <path>` 在启动时添加它们。
  - 项目级本地配置现在由 `.mirri-code/local.toml` 管理；我们建议将其添加到你的 `.gitignore` 中。
...
- 新增 Mirri Code Web 模式，可通过 `mirri web` 或 CLI 内的 `/web` 启动，在浏览器中的聊天界面继续会话。
...
- 新增内置的 `mirri vis` 命令，可在浏览器中启动会话可视化工具，并指向本地会话。支持 `--port`/`--host`、`--no-open` 以及 `mirri vis <sessionId>` 深度链接。
...
- 修复 Xcode 26.5 MCP server 发出的 JSON Schema 类型与 Mirri 不兼容的问题。
...
- 修复 Mirri Datasource，使其在当前 Mirri Code 环境中使用匹配的 OAuth 凭证和服务端点。
...
- 新增由环境变量 `MIRRICODE_EXPERIMENTAL_SUB_SKILL` 控制的实验性子 Skill 发现能力。随附 `sub-skill` 内置包（`sub-skill.review`、`sub-skill.consolidate`），用于盘点 Skill 并将其整理为分层分组。
- 新增以下环境变量：
  - `MIRRICODE_MODEL_TEMPERATURE`、`MIRRICODE_MODEL_TOP_P` —— 全局应用于任意 `mirri` 供应商的采样参数（不绑定到 `MIRRICODE_MODEL_NAME`）。
  - `MIRRICODE_MODEL_THINKING_KEEP` —— Mirri 的 preserved-thinking 透传（`thinking.keep`），仅在开启 Thinking 时注入。
  - `MIRRICODE_NO_AUTO_UPDATE`（旧别名 `MIRRICODE_CLI_NO_AUTO_UPDATE`）—— 完全禁用更新预检（不检查、不后台安装、不提示）。
...
- 支持 `mirri acp` 子命令：mirri-code 现在可通过 stdio 使用 [Agent Client Protocol 0.23](https://agentclientprotocol.com/)，因此 IDE（Zed、JetBrains AI Chat、自定义客户端）可以直接驱动会话；覆盖矩阵、Zed 配置和破坏性预发布说明见 [mirri acp 子命令页面](https://moonshotai.github.io/mirri-code/zh/reference/mirri-acp.html)。
...
- 新增实验性 goal 模式，用于需要多轮处理的较长任务。在启动 Mirri 前设置 `MIRRICODE_EXPERIMENTAL_GOAL_COMMAND=1` 即可开启。
  在终端界面中使用 `/goal <objective>` 让 Mirri 跨轮次持续专注于同一任务。例如：
  ```text
  /goal Fix the failing checkout test
  ```
  Mirri 会在终端界面中显示目标，并在工作过程中保持进度可见。使用 `/goal status`、`/goal pause`、`/goal resume`、`/goal cancel` 和 `/goal replace <objective>` 来管理该目标。该功能仍处于实验阶段，欢迎试用并反馈改进建议。
- 新增 `mirri provider` CLI 子命令，支持 `add`、`remove`、`list` 以及 `catalog list` / `catalog add` 操作，可在不启动终端界面的情况下导入和管理来自自定义 registry（api.json）或公开 models.dev 目录的供应商。
- 新增后台结构化提问，让 Agent 在等待用户回答时也能继续工作。
- 新增后台自动更新，可在 tui.toml 中关闭。
- 新增 `/undo` 斜杠命令，用于从对话历史中撤回上一条提示词，并在撤回时保持回放记录同步。
- 新增 `mirri upgrade` 命令，用于手动检查并升级 Mirri Code CLI。
...
- 新增 `MIRRICODE_MODEL_ADAPTIVE_THINKING`（以及对应的 `adaptive_thinking` 模型别名字段），用于强制开启或关闭自适应 thinking（`thinking: { type: 'adaptive' }`），覆盖基于 Anthropic 模型名的版本推断。这样一来，背后由支持自适应能力的模型驱动、且使用自定义名称的兼容端点，即使模型名没有编码出可解析的 Claude 版本，也能选择启用该能力。
...
- 新增 `MIRRICODE_MODEL_*` 环境变量通道，让你无需编辑 `config.toml` 即可让 Mirri Code 使用指定模型（供应商类型、base URL、API 密钥、上下文大小、能力以及 thinking 设置）。
- 支持直接从 GitHub 仓库 URL 安装 plugin，并在 plugin 管理器中展示每次安装的来源和信任级别（mirri-official、curated、third-party）。
...
- 改进终端界面中的用量信息展示。
- 将 plugin 信任徽章限制为仅匹配 Mirri 托管的 plugin CDN URL 模式。
...
- 增强 `mirri export`，在 manifest 中记录更多诊断信息。
...
- 在控制终端消失时及时退出。终端界面现在会处理 `SIGHUP` / `SIGTERM` 信号以及 stdout/stderr 的 `EIO` / `EPIPE` / `ENOTCONN` 错误，避免父 shell 或终端复用器异常退出后残留占用 CPU 核心的 `mirri` 进程。
...
- 将 `X-Msh-Platform` 请求头的取值修正为 `mirri_code_cli`。
...
- 默认让 Mirri 请求使用剩余的上下文窗口作为补全 token 的额度，同时将显式设置的环境变量上限作为硬上限保留。
