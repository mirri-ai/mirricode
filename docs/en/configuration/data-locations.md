...
├── agents/                 # Mirri-specific user-level custom agent profiles (optional)
├── plugins/
...
├── logs/
│   └── mirri-code.log       # Global diagnostic log
├── updates/
│   ├── latest.json
│   ├── install.json
│   ├── install.lock
...
- **`agents/`**: Mirri-specific user-level custom agent profiles (YAML files). See [Agents and Sub-Agents](../customization/agents.md).
...
- [Environment variables](./env-vars.md) — detailed usage of `MIRRICODE_HOME` and related path variables
