...
- Fix `mirri -p` runs exiting with code 0 when a turn fails.
...
- Enrich PATH from the user's login shell at startup, so shell commands find user-installed tools (e.g. Homebrew's `gh`) even when mirri-code was launched without the full profile PATH.
...
- Route Mirri Code models on the Anthropic-compatible protocol through the beta Messages API.
...
- Repair mismatched JSON Schema types emitted by Xcode 26.5 MCP server for Mirri compatibility.
...
- Set the CLI process title to mirri-code during startup.
