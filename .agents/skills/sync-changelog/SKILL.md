---
name: sync-changelog
description: Use after a release succeeds, when maintainers need to sync apps/mirri-code/CHANGELOG.md into docs/en/release-notes/changelog.md and docs/zh/release-notes/changelog.md, then open a PR on a dedicated branch.
---
...
`mirri-code` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@mirri-ai/mirri-code`, writes its changelog here:

```text
apps/mirri-code/CHANGELOG.md
```
...
- The top of `apps/mirri-code/CHANGELOG.md` contains version blocks that are not yet in `docs/en/release-notes/changelog.md`.
- The `gen-docs` flow does not run this automatically; maintainers must explicitly do it after release.

Do **not** run this before the Release PR is merged. At that point, changesets has not yet written the new version into `apps/mirri-code/CHANGELOG.md`.

## Source And Targets

| File | Role | Edited by |
|---|---|---|
| `apps/mirri-code/CHANGELOG.md` | **Only upstream source**, generated by changesets | Never edit manually |
...
- The released version exists on npm (`npm view @mirri-ai/mirri-code versions --json`) or has a matching GitHub Release tag.
- The top of `apps/mirri-code/CHANGELOG.md` is that new version.
...
```bash
rg '^## ' apps/mirri-code/CHANGELOG.md | head -5
...
- The upstream H1 `# @mirri-ai/mirri-code` because the docs page already has `# Changelog`.
...
Drop SDK-only and provider-internal detail. This changelog serves `@mirri-ai/mirri-code` CLI and web users. Within an entry, keep only what CLI/web users can perceive, and remove sentences that document internals instead of user-visible behavior. Apply this on both the English and Chinese pages:
...
```bash
git log -1 --format=%cs "@mirri-ai/mirri-code@<version>"
```
...
```text
docs(changelog): sync <version range> from apps/mirri-code/CHANGELOG.md
```
...
- Synced `<version range>` from `apps/mirri-code/CHANGELOG.md` into `docs/en/release-notes/changelog.md`
...
- Never edit upstream `apps/mirri-code/CHANGELOG.md`.
- Do not backfill unreleased `.changeset/*.md` drafts into the docs site.
- Prefix web UI entries with `web: ` (when the upstream commit touches `apps/mirri-web/`), and keep the prefix on both the English and Chinese pages.
...
| Mistake | Fix |
|---|---|
| Adding entries directly to the English docs page without reading upstream | Use `apps/mirri-code/CHANGELOG.md` as the source |
...
| Forgetting or translating the `web:` prefix on web UI entries | Prefix web UI entries (commit touches `apps/mirri-web/`) with `web: ` on both pages; keep the prefix as-is when translating |

## Stop Signals

- The top version in `apps/mirri-code/CHANGELOG.md` is not published on npm or GitHub Releases.
- You are about to edit `apps/mirri-code/CHANGELOG.md`.
