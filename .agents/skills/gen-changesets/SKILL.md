---
name: gen-changesets
description: Use when generating changesets in the mirri-code repository, including package bump selection, internal package and CLI bundle handling, bump levels, major confirmation, and English changelog wording.
---

# Generate Changesets

`mirri-code` uses changesets to manage versions and changelogs. The current user-facing published package is:
...
3. **Map ignored internal changes to the affected released package.** If an ignored internal package changes CLI output or behavior, list `@mirri-ai/mirri-code` and describe the actual user-visible or release-artifact change in the changelog text.
4. **Internal package source changes that enter the CLI bundle must manually list the CLI.** `@mirri-ai/mirri-code` inline-bundles `@mirri-ai/*` source, but those internal packages are devDependencies from the CLI's perspective, so changesets will not automatically propagate bumps. If a change enters the CLI output, list `@mirri-ai/mirri-code`.
   - **Web app (`@mirri-ai/mirri-web`) changes always enter the CLI bundle.** `@mirri-ai/mirri-web` is ignored by changesets (see `.changeset/config.json`) and cannot be mixed with `@mirri-ai/mirri-code` in one changeset frontmatter. Describe the web change in the changelog text, but list `@mirri-ai/mirri-code` so the CLI release carries the bundled `dist-web` output.
...
6. `@mirri-ai/vis` / `vis-server` / `vis-web` are ignored by changesets and should not be handled.
...
3. If an ignored internal package change enters the CLI bundle, put `@mirri-ai/mirri-code` in frontmatter instead of mixing the ignored package into the same changeset.
...
`@mirri-ai/mirri-web` is ignored by changesets and must **never** appear in a changeset frontmatter. Because the web app is bundled into the CLI release artifact, any web change that ships must list `@mirri-ai/mirri-code` instead and describe the actual web-facing change in the text.

- Prefix the changelog entry text with `web: ` (for example `web: Fix the chat not scrolling to the bottom after sending a message.`) so the synced docs changelog can mark web UI entries. Apply this whenever the change is to the web project (`@mirri-ai/mirri-web`).
...
## `@mirri-ai/pi-tui` changes

`@mirri-ai/pi-tui` is a vendored fork that lives in `packages/pi-tui`. It is `private: true` and is never published, but it is **not** ignored by changesets: changesets versions it and writes `packages/pi-tui/CHANGELOG.md` so the fork keeps its own history. Because it is bundled into the CLI like other internal packages, it is an exception to Core Rule 4 — do **not** list `@mirri-ai/mirri-code` for a change that only touches pi-tui.

- Changes that only affect pi-tui (build, package, strict-mode cleanup, renderer fixes): list `@mirri-ai/pi-tui` only. No CLI changeset.
- If the same change is also user-visible in the CLI (for example a terminal rendering fix that CLI users can see), add a **separate** changeset that lists `@mirri-ai/mirri-code` with CLI-focused wording, in addition to the pi-tui changeset. Do not mix both packages in one frontmatter — the two changelogs need different wording.

pi-tui-only change:

```markdown
---
"@mirri-ai/pi-tui": patch
---
...
- Internal package source enters the CLI bundle, but `@mirri-ai/mirri-code` is missing.
- A changeset frontmatter mixes ignored internal packages with non-ignored packages.
- `packages/node-sdk` was not changed, but `@mirri-ai/mirri-code-sdk` was listed for "internal package sync".
...
- A change that only touches `@mirri-ai/pi-tui` lists `@mirri-ai/mirri-code` instead of `@mirri-ai/pi-tui`, or mixes both packages in one frontmatter.
