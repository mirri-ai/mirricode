...
Mirri Code already has opinions on CLI/TUI behavior, agent workflows, and public APIs. If your change shifts that direction, open an issue first so we can align before you invest time in a PR.
...
- `apps/mirri-code` — CLI / TUI
- `apps/vis` — session replay & debugging visualizer
- `packages/node-sdk` — public TypeScript SDK (`@mirri-ai/mirri-code-sdk`)
