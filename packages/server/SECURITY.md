...
Operational guide for exposing the `@mirri-ai/server` HTTP/WebSocket API beyond
...
token (printed in the startup banner); `MIRRICODE_PASSWORD` is an optional additional
...
- Authentication is the persistent bearer token printed in the startup banner; send it
  as `Authorization: Bearer <token>`. `MIRRICODE_PASSWORD` is optional and adds a
...
- Authentication is the persistent bearer token printed in the startup banner;
  `MIRRICODE_PASSWORD` is optional. `--insecure-no-tls` is mandatory here because
...
- **Stored password hash** — the current path is env-only. There is **no**
  ...
  ```
  node -e "require('bcryptjs').hash(process.env.MIRRICODE_PASSWORD,12).then(h=>console.log(h))"
  ```
...
- **WebSocket:** send either an `Authorization: Bearer <token|password>` header or
  the subprotocol `Sec-WebSocket-Protocol: mirri-code.bearer.<token>` (for browsers
