...
- Serves the built-in web UI (`apps/mirri-web`) as static assets when a
...
# Checks
pnpm --filter @mirri-ai/server typecheck   # tsc --noEmit
...
```
apps/mirri-code (CLI)            apps/mirri-web (browser)
        │                              │
        └──────────┬───────────────────┘
        ...
        │  Fastify REST        │
        │  ws gateway          │
        │  DI container        │  ← @mirri-ai/agent-core
        │  agent-core sessions │  ← @mirri-ai/agent-core
        └──────────────────────┘
```
...
- **DI** (`src/services/serviceCollection.ts`): seeds the container from
  `@mirri-ai/agent-core` (`getSingletonServiceDescriptors()`) and layers in
...
- `@mirri-ai/agent-core` — the agent engine the server hosts, including the
