...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-oauth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-oauth-home-'));
});
...
function makeStub(scripted: {
  ...
  return {
    ...
    logout: async (provider) => {
      ...
      return scripted.logout ?? { logged_out: true, provider: 'managed:mirri-code' };
    },
  };
}
