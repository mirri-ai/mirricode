...
import { IPromptService, PromptService } from '@mirri-ai/agent-core';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-ws-abort-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-ws-abort-home-'));
});
...
async function openSubscriber(r: RunningServer, sid: string): Promise<Subscriber> {
  ...
  const ws = await new Promise<WebSocket>((resolve, reject) => {
    const sock = new WebSocket(wsUrl, ['mirri-code.bearer.test-token']);
