 ...
 * `host: '0.0.0.0'` + `MIRRICODE_PASSWORD` + `insecureNoTls: true` server:
...
function tmpPaths(): { lockPath: string; homeDir: string } {
  const dir = mkdtempSync(join(tmpdir(), 'mirri-host-exposure-'));
  const home = mkdtempSync(join(tmpdir(), 'mirri-host-exposure-home-'));
...
describe('public-bind §3.5 end-to-end (M6.7)', () => {
   ...
   * the password `verifyPassword` path is exercised. `MIRRICODE_PASSWORD` is set
