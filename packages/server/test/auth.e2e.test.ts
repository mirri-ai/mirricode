...
import { authSummarySchema, type AuthSummary } from '@mirri-ai/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-auth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-auth-home-'));
});
...
describe('GET /api/v1/auth — readiness probe (P2.1 D2)', () => {
  ...
  it('surfaces managed_provider.unauthenticated when config has managed:mirri-code but no cached token', async () => {
    seedConfig(
      [
