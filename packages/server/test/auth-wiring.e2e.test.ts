...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-auth-wiring-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-auth-wiring-home-'));
});
...
describe('production auth wiring (M5.1)', () => {
  ...
  it('gates WS: server_hello with the token, rejected without', async () => {
    ...
    const conn = await openConn(wsUrl(r.address), [`mirri-code.bearer.${token}`]);
