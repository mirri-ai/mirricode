...
import { SyncDescriptor, ITerminalService, TerminalService } from '@mirri-ai/agent-core';
import type { Terminal } from '@mirri-ai/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-ws-terminal-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-ws-terminal-home-'));
