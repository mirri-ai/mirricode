...
import { IEventService } from '@mirri-ai/agent-core';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-ws-broadcast-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-ws-broadcast-home-'));
});
...
function openConn(url: string): Promise<Conn> {
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(url, ['mirri-code.bearer.test-token']);
    const queue: WsFrame[] = [];
