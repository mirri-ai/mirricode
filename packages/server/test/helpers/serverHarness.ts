...
import type { ServiceIdentifier } from '@mirri-ai/agent-core';
 ...
 */
const WS_BEARER_PROTOCOL_PREFIX = 'mirri-code.bearer.';
...
export interface ServerHarness {
   ...
   * `mirri-code.bearer.<token>` and an `Authorization: Bearer <token>` header.
...
export async function boot(opts: BootOptions = {}): Promise<ServerHarness> {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-harness-'));
  const homeDir = mkdtempSync(join(tmpdir(), 'mirri-server-harness-home-'));
