...
import { metaResponseSchema, ulidRegex } from '@mirri-ai/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-meta-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-meta-home-'));
});
...
describe('GET /api/v1/meta — envelope + metaResponseSchema', () => {
  ...
  it('two independent daemons get distinct daemon_ids', async () => {
    ...
    const homeA = mkdtempSync(join(tmpdir(), 'mirri-server-meta-home-a-'));
    const homeB = mkdtempSync(join(tmpdir(), 'mirri-server-meta-home-b-'));
...
describe('GET /api/v1/meta — server_version precedence', () => {
  it('reports the host mirri-code identity version when provided', async () => {
    server = await startServer({
      serviceOverrides: [fixedTokenAuth()],
      host: '127.0.0.1',
      port: 0,
      ...
      coreProcessOptions: {
        homeDir: bridgeHome,
        identity: { userAgentProduct: 'mirri-code-cli', version: '9.9.9-test' },
      },
    });
