...
import { IModelCatalogService, type IModelCatalogService as ModelCatalogServiceShape } from '@mirri-ai/agent-core';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-model-catalog-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-model-catalog-home-'));
...
describe('model/provider catalog routes', () => {
  ...
  it('refreshes OAuth provider models through the catalog route', async () => {
    const stub: ModelCatalogServiceShape = {
      ...
      refreshOAuthProviderModels: async () => ({
        changed: [
          {
            provider_id: 'managed:mirri-code',
            provider_name: 'Mirri Code',
            added: 1,
            removed: 0,
          },
        ],
  ...
  it('refreshes a single provider through POST /providers/{id}:refresh', async () => {
    ...
    const res = await appOf(r).inject({
      method: 'POST',
      url: '/api/v1/providers/managed%3Amirri-code:refresh',
      payload: {},
    });
