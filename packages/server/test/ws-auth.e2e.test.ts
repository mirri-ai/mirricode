 ...
 *
 * M3.1 adds the `mirri-code.bearer.<token>` subprotocol parser; M3.2 wires it
...
describe('extractWsBearerToken', () => {
  ...
  it('extracts the token from a single bearer subprotocol', () => {
    expect(extractWsBearerToken('mirri-code.bearer.TOKEN')).toBe('TOKEN');
  });

  it('finds the bearer subprotocol among a comma-separated list', () => {
    expect(extractWsBearerToken('other, mirri-code.bearer.TOKEN2')).toBe('TOKEN2');
  });

  it('returns undefined for an empty token', () => {
    expect(extractWsBearerToken('mirri-code.bearer.')).toBeUndefined();
  });
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-ws-auth-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-ws-auth-home-'));
});
...
describe('ws upgrade auth', () => {
  it('accepts a valid bearer subprotocol and echoes it', async () => {
    ...
    const conn = await openConn(wsUrl(r.address), {
      protocols: ['mirri-code.bearer.test-token'],
    });

    await receiveType(conn, 'server_hello', 1000);
    expect(conn.ws.protocol).toBe('mirri-code.bearer.test-token');
  ...
  it('rejects a wrong bearer token without server_hello', async () => {
    ...
    await expectRejected(wsUrl(r.address), {
      protocols: ['mirri-code.bearer.wrong'],
    });
  });
  ...
  it('rejects upgrades to a non-/api/v1/ws path', async () => {
    ...
    await expectRejected(badUrl, { protocols: ['mirri-code.bearer.test-token'] });
  });
});
