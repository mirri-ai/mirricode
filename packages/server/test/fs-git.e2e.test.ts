...
import { parsePorcelain, parseNumstat } from '@mirri-ai/agent-core';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-fs-git-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-fs-git-home-'));
