 ...
 * `~/.mirricode/server/lock` is never touched. We mint pid values that don't
...
describe('acquireLock — defaults', () => {
  it('DEFAULT_LOCK_PATH points under the mirri-code home', () => {
    expect(DEFAULT_LOCK_PATH).toMatch(/[/\\]\.mirri-code[/\\]server[/\\]lock$/);
  });
});
