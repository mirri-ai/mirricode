...
import { encodeWorkDirKey } from '@mirri-ai/agent-core/session/store';
import type { Session, Workspace } from '@mirri-ai/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-swsessions-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-swsessions-home-'));
});
