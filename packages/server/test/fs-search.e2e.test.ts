...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-fs-search-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-fs-search-home-'));
  ...
  it('grep timeout fires FsGrepTimeoutError → 41305', async () => {
    ...
    class StubTimeout extends FsSearchService {
      protected override async grepWithNode(
        _cwd: string,
        _req: import('@mirri-ai/protocol').FsGrepRequest,
        _signal: AbortSignal,
        startedAt: number,
      ): Promise<import('@mirri-ai/protocol').FsGrepResponse> {

        throw new (
          await import('@mirri-ai/agent-core')
