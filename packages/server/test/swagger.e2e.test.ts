...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-swagger-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'mirri-server-swagger-home-'));
});
...
describe('Swagger / OpenAPI', () => {
  it('/openapi.json returns a valid OpenAPI document', async () => {
    ...
    expect((doc['info'] as Record<string, unknown>)['title']).toBe('Mirri Code Server API');
