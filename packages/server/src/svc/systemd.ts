...
export function createSystemdManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    const enable = await deps.execSystemctl(['enable', '--now', MIRRI_SERVER_SYSTEMD_UNIT]);
    if (enable.code !== 0) {
      throw new Error(
  ...
  async function uninstall(): Promise<LifecycleResult> {
    ...
    await deps.execSystemctl(['disable', '--now', MIRRI_SERVER_SYSTEMD_UNIT]).catch(() => undefined);
    try {
  ...
  async function stop(): Promise<LifecycleResult> {
    const result = await deps.execSystemctl(['stop', MIRRI_SERVER_SYSTEMD_UNIT]);
    if (result.code !== 0) {
      return {
        ok: false,
  ...
  async function restart(): Promise<LifecycleResult> {
    const result = await deps.execSystemctl(['restart', MIRRI_SERVER_SYSTEMD_UNIT]);
    if (result.code !== 0) {
      return {
        ok: false,
  ...
  async function status(): Promise<ServiceStatus> {
    ...
    const base: ServiceStatus = {
      ...
      installed,
      running: false,
      unitName: MIRRI_SERVER_SYSTEMD_UNIT,
