...
import { MIRRI_SERVER_TASK_NAME, supervisorLogPath as defaultSupervisorLogPath } from './paths';
...
export function createSchtasksManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    if (alreadyInstalled && args.force !== true) {
      return {
        ...
        message: `Scheduled task "${MIRRI_SERVER_TASK_NAME}" already exists. Rerun with --force to replace it.`,
    ...
    try {
      const createArgs = ['/Create', '/TN', MIRRI_SERVER_TASK_NAME, '/XML', xmlPath];
      if (alreadyInstalled) {
        createArgs.push('/F');
      }
    ...
    const run = await deps.execSchtasks(['/Run', '/TN', MIRRI_SERVER_TASK_NAME]);
    if (run.code !== 0) {
  ...
  async function uninstall(): Promise<LifecycleResult> {
    ...
    await deps.execSchtasks(['/End', '/TN', MIRRI_SERVER_TASK_NAME]).catch(() => undefined);
    const del = await deps.execSchtasks(['/Delete', '/TN', MIRRI_SERVER_TASK_NAME, '/F']);
    if (del.code !== 0) {
      return {
        ok: false,
    ...
    deleteInstallPlan();
    return { ok: true, message: `Scheduled task removed (${MIRRI_SERVER_TASK_NAME}).` };
  }
  ...
  async function stop(): Promise<LifecycleResult> {
    const result = await deps.execSchtasks(['/End', '/TN', MIRRI_SERVER_TASK_NAME]);
    if (result.code !== 0) {
      return {
        ok: false,
  ...
  async function restart(): Promise<LifecycleResult> {
    const end = await deps.execSchtasks(['/End', '/TN', MIRRI_SERVER_TASK_NAME]);
    if (end.code !== 0) {
      return {
        ok: false,
...
async function defaultTaskExists(): Promise<boolean> {
  const res = await execFileUtf8('schtasks', ['/Query', '/TN', MIRRI_SERVER_TASK_NAME], {
    windowsHide: true,
  });
  return res.code === 0;
}
