...
export interface ServerStartOptions {
   ...
   * and `MIRRICODE_ALLOWED_HOSTS`. A leading dot matches a domain suffix.
   */
...
export async function startServer(opts: ServerStartOptions): Promise<RunningServer> {
  ...
  // its mtime changes, so `mirri server rotate-token` takes effect without a
  ...
  // a non-loopback bind — `MIRRICODE_PASSWORD` remains an optional additional
  ...
  // `mirri server status` / `kill` / `ps` can find this daemon.
 ...
 * lock *before* listening, so by the time we reach `listen` a live mirri server
 ...
 * ("if the port is taken by something other than mirri server itself, +1").
 *
