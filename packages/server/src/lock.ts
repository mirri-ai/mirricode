 ...
 *
 * The lock is a small JSON file at `<MIRRICODE_HOME>/server/lock` (defaults
 * to `~/.mirricode-code/server/lock`; overridable via `MIRRICODE_HOME` env or
...
export interface AcquireLockOptions {
  /** Override default `<MIRRICODE_HOME>/server/lock` — used in tests. */
  lockPath?: string;
