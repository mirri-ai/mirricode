      ...
      try {
        ...
        const result = await ix.invokeFunction(async (a) => {
          ...
          const resolved = await resolvePromptMediaFiles(body, fileStore, cacheDir, {
            ...
            // Resolved lazily — only when a file attachment or an unsupported
            // image needs to be materialized to a path the model can Read.
            resolveAttachmentsDir: async () => {
              const summaries = await core.rpc.listSessions({ sessionId: session_id });
              const sessionDir = summaries[0]?.sessionDir;
              return sessionDir === undefined ? undefined : join(sessionDir, 'attachments');
            },
          });
  ...
  for (const part of body.content) {
    ...
    if (part.type === 'image' && part.source.kind === 'base64') {
      ...
      if (!isModelAcceptedImageMime(effectiveMime)) {
        // The bytes are still the user's content, though: persist them as a
    ...
    if (part.type === 'image') {
      ...
      if (!isModelAcceptedImageMime(mediaType)) {
        // Like the inline path, keep the bytes as a path-referenced attachment
        // instead of dropping them (best effort — the plain notice stands in
...
/** Copy an uploaded file into `dir` as `<fileId>-<sanitized name>`. */
