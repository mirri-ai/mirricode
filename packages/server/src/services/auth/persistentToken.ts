 ...
 *
 * The token lives at `<MIRRICODE_HOME>/server.token` (mode 0600) and is reused
 ...
 * `mirri server rotate-token` (which calls {@link rotateServerToken}).
 *
...
/** On-disk filename for the persistent token, relative to MIRRICODE_HOME. */
