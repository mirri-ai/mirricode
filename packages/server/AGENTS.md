...
Package-local rules for `packages/server` (`@mirri-ai/server`).

## What it is

The Mirri Code server. It hosts `agent-core` sessions and exposes them over REST + WebSocket under a single `/api/v1` prefix. It is consumed by `apps/mirri-code` (the CLI/TUI) — **do not add a reverse dependency** on the CLI app.
...
- Doc/meta endpoints in `start.ts`: `/openapi.json` (`@fastify/swagger`, lazily imported), `/asyncapi.json` (`createAsyncApiDocument` from `@mirri-ai/protocol`), `/healthz`. `webAssetsDir` enables `registerWebAssetRoutes`.
...
- `pnpm --filter @mirri-ai/server build` — `tsdown`.
- `pnpm --filter @mirri-ai/server typecheck` — `tsc -p tsconfig.json --noEmit`.
