...
async function main(): Promise<void> {
  const explicitHomeDir = process.env['MIRRI_OAUTH_SMOKE_HOME'];
  ...
  const forceLogin = process.env['MIRRI_OAUTH_SMOKE_FORCE_LOGIN'] === '1';
...
function smokeIdentityFromEnv(): MirriHostIdentity {
  ...
  if (version === undefined || version.trim().length === 0) {
    throw new Error('MIRRICODE_SMOKE_VERSION is required for Kimi OAuth smoke.');
  }
  return {
...
function shouldKeepToken(hasExplicitHomeDir: boolean): boolean {
  const value = process.env['MIRRI_OAUTH_SMOKE_KEEP_TOKEN'];
