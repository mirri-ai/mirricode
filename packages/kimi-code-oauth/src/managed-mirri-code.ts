...
export const MIRRICODE_PROVIDER_NAME = 'managed:mirri-code';
export const MIRRICODE_OAUTH_KEY = 'oauth/mirri-code';
const MIRRICODE_SCOPED_OAUTH_KEY_PREFIX = 'oauth/mirri-code-env-';

export type ManagedMirriCodeProtocol = 'kimi' | 'anthropic';

export function parseModelProtocol(value: unknown): ManagedMirriCodeProtocol | undefined {
...
export interface ManagedMirriCodeModelInfo {
  ...
  readonly protocol?: ManagedMirriCodeProtocol | undefined;
}
...
export interface ManagedMirriCodeApplyResult {
  readonly defaultModel: string;
  readonly defaultThinking: boolean;
}

export interface ManagedMirriCodeCleanupResult {
  readonly providerName: typeof MIRRICODE_PROVIDER_NAME;
...
export interface ManagedMirriProviderConfig {
  type: ManagedMirriCodeProtocol;
...
export interface ManagedMirriModelAliasOverrides {
...
export interface ManagedMirriModelAlias {
  provider: string;
  model: string;
  ...
  defaultEffort?: string | undefined;
  displayName?: string | undefined;
  protocol?: ManagedMirriCodeProtocol;
  betaApi?: boolean;
  adaptiveThinking?: boolean | undefined;
  overrides?: ManagedMirriModelAliasOverrides | undefined;
  readonly [key: string]: unknown;
}

export interface ManagedMirriServiceConfig {
  baseUrl?: string | undefined;
  apiKey?: string | undefined;
  oauth?: ManagedMirriOAuthRef | undefined;
}

export interface ManagedMirriServicesConfig {
  moonshotSearch?: ManagedMirriServiceConfig | undefined;
  moonshotFetch?: ManagedMirriServiceConfig | undefined;
  readonly [key: string]: unknown;
}

export interface ManagedMirriThinkingShape {
...
export interface ManagedMirriConfigShape {
  providers: Record<string, ManagedMirriProviderConfig | Record<string, unknown>>;
  models?: Record<string, ManagedMirriModelAlias | Record<string, unknown>> | undefined;
  defaultModel?: string | undefined;
  thinking?: ManagedMirriThinkingShape | undefined;
  services?: ManagedMirriServicesConfig | undefined;
  [key: string]: unknown;
}
...
function persistedOAuthHost(options: {
  ...
  const normalized = normalizeEndpoint(oauthHost ?? DEFAULT_MIRRICODE_OAUTH_HOST);
  if (
    options.key === MIRRICODE_OAUTH_KEY &&
    normalized === normalizeEndpoint(DEFAULT_MIRRICODE_OAUTH_HOST)
  ) {
    return undefined;
  }
  return normalized;
...
export function kimiCodeEnvBaseUrl(env: ManagedMirriEnv = process.env): string | undefined {
  return env.MIRRICODE_BASE_URL;
}

export function kimiCodeEnvOAuthHost(env: ManagedMirriEnv = process.env): string | undefined {
  return env.MIRRICODE_OAUTH_HOST ?? env.MIRRI_OAUTH_HOST;
}
...
export function resolveMirriCodeOAuthKey(options: {
  ...
  const oauthHost = normalizeEndpoint(options.oauthHost ?? DEFAULT_MIRRICODE_OAUTH_HOST);
  ...
  const digest = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 16);
  return `${MIRRICODE_SCOPED_OAUTH_KEY_PREFIX}${digest}`;
}
...
export function resolveMirriCodeRuntimeAuth(options: {
  ...
  readonly env?: ManagedMirriEnv | undefined;
 ...
 * `applyManagedMirriCodeConfig` (OAuth provisioning/refresh) and
...
export function toManagedModelAlias(
  providerId: string,
  model: ManagedMirriCodeModelInfo,
): ManagedMirriModelAlias {
...
export function applyManagedMirriCodeConfig(
  ...
  for (const model of options.models) {
    ...
    existingModels[key] = mergeRefreshedModelAlias(
      existing,
      toManagedModelAlias(MIRRICODE_PROVIDER_NAME, model),
 ...
 * a distributed API key instead of OAuth). Unlike `applyManagedMirriCodeConfig`
 ...
 * `${providerId}/` for ordinary providers, `mirri-code/` for a hand-written
 * `managed:mirri-code` entry so its aliases line up with the OAuth provisioned
...
export function applyManagedApiKeyProviderModels(
  ...
  const existingModels = config.models ?? {};
  // Same merge contract as `applyManagedMirriCodeConfig`: upstream-owned fields
...
export function applyManagedMirriCodeLogoutConfig(config: ManagedMirriConfigShape): void {
  delete config.providers[MIRRICODE_PROVIDER_NAME];

  let removedDefaultModel = false;
  const existingModels = config.models ?? {};
  for (const [key, model] of Object.entries(existingModels)) {
    if (!isRecord(model) || model['provider'] !== MIRRICODE_PROVIDER_NAME) continue;
    delete existingModels[key];
  ...
  if (config['defaultProvider'] === MIRRICODE_PROVIDER_NAME) {
...
function canPreserveDefaultModel(
  existingModels: Record<string, ManagedMirriModelAlias | Record<string, unknown>>,
  defaultModel: string,
