...
export interface OAuthManagerOptions {
   ...
   * `process.env.MIRRI_DISABLE_OAUTH_LOCK === '1'` always skip; the
...
export class OAuthManager {
  ...
  constructor(options: OAuthManagerOptions) {
    ...
    // The `MIRRICODE_HOME` fallback MUST stay test-only so production
    ...
    const envConfigDir =
      process.env['NODE_ENV'] === 'test' ? process.env['MIRRICODE_HOME'] : undefined;
  ...
  private resolveLockTarget(): string | undefined {
    ...
    if (process.env['MIRRI_DISABLE_OAUTH_LOCK'] === '1') return undefined;
