...
import {
  ...
  type ManagedMirriModelAlias,
  type ManagedMirriOAuthRef,
} from './managed-mirri-code';
import { isManagedMirriCodeBaseUrl } from './managed-usage';
import {
...
export interface RefreshProviderHost {
   ...
   * `mirri-code-cli/1.2.3`. When omitted the fetch falls back to the runtime
 ...
 * wins, with `env.MIRRI_API_KEY` as the documented config-file fallback.
 */
function resolveProviderApiKey(provider: ProviderView): string | undefined {
  ...
  if (isRecord(provider.env)) {
    const fromEnv = provider.env['MIRRI_API_KEY'];
...
function readModel(
  config: ManagedMirriConfigShape,
  alias: string,
): ManagedMirriModelAlias | undefined {
  const model = config.models?.[alias];
  if (model === undefined) return undefined;
  return model as ManagedMirriModelAlias;
}
...
function providerAliasKeys(config: ManagedMirriConfigShape, providerId: string): Set<string> {
  ...
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    if ((raw as ManagedMirriModelAlias).provider === providerId) keys.add(alias);
  }
  return keys;
}
...
function generatedProviderAliasKeys(
  ...
  const keys = new Set<string>();
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    const model = raw as ManagedMirriModelAlias;
...
interface ProviderModelSnapshot {
  ...
  readonly model: ManagedMirriModelAlias;
}
...
): Record<string, ManagedMirriModelAlias> {
  const preserved: Record<string, ManagedMirriModelAlias> = {};
  for (const [alias, raw] of Object.entries(config.models ?? {})) {
    const model = raw as ManagedMirriModelAlias;
...
function restoreProviderAliases(
  config: ManagedMirriConfigShape,
  aliases: Record<string, ManagedMirriModelAlias>,
): void {
 ...
 *     providers (including a hand-written `managed:mirri-code` without an oauth
...
export async function refreshProviderModels(
  ...
  // ---------------------------------------------------------------------------
  const managedProvider = readProvider(config, MIRRICODE_PROVIDER_NAME);
  const managedWanted = targetId === undefined || targetId === MIRRICODE_PROVIDER_NAME;
  if (
    managedWanted &&
    ...
    try {
      ...
      if (models.length > 0) {
        ...
        restoreProviderAliases(
          next,
          preserveUserProviderAliases(config, MIRRICODE_PROVIDER_NAME, refreshedAliasKeys),
        );
        ...
        if (providerModelsEqual(config, next, MIRRICODE_PROVIDER_NAME, refreshedAliasKeys)) {
          ...
          await host.removeProvider(MIRRICODE_PROVIDER_NAME);
  ...
  // handles a hand-written `managed:mirri-code` that carries an API key instead
  ...
  // Kimi Code endpoint, carrying an API key (inline or via `env.MIRRI_API_KEY`)
  ...
  for (const providerId of Object.keys(config.providers)) {
    ...
    if (!isManagedMirriCodeBaseUrl(provider.baseUrl)) continue;
    ...
    try {
      ...
      // A hand-written `managed:mirri-code` shares the OAuth branch's
      // `mirri-code/` alias prefix so the two shapes merge cleanly if the user
      // later logs in via OAuth; ordinary providers use their own id.
      const aliasPrefix =
        providerId === MIRRICODE_PROVIDER_NAME ? `${MIRRICODE_PLATFORM_ID}/` : `${providerId}/`;
      const next = structuredClone(config);
  ...
  >();
  for (const providerId of Object.keys(config.providers)) {
    if (providerId === MIRRICODE_PROVIDER_NAME) continue;
    if (isOpenPlatformId(providerId)) continue;
    const provider = readProvider(config, providerId);
    if (provider === undefined) continue;
