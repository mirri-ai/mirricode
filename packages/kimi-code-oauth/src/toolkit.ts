...
export class MirriOAuthToolkit<TConfig = unknown> {
  ...
  async logout(
    ...
    if (this.configAdapter?.remove !== undefined && name === MIRRICODE_PROVIDER_NAME) {
      ...
      deviceHeaders:
        identity === undefined
          ...
          : () =>
              ...
              // for the runtime surface, e.g. mirri web's `(web)`.
              createMirriDefaultHeaders({
...
export function resolveMirriTokenStorageName(input: {
  ...
  const key = input.oauthKey ?? MIRRICODE_OAUTH_KEY;
  if (key === 'mirri-code' || key === MIRRICODE_OAUTH_KEY) return 'mirri-code';
