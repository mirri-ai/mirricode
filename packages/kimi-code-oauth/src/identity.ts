...
export const MIRRICODE_PLATFORM = 'mirri_code_cli';
...
export interface MirriHostIdentity {
   ...
   * (e.g. `mirri_code_cli`, `mirri_code_desktop`). Every host must state its own
   * explicitly — `MIRRICODE_PLATFORM` is the CLI's value, not a default to
   * inherit silently.
   */
