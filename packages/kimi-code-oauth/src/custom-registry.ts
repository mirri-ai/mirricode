...
import type { ManagedMirriConfigShape, ManagedMirriModelAlias } from './managed-mirri-code';
 ...
 * by editing `~/.mirri-code/config.toml`.
 */
 ...
 *
 * `userAgent` identifies the host product (e.g. `mirri-code-cli/1.2.3`); when
...
export async function fetchCustomRegistry(
  ...
  for (const [key, raw] of Object.entries(payload)) {
    ...
    if (entry === undefined) {
      ...
      // eslint-disable-next-line no-console -- best-effort registry validation warning
      console.warn(
 ...
 * provider object via `ManagedMirriProviderConfig`'s index signature so the
...
export function applyCustomRegistryProvider(
  ...
  for (const [modelKey, model] of Object.entries(entry.models)) {
    ...
    const remoteAlias: ManagedMirriModelAlias = {
