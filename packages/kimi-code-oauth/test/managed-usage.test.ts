...
describe('kimiCodeBaseUrl', () => {
  it('strips trailing slashes from the MIRRICODE_BASE_URL override', () => {
    ...
    vi.stubEnv('MIRRICODE_BASE_URL', 'https://gw.example.com/');
    expect(kimiCodeBaseUrl()).toBe('https://gw.example.com');
    expect(kimiCodeUsageUrl()).toBe('https://gw.example.com/usages');
  });
});

describe('isManagedMirriCodeBaseUrl', () => {
  it('matches the default managed endpoint, with or without a trailing slash', () => {
    expect(isManagedMirriCodeBaseUrl('https://api.mirricode.com/coding/v1')).toBe(true);
    expect(isManagedMirriCodeBaseUrl('https://api.mirricode.com/coding/v1/')).toBe(true);
  });

  it('matches against the MIRRICODE_BASE_URL override', () => {
    vi.stubEnv('MIRRICODE_BASE_URL', 'https://gw.example.com/coding/v1/');
    expect(isManagedMirriCodeBaseUrl('https://gw.example.com/coding/v1')).toBe(true);
    expect(isManagedMirriCodeBaseUrl('https://api.mirricode.com/coding/v1')).toBe(false);
  });
  ...
  it('rejects other paths on the managed host and other hosts entirely', () => {
    expect(isManagedMirriCodeBaseUrl('https://api.mirricode.com/coding/v2')).toBe(false);
    expect(isManagedMirriCodeBaseUrl('https://api.mirricode.com/v1')).toBe(false);
    expect(isManagedMirriCodeBaseUrl('https://gateway.example.com/coding/v1')).toBe(false);
    expect(isManagedMirriCodeBaseUrl('https://api.moonshot.cn/v1')).toBe(false);
  });

  it('rejects undefined and unparseable values', () => {
    expect(isManagedMirriCodeBaseUrl(undefined)).toBe(false);
    expect(isManagedMirriCodeBaseUrl('')).toBe(false);
    expect(isManagedMirriCodeBaseUrl('not a url')).toBe(false);
  });
});

describe('isManagedMirriCode', () => {
  it('matches only the mirri-code managed provider', () => {
    expect(isManagedMirriCode('managed:mirri-code')).toBe(true);
    expect(isManagedMirriCode('managed:moonshot-ai')).toBe(false);
    expect(isManagedMirriCode('openai')).toBe(false);
    expect(isManagedMirriCode('')).toBe(false);
    expect(isManagedMirriCode(null)).toBe(false);
    expect(isManagedMirriCode()).toBe(false);
  });
});
