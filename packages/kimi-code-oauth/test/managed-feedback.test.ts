...
describe('kimiCodeFeedbackUrl', () => {
  it('appends /feedback to the default base URL', () => {
    expect(kimiCodeFeedbackUrl()).toBe('https://api.mirricode.com/coding/v1/feedback');
  });
...
describe('fetchSubmitFeedback', () => {
  ...
  it('preserves the mirri-code- version prefix in the request body', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({ feedback_id: 3 }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    ...
    expect(sent['version']).toBe('mirri-code-0.1.1');
  });
