...
describe('Kimi identity factories', () => {
  ...
  it('honors an explicit X-Msh-Platform value', () => {
    const headers = createMirriDeviceHeaders({
      homeDir: tempHome(),
      version: '1.2.3-test',
      platform: 'mirri_code_desktop',
    });

    expect(headers['X-Msh-Platform']).toBe('mirri_code_desktop');
  });

  it('rejects an empty, whitespace, or all-non-ASCII platform instead of emitting a bad header', () => {
    for (const platform of ['', '   ', '桌面']) {
      expect(
        () => createMirriDeviceHeaders({ homeDir: tempHome(), version: '1.2.3', platform }),
  ...
  it('sanitizes header-unsafe characters out of the platform value', () => {
    const headers = createMirriDeviceHeaders({
      homeDir: tempHome(),
      version: '1.2.3',
      platform: 'mirri_code_桌面\n',
    });
    expect(headers['X-Msh-Platform']).toBe('mirri_code_');
  });
  ...
  it('merges User-Agent and device headers into default headers', () => {
    ...
    expect(headers['X-Msh-Platform']).toBe('mirri_code_cli');
  ...
  it('threads the identity platform into default headers', () => {
    ...
    expect(headers['User-Agent']).toBe('mirri-code-desktop/0.0.13');
    expect(headers['X-Msh-Platform']).toBe('mirri_code_desktop');
  });
});
...
describe('ascii header value sanitization', () => {
  it('strips a trailing newline from a header value', () => {
    const ua = createMirriUserAgent({ productName: 'mirri-code-cli', version: '6.8.0-101\n' });
    expect(ua).toBe('mirri-code-cli/6.8.0-101');
  });

  it('drops non-ASCII codepoints while keeping the ASCII remainder', () => {
    const ua = createMirriUserAgent({ productName: 'mirri-code-cli', version: 'héllo' });
  ...
  it('falls back to Darwin kernel version when sw_vers is unavailable', async () => {
    ...
    try {
      ...
      const headers = createMirriDeviceHeaders({ homeDir: tempHome(), version: '1.0.0', platform: 'test' });
