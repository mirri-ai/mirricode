...
describe('MirriOAuthToolkit', () => {
  ...
  it('resolves bearer token providers using the configured oauth key', async () => {
    ...
    await expect(
      toolkit
        .tokenProvider(MIRRICODE_PROVIDER_NAME, { key: 'oauth/custom-mirri-code' })
        .getAccessToken(),
  ...
  it('refreshes configured bearer token refs against their OAuth host', async () => {
    ...
    await expect(
      toolkit
        .tokenProvider(MIRRICODE_PROVIDER_NAME, { key: oauthKey, oauthHost })
        .getAccessToken(),
  ...
  it('does not reuse a cached OAuth manager across different hosts for the same token key', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('custom-mirri-code', {
    ...
    await expect(
      toolkit
        .tokenProvider(MIRRICODE_PROVIDER_NAME, {
  ...
  it('resolves cached access tokens using the configured oauth key', async () => {
    ...
    await expect(
      toolkit.getCachedAccessToken(MIRRICODE_PROVIDER_NAME, { key: 'oauth/custom-mirri-code' }),
    ...
    const toolkit = new MirriOAuthToolkit({
      ...
      configAdapter: {
        ...
        apply: (target, input) => {
          ...
          return {
            defaultModel: `mirri-code/${input.models[0]?.id ?? 'unknown'}`,
  ...
  it('uses a scoped credential slot for non-default OAuth login environments', async () => {
    ...
    expect(storage.tokens.get('mirri-code')?.accessToken).toBe('prod-access');
    expect(storage.tokens.get(devStorageName)?.accessToken).toBe('dev-access');
    expect(config.providers[MIRRICODE_PROVIDER_NAME]?.oauth).toEqual({
      storage: 'file',
      key: devOauthKey,
      oauthHost: devOauthHost,
    });
  ...
  it('removes managed config on logout when an adapter supports cleanup', async () => {
    ...
    const toolkit = new MirriOAuthToolkit({
      ...
      configAdapter: {
        read: () => config,
        write,
        apply: () => ({ defaultModel: 'mirri-code/kimi-for-coding', defaultThinking: true }),
        remove,
      },
    });
    ...
    expect(write).toHaveBeenCalledWith(config);
    await expect(storage.load('mirri-code')).resolves.toBeUndefined();
  });
});
