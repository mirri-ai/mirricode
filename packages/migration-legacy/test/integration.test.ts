...
describe('runMigration (end-to-end on multi-workdir fixture)', () => {
  ...
  it('does not copy OAuth credentials into the target', async () => {
    ...
    try {
      ...
      await expect(
        readFile(join(tgt, 'credentials', 'mirri-code.json'), 'utf-8'),
      ).rejects.toThrow();
      // The report tells the user to sign in again in mirri-code.
      expect(report.notices.oauthLoginsRequiringRelogin).toContain('mirri-code.json');
    } finally {
      await rm(src, { recursive: true, force: true });
    }
  });
