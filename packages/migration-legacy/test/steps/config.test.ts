...
describe('migrateConfigStep', () => {
  ...
  it('additively merges into a user-modified target config', async () => {
    ...
    expect(cfg).toContain('mirri-code/kimi-for-coding'); // migrated model added
  });
...
`,
    ...
    const r = await migrateConfigStep({ sourceHome: src, targetHome: tgt });
    expect(r.configConflicts).toContain('providers.managed:mirri-code');
    const cfg = await readFile(join(tgt, 'config.toml'), 'utf-8');
    expect(cfg).toContain('https://target.example/v1');
    expect(cfg).not.toContain('https://source.example/v1');
  });

  it('drops top-level keys mirri-code does not support', async () => {
    await writeFile(
      join(src, 'config.toml'),
  ...
  it('drops a kept-provider model missing required schema fields', async () => {
    // `bad-model` references the kept `managed:mirri-code` provider but omits
    // `max_context_size`, which mirri-code's ModelAliasSchema requires. Written
    // verbatim it would make getConfig() reject the whole config post-migration.
    const cfg = `[providers."managed:mirri-code"]
type = "kimi"
base_url = "https://api.kimi.com/coding/v1"
...
`,
  ...
  it('drops a legacy theme outside the mirri-code TUI enum', async () => {
  ...
  it('drops a single hook mirri-code\'s schema rejects, keeps the rest', async () => {
    await writeFile(
      join(src, 'config.toml'),
      '[[hooks]]\n' +
        'event = "PreToolUse"\n' +
