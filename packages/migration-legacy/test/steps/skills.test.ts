...
describe('migrateSkillsStep', () => {
  it('copies SKILL.md bundles and flat .md skills under ~/.mirricode/skills/', async () => {
  ...
  it('returns zero counters when source ~/.mirricode/skills/ is missing', async () => {
    const r = await migrateSkillsStep({ sourceHome: src, targetHome: tgt });
  ...
  it('copies non-skill files at top level too (no filtering)', async () => {
    ...
    // ~/.mirricode/skills/ is preserved verbatim. The new scanner ignores anything
