 ...
 *     all from `@mirri-ai/agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `getReadyAgent()`), from
 *     `@mirri-ai/agent-core`; `localKaos` from `@mirri-ai/kaos`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@mirri-ai/agent-core/session/store/index';
import { Session, type SDKSessionRPC } from '@mirri-ai/agent-core';
