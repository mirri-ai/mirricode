...
describe('paths', () => {
  it('sourceCredentialsDir joins ~/.mirricode/credentials', () => {
    expect(paths.sourceCredentialsDir('/x/.mirricode')).toBe(join('/x/.mirricode', 'credentials'));
  });
  ...
  it('migratedMarker is under source', () => {
    expect(paths.migratedMarker('/x/.mirricode')).toBe(join('/x/.mirricode', '.migrated-to-mirri-code'));
  });

  it('skipMarker is under target', () => {
    expect(paths.skipMarker('/y/.mirricode-code')).toBe(join('/y/.mirricode-code', '.skip-migration-from-kimi-cli'));
  });
