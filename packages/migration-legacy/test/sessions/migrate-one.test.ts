...
describe('migrateOneSession (tiny-hello-world fixture)', () => {
  ...
  it('reports conflict when an unrelated mirri-code session occupies the dir', async () => {
    ...
    // Overwrite state.json with a non-migrated (real) mirri-code session.
  ...
  it('re-migrates a target dir left half-written by an interrupted run', async () => {
    ...
    // real mirri-code session, so it must be re-migrated, not reported as a
  ...
  it('re-migrates a target dir whose state.json is corrupt', async () => {
    ...
    // is `ses_<uuid>`), not a real mirri-code session, so it must be
