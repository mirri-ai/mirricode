...
import { FLAG_DEFINITIONS } from '@mirri-ai/agent-core/flags/registry';
...
// mirri-code's tui.toml `theme` enum (mirrors apps/mirri-code TuiThemeSchema).
...
export interface ConfigStepResult {
  ...
  /** Top-level keys dropped because mirri-code's config schema lacks them. */
  ...
  /** Count of kimi-cli hook entries dropped because mirri-code's schema rejects them. */
...
/** True when the kimi-cli provider entry validates against mirri-code's schema. */
...
/** True when the kimi-cli model entry validates against mirri-code's schema. */
...
export async function migrateConfigStep(input: ConfigStepInput): Promise<ConfigStepResult> {
  ...
  // 1) Providers — keep only those mirri-code's schema accepts.
  ...
  // 2) Models — keep only those mirri-code's schema accepts, and not those
  ...
  // 2b) Hooks — keep only entries mirri-code's HookDefSchema accepts. kimi-cli
  //     and mirri-code share an identical hook shape, so a valid legacy hook
  //     passes straight through; the per-entry filter only guards against
  //     future schema drift (an event type / field mirri-code does not know).
  ...
  // 4) Build the migrated top-level — only keys mirri-code's schema supports.
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (k === 'default_yolo') {
      // kimi-cli's `default_yolo` maps to mirri-code's `default_permission_mode`.
  ...
  // 4b) Drop any supported top-level key whose VALUE mirri-code's config
