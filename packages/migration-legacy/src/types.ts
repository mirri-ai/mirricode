/**
 * Plan describing the contents detected under the source `~/.mirricode/` directory.
...
/**
 * One workdir bucket (`~/.mirricode/sessions/<md5>/`) with reverse-looked-up
 * path from `~/.mirricode/mirri.json`. Buckets with kaos != 'local' or no
 * workdir found are excluded from this list (they appear in counters).
 */
export interface WorkDirEntry {
  readonly oldHashDir: string; // absolute path to ~/.mirricode/sessions/<md5>/
 ...
 * `~/.mirricode-code/migration-report.json` and surfaced in the terminal summary.
 */
...
export interface MigrationSummary {
  ...
  readonly mcp: {
    ...
    /** Source servers dropped because mirri-code's MCP schema rejects them. */
...
export interface MigrationNotices {
  ...
  /**
   * Basenames of kimi-cli OAuth logins (`~/.mirricode/credentials/<name>.json`)
   ...
   * mirri-code instead. Empty when the legacy install had no OAuth login.
   */
