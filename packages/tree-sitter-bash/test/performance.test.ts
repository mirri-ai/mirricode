...
describe('performance smoke', () => {
  it('parses a 100KB realistic script fast (default budget suffices)', () => {
    ...
    if (!result.ok) {
      // An abort is acceptable only when it is prompt (< 200 ms).
      expect(result).toEqual({ ok: false, reason: 'aborted' });
      expect(elapsed).toBeLessThan(200);
      return;
    }
    // Completed within the default budget; re-parse unbounded and
  ...
  it('the abort path is prompt (< 500 ms) on a node-budget bomb', () => {
    const source = 'echo a; '.repeat(50_000);
    const start = performance.now();
    const result = parse(source);
    const elapsed = performance.now() - start;
    expect(result).toEqual({ ok: false, reason: 'aborted' });
    // The 200 ms budget should trigger well under 500 ms. A loaded CI runner
    // with GC pauses and OS scheduling pressure can push wall-clock time
    // higher than the budget; 500 ms gives a 2.5x safety margin while still
    // catching a quadratic regression (which would take seconds, not ms).
