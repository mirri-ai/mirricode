...
describe('OPEN_PLATFORMS', () => {
  ...
  it('isOpenPlatformId works', () => {
    ...
    expect(isOpenPlatformId('mirri-code')).toBe(false);
  });
});
...
describe('filterModelsByPrefix', () => {
  it('filters by allowedPrefixes when present', () => {
    ...
    const filtered = filterModelsByPrefix(models as unknown as import('../src/managed-mirri-code').ManagedMirriCodeModelInfo[], platform);
...
describe('removeOpenPlatformConfig', () => {
  it('removes provider, its models, and defaultModel when matched', () => {
    const config: ManagedMirriConfigShape = {
      providers: {
        'moonshot-cn': { type: 'openai', baseUrl: 'https://api.moonshot.cn/v1', apiKey: 'sk-test' },
