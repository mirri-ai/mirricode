...
import { emptySessionUsage, type Event, type Session } from '@mirri-ai/protocol';
...
describe('SessionService.list', () => {
  ...
  it('busy filter applies post-hydration', async () => {
...
describe('SessionService.update', () => {
  ...
  it('forwards agent_config.model through IPromptService.applyAgentState (source="meta")', async () => {
    await svc.update(created.id, { agent_config: { model: 'mirri-code/k9' } });
    expect(promptStub.calls).toEqual([
      { sid: created.id, patch: { model: 'mirri-code/k9' }, source: 'meta', promptId: undefined },
    ]);
  });
  ...
  it('combines model + runtime controls into a single applyAgentState call', async () => {
    await svc.update(created.id, {
      agent_config: { model: 'mirri-code/k9', plan_mode: false },
    });
    expect(promptStub.calls).toHaveLength(1);
    expect(promptStub.calls[0]?.patch).toEqual({ model: 'mirri-code/k9', plan_mode: false });
...
describe('SessionService busy lifecycle', () => {
  ...
  it('turn.ended marks the session not busy, whatever the reason', async () => {
    for (const reason of ['completed', 'failed', 'filtered']) {
  ...
  it('projects a filtered turn as failed across session wire surfaces', async () => {
    const session = await svc.create({ metadata: { cwd: '/tmp/filtered' } });
    eventBus.eventService.publish({ type: 'turn.started', sessionId: session.id } as unknown as Event);
    eventBus.eventService.publish({
      type: 'turn.ended',
      sessionId: session.id,
  ...
  it('should not synthesize session work from sub-agent turn boundaries', async () => {
    const session = await svc.create({ metadata: { cwd: '/tmp/subagent' } });
    const workChangedEvents = () =>
      eventBus.events.filter((e) => (e as { type?: string }).type === 'event.session.work_changed');

    // Main agent turn starts → busy.
    eventBus.eventService.publish({
      type: 'turn.started',
      sessionId: session.id,
      agentId: 'main',
    ...
    // A sub-agent runs and completes while the main turn is still in flight.
    const before = workChangedEvents().length;
    eventBus.eventService.publish({
      type: 'turn.started',
      sessionId: session.id,
      agentId: 'agent-0',
    ...
    // Work is still busy — the sub-agent's turn.ended produced no work transition.
    expect((await svc.get(session.id)).busy).toBe(true);
    expect(workChangedEvents().length).toBe(before);

    // Main agent turn ends → not busy. Exactly two transitions: busy, not-busy.
    eventBus.eventService.publish({
      type: 'turn.ended',
      sessionId: session.id,
      agentId: 'main',
    ...
    expect((await svc.get(session.id)).busy).toBe(false);
    expect(workChangedEvents().map((e) => (e as { busy: boolean }).busy)).toEqual([
      true,
      false,
    ]);
  });
