...
import type { Environment, KaosProcess } from '@mirri-ai/kaos';
...
describe('BashTool noninteractive env semantics', () => {
  ...
  it('lets kaos-level env override BashTool env and observes in-place updates', async () => {
    ...
    expect(firstEnv['MIRRICODE_ENV']).toBe('initial');

    sessionEnv.MIRRICODE_ENV = 'updated';
    await executeTool(tool, {
      turnId: '0',
    ...
    expect(secondEnv['MIRRICODE_ENV']).toBe('updated');
  });
});
