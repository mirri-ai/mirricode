...
describe('LocalFetchURLProvider SSRF guard', () => {
  ...
  // Lock down every private / reserved range the blocklist is supposed to
  // refuse. Each entry is an IP literal so DNS is never touched — this
  // verifies the SSRF policy itself, not the resolver path.
  it.each([
    ...
    ['172.31.255.254', 'rfc1918-172'],
    ['192.168.0.1', 'rfc1918-192'],
    ['192.168.1.254', 'rfc1918-192'],
    ['[::]', 'ipv6 unspecified'],
    ['[::1]', 'ipv6 loopback'],
    ['[fc00::1]', 'ipv6 ula'],
    ['[fd00::1]', 'ipv6 ula'],
    ['[fe80::1]', 'ipv6 link-local'],
  ])('rejects private IP literal %s (%s) without fetching', async (host) => {
    const fetchImpl = vi.fn<typeof fetch>();
    const provider = new LocalFetchURLProvider({ fetchImpl });

    await expect(provider.fetch(`http://${host}/`)).rejects.toThrow(
      'Refusing to fetch private address',
    );
    expect(fetchImpl).not.toHaveBeenCalled();
    expect(lookupMock).not.toHaveBeenCalled();
  ...
  it.each([
    ['172.32.0.1', 'just above rfc1918-172 range'],
    ['172.15.0.1', 'just below rfc1918-172 range'],
    ['100.63.0.1', 'just below cgnat range'],
    ['100.128.0.1', 'just above cgnat range'],
    ['11.0.0.1', 'public 11.x'],
    ['128.0.0.1', 'public 128.x'],
  ])('allows public IP literal %s (%s)', async (host) => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValue(htmlResponse('ok', 'text/plain'));
    const provider = new LocalFetchURLProvider({ fetchImpl });

    const result = await provider.fetch(`http://${host}/`);
