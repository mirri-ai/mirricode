...
describe('CronCreateTool', () => {
  ...
  it('documents the session task cap and near-term one-shot guidance, without bench env vars', () => {
    ...
    expect(tool.description).not.toContain('MIRRICODE_CRON_NO_STALE');
    expect(tool.description).not.toContain('MIRRICODE_CRON_NO_JITTER');
  ...
  it('returns an error when MIRRICODE_DISABLE_CRON=1', async () => {
    vi.stubEnv('MIRRICODE_DISABLE_CRON', '1');
    const { manager, tool, stub } = makeHarness();
    const msg = assertError(
      await runTool(tool, {
        cron: '*/5 * * * *',
    ...
    expect(msg).toMatchInlineSnapshot(
      `"Cron scheduling is disabled (MIRRICODE_DISABLE_CRON=1)."`,
    );
