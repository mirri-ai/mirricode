...
import { MirriFetchURLProvider } from '../../src/tools/providers/moonshot-fetch-url';
...
describe('FetchURLTool', () => {
  ...
  it('documents both fetch modes (extracted main text vs verbatim passthrough)', () => {
    ...
    // SSRF/size are provider-internal (the Mirri primary path enforces neither);
...
describe('MirriFetchURLProvider', () => {
  it('does not force-refresh request auth after a 401 response', async () => {
    ...
    const provider = new MirriFetchURLProvider({
