...
describe('SkillTool metadata and schema', () => {
  ...
  it('documents the skill and args parameters and the already-loaded guard', () => {
    ...
    // A skill loaded earlier surfaces a <mirri-skill-loaded> block; the description
    // must steer the model to follow it rather than re-invoking the tool.
    expect(tool.description).toContain('mirri-skill-loaded');
...
describe('SkillTool execution', () => {
  ...
  it('records inline skill content as a loaded skill message', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<mirri-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="message text">\nbody of commit\n\nARGUMENTS: message text\n</mirri-skill-loaded>',
    );
  ...
  it('keeps plugin instructions adjacent to model-invoked skill content', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<mirri-skill-loaded name="brainstorming" trigger="model-tool" source="extra" dir="/skills/brainstorming" args="">\n' +
        '<mirri-plugin-instructions plugin="superpowers">\n' +
  ...
  it('expands skill body placeholders for model-invoked inline skills', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<mirri-skill-loaded name="commit" trigger="model-tool" source="user" dir="/skills/commit" args="-m &quot;fix login&quot;">\nFlag: -m\nCommit message: fix login\nRaw: -m "fix login"\n</mirri-skill-loaded>',
    );
  ...
  it('expands session id from the skill registry for model-invoked skills', async () => {
    const methods = skillToolMethods();
    const tool = skillTool(
      registry([skill('session-aware', {}, 'Session: ${MIRRICODE_SESSION_ID}')], {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<mirri-skill-loaded name="session-aware" trigger="model-tool" source="user" dir="/skills/session-aware" args="">\nSession: ses_model_skill\n</mirri-skill-loaded>',
    );
  });
  ...
  it('escapes skill name and args in the wrapper boundaries', async () => {
    ...
    expect(methods.recordUserMessage.mock.calls[0]?.[0][0]?.text).toBe(
      'Skill tool loaded instructions for this request. Follow them.\n\n' +
        '<mirri-skill-loaded name="a&amp;b" trigger="model-tool" source="user" dir="/skills/a&amp;b" args="&lt;raw &quot;value&quot;&gt;">\nbody of a&b\n\nARGUMENTS: &lt;raw "value"&gt;\n</mirri-skill-loaded>',
    );
    expect(methods.recordSkillActivation).toHaveBeenCalledTimes(1);
  });
