...
import { MirriWebSearchProvider } from '../../src/tools/providers/moonshot-web-search';
  ...
  it('classifies authentication failures', async () => {
    const provider: WebSearchProvider = {
      search: vi
        .fn()
        .mockRejectedValue(
          new Error('Mirri search request failed: HTTP 401 (auth/unauthorized).'),
        ),
    };
    const tool = new WebSearchTool(provider);
    const result = await executeTool(tool, {
...
describe('MirriWebSearchProvider', () => {
  it('maps site_name to siteName and does not forward page content', async () => {
    ...
    const provider = new MirriWebSearchProvider({
      apiKey: 'k',
