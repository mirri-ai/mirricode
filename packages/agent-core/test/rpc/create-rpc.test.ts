...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as MirriError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(MirriError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown MirriError across the wire preserving code and details', async () => {
    ...
    expect(received).toBeInstanceOf(MirriError);
    const error = received as MirriError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new MirriError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as MirriError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new MirriError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to MirriError(internal)', async () => {
    interface RemoteSide {
    ...
    await expect(remoteProxy.misbehave({})).rejects.toBeInstanceOf(MirriError);
  });
});
