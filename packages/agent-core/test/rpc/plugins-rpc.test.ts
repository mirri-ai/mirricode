...
describe('MirriCore plugin RPCs', () => {
  ...
  it('injects persisted managed Mirri Code environment into the datasource plugin MCP server', async () => {
    const previousBaseUrl = process.env['MIRRICODE_BASE_URL'];
    const previousCodeOAuthHost = process.env['MIRRICODE_OAUTH_HOST'];
    const previousOAuthHost = process.env['KIMI_OAUTH_HOST'];
    delete process.env['MIRRICODE_BASE_URL'];
...
`,
      ...
      restoreEnv('MIRRICODE_BASE_URL', previousBaseUrl);
      restoreEnv('MIRRICODE_OAUTH_HOST', previousCodeOAuthHost);
