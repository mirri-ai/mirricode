...
[providers."managed:mirri-code".env]
...
`;
    const config = parseConfigString(toml, configPath);
    expect(config.providers['managed:mirri-code']?.oauth).toEqual({
      storage: 'file',
      key: 'oauth/mirri-code-env-1234',
    ...
    const roundTripped = parseConfigString(text, configPath);
    expect(roundTripped.providers['managed:mirri-code']?.oauth?.oauthHost).toBe(
...
`;
  ...
  it('loads defaults for absent files and writes typed fields without dropping raw sections', async () => {
    ...
    const text = await readFile(configPath, 'utf-8');
    expect(text).toContain('default_model = "mirri-code/kimi-for-coding"');
...
describe('harness config schema and patch merge', () => {
  ...
  it('deep-merges validated patches while preserving existing typed and raw data', () => {
    ...
    expect(merged.models?.['mirri-code/kimi-for-coding']).toMatchObject({
      provider: 'managed:mirri-code',
      model: 'kimi-for-coding',
      maxContextSize: 262144,
      capabilities: ['tool_use'],
