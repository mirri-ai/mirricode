    ...
    skills: {
      registry: {
        ...
        renderSkillPrompt: (skill) => {
          ...
          return `<mirri-plugin-instructions plugin="${plugin.id}">\n${instructions}\n</mirri-plugin-instructions>\n\n${skill.content}`;
        },
      },
    },
    log: {
...
describe('PluginSessionStartInjector', () => {
  it('injects one <plugin_session_start> block per declared sessionStart on first call', async () => {
    ...
    expect(text).toContain('<mirri-plugin-instructions plugin="superpowers">');
  ...
  it('does not hard-code Superpowers guidance when the skill has no plugin instructions', async () => {
    ...
    expect(text).not.toContain('<mirri-plugin-instructions plugin="superpowers">');
