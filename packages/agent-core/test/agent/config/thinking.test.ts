...
describe('resolveThinkingEffort', () => {
  ...
  it('always accepts off regardless of kimiProtocol', () => {
    expect(supportsThinkingEffort('off', effortModel, true)).toBe(true);
    expect(supportsThinkingEffort('off', nonThinkingModel, true)).toBe(true);
    expect(supportsThinkingEffort('off', undefined, true)).toBe(true);
  });

  it('accepts on for any kimi model with efforts', () => {
    expect(supportsThinkingEffort('on', effortModel, true)).toBe(true);
    expect(supportsThinkingEffort('on', booleanModel, true)).toBe(true);
  });

  it('rejects concrete efforts on non-thinking kimi models', () => {
    expect(supportsThinkingEffort('high', nonThinkingModel, true)).toBe(false);
  });

  it('accepts any effort on a boolean-only kimi model (no supportEfforts)', () => {
    expect(supportsThinkingEffort('high', booleanModel, true)).toBe(true);
  });

  it('accepts any effort when kimiProtocol is false', () => {
    expect(supportsThinkingEffort('ultra', effortModel, false)).toBe(true);
    expect(supportsThinkingEffort('high', nonThinkingModel, false)).toBe(true);
    expect(supportsThinkingEffort('ultra', undefined, false)).toBe(true);
  });
});
