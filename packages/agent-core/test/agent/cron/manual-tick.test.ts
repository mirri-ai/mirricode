 ...
 * `MIRRICODE_CRON_MANUAL_TICK=1` env disables the auto-tick interval and,
...
describe('CronManager — P1.8 manual tick + SIGUSR1', () => {
  ...
  describe('MIRRICODE_CRON_MANUAL_TICK=1', () => {
  ...
  describe('without MIRRICODE_CRON_MANUAL_TICK', () => {
  ...
  describe('SIGUSR1', () => {
    // SIGUSR1 binding is opt-in via MIRRICODE_CRON_MANUAL_TICK=1 so that
    ...
    it('logs swallowed tick() throws to stderr when MIRRICODE_CRON_DEBUG=1', async () => {
      if (process.platform === 'win32') return;
      vi.stubEnv('MIRRICODE_CRON_DEBUG', '1');
    ...
    it('does not write to stderr on tick() throw when MIRRICODE_CRON_DEBUG is unset', async () => {
      if (process.platform === 'win32') return;
      // MIRRICODE_CRON_DEBUG intentionally NOT set in this test.
    ...
    it('stop() removes the SIGUSR1 listener (no leak)', async () => {
      ...
      // Constructor auto-starts, which binds SIGUSR1 under MIRRICODE_CRON_MANUAL_TICK=1.
      const manager = new CronManager(stub.agent, { pollIntervalMs: null });
    ...
    it('does not bind when MIRRICODE_CRON_MANUAL_TICK is unset', async () => {
      if (process.platform === 'win32') return;
