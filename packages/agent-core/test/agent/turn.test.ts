...
import type { Kaos, KaosProcess } from '@mirri-ai/kaos';
...
import { createCommandKaos, testAgent, type TestAgentContext, type TestAgentOptions } from './harness/agent';
...
describe('Agent turn flow', () => {
  it('gates unsupported image formats at the prompt and steer entry so the session cannot be poisoned', async () => {
    ...
    ctx.configure({
      provider: { type: 'openai', apiKey: 'test-key', model: 'kimi-code' },
      modelCapabilities: {
        image_in: true,
        video_in: false,
        audio_in: false,
  ...
  it('tracks turn_ended telemetry with protocol props', async () => {
    ...
    expect(started).toEqual({
      event: 'turn_started',
      properties: expect.objectContaining({ mode: 'agent', provider_type: 'openai', protocol: 'openai' }),
    });
  ...
  it('emits a friendly model.not_configured error when no model is configured', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] turn.ended               { "turnId": 0, "reason": "failed", "error": { "code": "model.not_configured", "message": "LLM not set, send \\"/login-with-kimi\\" to login", "name": "MirriError", "details": { "turnId": 0 }, "retryable": false } }
    `);
    expect(ctx.newEvents()).toMatchInlineSnapshot(
      `[emit] error   { "code": "model.not_configured", "message": "LLM not set, send \\"/login-with-kimi\\" to login", "name": "MirriError", "details": { "turnId": 0 }, "retryable": false }`,
    );
  });
  ...
  it('force-refreshes OAuth credentials on video upload 401 and surfaces the provider auth error when replay 401', async () => {
    ...
    expect(result.output).not.toContain('Send /login-with-kimi to login');
  });
