...
function requiredFlagEnv(id: string): string {
  ...
  return `MIRRICODE_EXPERIMENTAL_${id.toUpperCase()}`;
}
...
`,
  ...
  // Regression for https://github.com/mirri-ai/mirricode/issues/988: during
  // ACP `session/new` the tool kaos is the reverse-RPC bridge and the client
  // does not know the session yet, so reading `.mirri-code/local.toml` through
  ...
  it('reads workspace local.toml through persistenceKaos during createSession', async () => {
    ...
    await mkdir(join(workDir, '.mirri-code'), { recursive: true });
    await mkdir(sharedDir, { recursive: true });
    await writeFile(
      join(workDir, '.mirri-code', 'local.toml'),
...
oauth = { storage = "file", key = "oauth/custom-mirri-code" }
...
`,
    ...
    expect(resolveOAuthTokenProvider).toHaveBeenCalledWith('managed:mirri-code', {
      storage: 'file',
      key: 'oauth/custom-mirri-code',
    });
...
`,
  ...
  it('adds an additional dir through the session RPC', async () => {
    ...
    const localToml = await readFile(join(workDir, '.mirri-code', 'local.toml'), 'utf-8');
    const session = core.sessions.get(created.id);
    const mainAgent = session?.getReadyAgent('main');
    ...
    expect(result).toMatchObject({
      ...
      projectRoot: workDir,
      configPath: join(workDir, '.mirri-code', 'local.toml'),
      persisted: true,
    });
  ...
  it('adds a session-only additional dir without writing local.toml', async () => {
    ...
    await expect(readFile(join(workDir, '.mirri-code', 'local.toml'), 'utf-8')).rejects.toThrow();
  });
...
describe('MirriCore print-mode defaults', () => {
  let tmp: string;
  ...
  async function createCorePair(homeDir: string, uiMode?: string) {
    const [coreRpc, sdkRpc] = createRPC<CoreAPI, SDKAPI>();
    const core = new MirriCore(coreRpc, { homeDir, uiMode });
    const rpc = await sdkRpc({
      emitEvent: vi.fn(),
      requestApproval: vi.fn(async (): Promise<ApprovalResponse> => ({ decision: 'rejected' })),
      requestQuestion: vi.fn(async () => null),
  ...
  async function setupDirs(configToml: string): Promise<{ homeDir: string; workDir: string }> {
    tmp = await mkdtemp(join(tmpdir(), 'mirri-core-print-defaults-'));
    const homeDir = join(tmp, 'home');
    const workDir = join(tmp, 'work');
    await mkdir(homeDir, { recursive: true });
    await mkdir(workDir, { recursive: true });
  ...
  it('applies print-mode subagent/loop defaults to sessions when uiMode is print', async () => {
    ...
    expect(main?.mirriConfig?.subagent?.timeoutMs).toBe(0);
    expect(main?.mirriConfig?.background?.bashTaskTimeoutS).toBe(0);
    expect(main?.mirriConfig?.loopControl?.maxStepsPerTurn).toBe(0);

    // The raw user config is left untouched so config reads/writes still
    // round-trip the user's file values.
    const raw = await core.getMirriConfig();
...
`);
    ...
    const main = core.sessions.get(created.id)?.getReadyAgent('main');
    expect(main?.mirriConfig?.subagent?.timeoutMs).toBe(5000);
    expect(main?.mirriConfig?.loopControl?.maxStepsPerTurn).toBe(7);
  });
  ...
  it('does not apply print-mode defaults outside print mode', async () => {
    ...
    const main = core.sessions.get(created.id)?.getReadyAgent('main');
    expect(main?.mirriConfig?.subagent).toBeUndefined();
    expect(main?.mirriConfig?.loopControl).toBeUndefined();
  });
