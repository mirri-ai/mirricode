...
describe('HarnessAPI session skills', () => {
  ...
  it('lists bundled built-in skills by default', async () => {
    ...
    expect(importer).toMatchObject({
      ...
      description: 'Import Claude Code and Codex instructions, skills, and MCP settings into Mirri Code.',
      source: 'builtin',
      disableModelInvocation: true,
    });
  ...
  it('resolves user brand skills from MIRRICODE_HOME when no explicit home is set', async () => {
  ...
  it('activates an inline skill through core and records display origin metadata', async () => {
    ...
    const skillDir = toPosix(await realpath(join(workDir, '.mirri-code', 'skills', 'phase-one-review')));
  ...
  it('expands skill body placeholders on user slash activation', async () => {
    ...
    const records = await readMainWire(created.sessionDir);
    const prompt = records.find((record) => record['type'] === 'turn.prompt');
    const skillDir = toPosix(await realpath(join(workDir, '.mirri-code', 'skills', 'templated-review')));
    const expectedPrompt = [
  ...
  it('keeps the skill directory in the resumed conversation context so bundled resources stay locatable', async () => {
    ...
    const scriptDir = join(workDir, '.mirri-code', 'skills', 'bundled-tool', 'scripts');
    ...
    const skillDir = toPosix(await realpath(join(workDir, '.mirri-code', 'skills', 'bundled-tool')));
  ...
  async function writeSkill(name: string, lines: readonly string[]): Promise<void> {
    const dir = join(workDir, '.mirri-code', 'skills', name);
  ...
  async function writeLegacyUserSkill(
    ...
    await writeSkillFile(join(userHomeDir, '.mirri-code', 'skills', name), name, description);
  }
  ...
  async function writeFlatSkill(name: string, lines: readonly string[]): Promise<void> {
    const dir = join(workDir, '.mirri-code', 'skills');
    await mkdir(dir, { recursive: true });
