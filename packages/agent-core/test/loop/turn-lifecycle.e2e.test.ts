...
describe('runTurn — turn lifecycle', () => {
  ...
  it('throws MirriError(loop.max_steps_exceeded) when steps reach maxSteps', async () => {
    const echo = new EchoTool();
    ...
    expect((error as MirriError).code).toBe(ErrorCodes.LOOP_MAX_STEPS_EXCEEDED);
    expect((error as MirriError).details).toEqual({ maxSteps: 2 });
