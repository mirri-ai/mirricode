...
function expectConfigInvalid(fn: () => unknown): void {
  try {
    ...
    expect((error as MirriError).code).toBe(ErrorCodes.CONFIG_INVALID);
    return;
  }
...
describe('buildMcpHttpHeaders', () => {
  ...
  it('throws MirriError(config.invalid) when a configured bearer token env var is empty or missing', () => {
    expectConfigInvalid(() =>
      buildMcpHttpHeaders(
