...
describe('FlagResolver', () => {
  ...
  it('L2 per-feature on (lenient truthy values)', () => {
    for (const v of ['1', 'true', 'yes', 'on', 'TRUE', ' On ']) {
      expect(make({ MIRRICODE_EXPERIMENTAL_B: v })('b-off-default')).toBe(true);
    }
  });

  it('L2 per-feature off (lenient falsy values) overrides default=true', () => {
    for (const v of ['0', 'false', 'no', 'off']) {
      expect(make({ MIRRICODE_EXPERIMENTAL_A: v })('a-on-default')).toBe(false);
    }
  });

  it('L2 unparseable value falls back to default', () => {
    expect(make({ MIRRICODE_EXPERIMENTAL_B: 'maybe' })('b-off-default')).toBe(false);
    expect(make({ MIRRICODE_EXPERIMENTAL_A: 'maybe' })('a-on-default')).toBe(true);
  });
  ...
  it('L1 master switch beats an L2 per-feature off', () => {
    const enabled = make({ [MASTER_ENV]: '1', MIRRICODE_EXPERIMENTAL_A: '0' });
    expect(enabled('a-on-default')).toBe(true);
  });
  ...
  it('returns a full snapshot and enabled ids in registry order', () => {
    const resolver = new FlagResolver({ MIRRICODE_EXPERIMENTAL_B: '1' }, DEFS);
  ...
  it('reads the env name declared in the registry (the declared name works, others do not)', () => {
    expect(make({ MIRRICODE_EXPERIMENTAL_B: '1' })('b-off-default')).toBe(true);
    // The name mechanically derived from the id must not take effect (env is explicitly ..._B).
    expect(make({ MIRRICODE_EXPERIMENTAL_B_OFF_DEFAULT: '1' })('b-off-default')).toBe(false);
  });
  ...
  it('keeps env precedence above config overrides', () => {
    const resolver = new FlagResolver(
      {
        MIRRICODE_EXPERIMENTAL_A: '1',
        MIRRICODE_EXPERIMENTAL_B: '0',
      },
      DEFS,
      {
        'a-on-default': false,
...
describe('FLAG_DEFINITIONS invariants', () => {
  it('every env satisfies: prefix / unique / not the master switch', () => {
    ...
    for (const def of defs) {
      expect(def.env.startsWith('MIRRICODE_EXPERIMENTAL_')).toBe(true);
