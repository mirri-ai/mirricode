import type { ChatProvider, ModelCapability } from '@mirri-ai/kosong';
...
describe('resolveCompletionBudget', () => {
  it('reads MIRRICODE_MODEL_MAX_COMPLETION_TOKENS first', () => {
  ...
  it('falls back to legacy MIRRICODE_MODEL_MAX_TOKENS when the new var is unset', () => {
    const budget = resolveCompletionBudget({
      reservedContextSize: 1000,
      env: { MIRRICODE_MODEL_MAX_TOKENS: '2048' },
    });
  ...
  it('treats non-positive MIRRICODE_MODEL_MAX_COMPLETION_TOKENS as an opt-out', () => {
    expect(
      resolveCompletionBudget({
        reservedContextSize: 1000,
        env: { MIRRICODE_MODEL_MAX_COMPLETION_TOKENS: '0' },
      }),
    ).toBeUndefined();
    expect(
      resolveCompletionBudget({
        reservedContextSize: 1000,
        env: { MIRRICODE_MODEL_MAX_COMPLETION_TOKENS: '-1' },
      }),
    ).toBeUndefined();
  });

  it('treats non-positive legacy MIRRICODE_MODEL_MAX_TOKENS as an opt-out when the new var is unset', () => {
    expect(
      resolveCompletionBudget({
        reservedContextSize: 1000,
        env: { MIRRICODE_MODEL_MAX_TOKENS: '-1' },
      }),
    ).toBeUndefined();
  });
  ...
  it('falls back to defaults when the env var is non-numeric garbage', () => {
    const budget = resolveCompletionBudget({
      env: { MIRRICODE_MODEL_MAX_COMPLETION_TOKENS: 'not-a-number' },
    });
    expect(budget?.hardCap).toBeUndefined();
    expect(budget?.fallback).toBe(32000);
  });
