...
async function makePlugin(
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'mirri-plugin-test-'));
...
describe('parseManifest', () => {
  it('reads a minimal mirri.plugin.json at the plugin root', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', version: '1.0.0' }),
    });
  ...
  it('prefers root mirri.plugin.json when .mirri-plugin/plugin.json also exists', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'root-version', version: '1.0.0' }),
      '.mirri-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('mirri-plugin-root');
    expect(result.manifest?.name).toBe('root-version');
    expect(result.shadowedManifestPath).toBe(path.join(root, '.mirri-plugin/plugin.json'));
  });

  it('falls back to .mirri-plugin/plugin.json when mirri.plugin.json is absent', async () => {
    const root = await makePlugin(
      {
        '.mirri-plugin/plugin.json': JSON.stringify({
          name: 'demo',
          version: '1.0.0',
          keywords: ['workflow'],
          skills: './skills/',
    ...
    expect(result.manifestPath).toBe(path.join(root, '.mirri-plugin/plugin.json'));
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    ...
    expect(result.manifest?.skillInstructions).toBe('Use Mirri tools.');
  });

  it('does NOT fall back to .mirri-plugin/plugin.json when mirri.plugin.json is invalid JSON', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': '{ not json',
      '.mirri-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
  ...
  it('rejects names that violate the regex', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'Bad Name!' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a single skills path', async () => {
    const root = await makePlugin(
      { 'mirri.plugin.json': JSON.stringify({ name: 'demo', skills: './skills/' }) },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifest?.skills).toEqual([path.join(root, 'skills')]);
  ...
  it('resolves an array of skills paths', async () => {
    const root = await makePlugin(
      {
        'mirri.plugin.json': JSON.stringify({
          name: 'demo',
          skills: ['./a/', './b/'],
        }),
      },
  ...
  it('rejects a skills path that escapes plugin_root', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', skills: './../escape' }),
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
  ...
  it('rejects a skills path that escapes via a symlink', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', skills: './sym' }),
    });
    const outside = await mkdtemp(path.join(tmpdir(), 'mirri-plugin-outside-'));
  ...
  it('warns when skills resolves to a non-directory', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', skills: './notes.md' }),
      'notes.md': 'hi',
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
  ...
  it('falls back to root SKILL.md when skills field is absent', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo' }),
      'SKILL.md': '---\nname: root-skill\n---\nbody',
    });
    const result = await parseManifest(root);
  ...
  it('parses skillInstructions', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', skillInstructions: 'Do this.' }),
    });
    const result = await parseManifest(root);
  ...
  it('parses keywords metadata', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', keywords: ['finance', 'workflow'] }),
    });
    const result = await parseManifest(root);
  ...
  it('warns when hooks is not an array', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', hooks: { event: 'Stop', command: 'x' } }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.hooks).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a commands directory to its .md files', async () => {
    const root = await makePlugin(
      {
        'mirri.plugin.json': JSON.stringify({ name: 'demo', commands: ['./commands'] }),
        'commands/deploy.md': '---\ndescription: Deploy\n---\nbody',
        'commands/env.md': '---\ndescription: Env\n---\nbody',
  ...
  it('accepts a single command .md file', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', commands: ['./deploy.md'] }),
      'deploy.md': '---\ndescription: Deploy\n---\nbody',
    });
    const result = await parseManifest(root);
    expect(result.manifest?.commands).toEqual([
  ...
  it('warns when commands is not a string or string[]', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', commands: { nope: true } }),
    });
    const result = await parseManifest(root);
  ...
  it('warns when a commands entry resolves outside the plugin', async () => {
    const root = await makePlugin({
      'mirri.plugin.json': JSON.stringify({ name: 'demo', commands: ['../outside.md'] }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.commands).toBeUndefined();
    expect(result.diagnostics).toContainEqual(expect.objectContaining({ severity: 'warn' }));
