...
describe('PluginManager', () => {
  ...
  it('install() accepts a .mirricode-plugin manifest', async () => {
    ...
    await mkdir(path.join(root, 'skills'), { recursive: true });
    await writeFile(
      path.join(root, '.mirricode-plugin', 'plugin.json'),
      JSON.stringify({
        name: 'superpowers',
        skills: './skills/',
  ...
  it('keeps a plugin in error state instead of losing it on a broken manifest', async () => {
    ...
    await manager.install(root);
    await writeFile(
      path.join(await managedPluginRoot(home, 'demo'), 'mirri-plugin.json'),
      '{ not json',
      'utf8',
    );
  ...
  it('install() from zip-url overwrites existing zip-url plugin', async () => {
    ...
    const zipBuffer2 = await createZipBuffer([
      { name: 'plugin/mirri-plugin.json', data: JSON.stringify({ name: 'zip-demo', version: '2.0.0' }) },
    ]);
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    const cdnZip = await createZipBuffer([
      { name: 'pkg/mirri-plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.0.0' }) },
    ]);
    ...
    const ghZip = await createZipBuffer([
      { name: 'pkg/mirri-plugin.json', data: JSON.stringify({ name: 'superpowers', version: '5.1.0' }) },
    ]);
    using _ = mockGithubFetch({
      releaseTag: 'v5.1.0',
      tarball: ghZip,
