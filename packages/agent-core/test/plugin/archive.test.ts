...
describe('extractZip', () => {
  ...
  it('prefers a plugin manifest at the archive root', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'mirri-plugin.json', data: '{"name":"root"}' },
      { name: 'examples/demo/mirri-plugin.json', data: '{"name":"demo"}' },
    ]);
  ...
  it('detects plugin root with mirri-plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/mirri-plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/readme.md', data: '# Test' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, 'mirri-plugin.json'), 'utf8');
  ...
  it('detects plugin root with .mirri-plugin/plugin.json', async () => {
    ...
    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, '.mirricode-plugin', 'plugin.json'), 'utf8');
    expect(manifest).toBe('{"name":"test"}');
  });
  ...
  it('detects a single wrapper directory before nested manifests', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'outer/mirri-plugin.json', data: '{"name":"outer"}' },
      { name: 'outer/inner/mirri-plugin.json', data: '{"name":"inner"}' },
    ]);
