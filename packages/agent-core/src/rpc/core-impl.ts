...
import { MirriFetchURLProvider } from '#/tools/providers/moonshot-fetch-url';
import { MirriWebSearchProvider } from '#/tools/providers/moonshot-web-search';
...
import { KaosShellNotFoundError, LocalKaos, type Kaos } from '@mirri-ai/kaos';
import type { ToolServices } from '../tools/support/services';

const MIRRICODE_PROVIDER_NAME = 'managed:mirri-code';
const MIRRICODE_BASE_URL_ENV = 'MIRRICODE_BASE_URL';
const MIRRICODE_OAUTH_HOST_ENV = 'MIRRICODE_OAUTH_HOST';
...
export class MirriCore implements PromisableMethods<CoreAPI> {
  ...
  async createSessionWithOverrides(
    ...
    // Read the workspace local config (`.mirri-code/local.toml`) through the
    ...
    // (https://github.com/mirri-ai/mirricode/issues/988). The local config is
  ...
  async resumeSessionWithOverrides(
    ...
    // Read `.mirri-code/local.toml` through the persistence (local) kaos, not the
  ...
  private managedMirriCodeEnvForPlugins(): Record<string, string> {
    ...
    if (baseUrl !== undefined) env[MIRRICODE_BASE_URL_ENV] = baseUrl;
    if (oauthHost !== undefined) env[MIRRICODE_OAUTH_HOST_ENV] = oauthHost;
    return env;
  }
  ...
  return {
    ...
    tokenProvider:
      service.oauth !== undefined
        ? resolveOAuthTokenProvider?.(MIRRICODE_PROVIDER_NAME, service.oauth)
        : undefined,
