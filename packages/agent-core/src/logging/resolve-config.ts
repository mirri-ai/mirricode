 ...
 *
 *   MIRRICODE_LOG_LEVEL=debug
 *   MIRRICODE_LOG_GLOBAL_MAX_BYTES=... MIRRICODE_LOG_GLOBAL_FILES=...
 *   MIRRICODE_LOG_SESSION_MAX_BYTES=... MIRRICODE_LOG_SESSION_FILES=...
 */
export function resolveLoggingConfig(input: ResolveLoggingInput): LoggingConfig {
  const env = input.env ?? process.env;
  return {
    level: parseLevel(env['MIRRICODE_LOG_LEVEL']) ?? DEFAULT_LOG_LEVEL,
    globalLogPath: resolveGlobalLogPath(input.homeDir),
    globalMaxBytes: parsePositiveInt(env['MIRRICODE_LOG_GLOBAL_MAX_BYTES']) ?? DEFAULT_GLOBAL_MAX_BYTES,
    globalFiles: parsePositiveInt(env['MIRRICODE_LOG_GLOBAL_FILES']) ?? DEFAULT_GLOBAL_FILES,
    sessionMaxBytes:
      parsePositiveInt(env['MIRRICODE_LOG_SESSION_MAX_BYTES']) ?? DEFAULT_SESSION_MAX_BYTES,
    sessionFiles: parsePositiveInt(env['MIRRICODE_LOG_SESSION_FILES']) ?? DEFAULT_SESSION_FILES,
  };
}
