...
import type { Event as ProtocolEvent } from '@mirri-ai/protocol';

/**
 * Naming convention inside this file:
 *
 * - `Event` (from `@mirri-ai/agent-core/base/common/event`) — the generic
 *   VSCode-style emitter accessor type. `Event<T>` is the listener-tuple
 *   type used to declare `readonly onDidXxx: Event<T>`.
 * - `ProtocolEvent` (alias of `@mirri-ai/protocol`'s `Event`) — the
