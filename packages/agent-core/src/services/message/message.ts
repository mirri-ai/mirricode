 ...
 *
 * Order:
 *   1. For `tool` role: emit a SINGLE `tool_result` part.
 *      - If the content contains media parts (image_url, video_url, audio_url),
 *        `output` is the raw mapped content-part array so the client can render
 *        images/attachments instead of plain text.
 *      - Otherwise the output is the flattened text of the kosong message's
 *        content parts (most tool messages emit a single text).
 *      `is_error` is taken from `ContextMessage.isError`.
...
function buildProtocolContent(msg: ContextMessage): MessageContent[] {
  if (msg.role === 'tool') {
    ...
    const output: unknown = hasMediaPart
      ? msg.content.map((p) => mapContentPart(p))
