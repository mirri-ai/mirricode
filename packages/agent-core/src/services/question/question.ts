 ...
 * role — routes `QuestionRequest`s coming out of `MirriCore` to a waiter
 ...
 *      `BridgeClientAPI` sees from `MirriCore.requestQuestion(...)`. See
...
export interface IQuestionService {
  ...
  /**
   * Called by the adapter when MirriCore needs the user to answer a question.
