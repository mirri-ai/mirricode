...
In-process service layer for the mirri-code server, now hosted inside
`@mirri-ai/agent-core` (merged from the former `@mirri-ai/services`
...
from `index.ts` so importing `@mirri-ai/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@mirri-ai/agent-core` loads the package barrel, whose impl
