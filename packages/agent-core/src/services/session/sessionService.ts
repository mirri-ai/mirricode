...
export class SessionService extends Disposable implements ISessionService {
  ...
  private _handleBusEvent(event: Event): void {
    ...
    switch (type) {
      // Session status transitions are synthesized only from the MAIN agent's
      // turn boundaries. Subagents share the session channel (their frames
      // carry their own agentId), so a subagent's turn.ended would otherwise
      // emit a bogus status_changed(idle) mid-turn — the client reads that as
      // "the turn finished" (driving notifications, sounds, unread dots and
      // the queued-message drain), while the real main-agent turn end is then
      // swallowed by dedup.
      case 'turn.started': {
        if (agentId !== MAIN_AGENT_ID) break;
