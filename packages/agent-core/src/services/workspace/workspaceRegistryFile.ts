 ...
 * best-effort touch (e.g. `MirriCore` registering the cwd on session creation).
 *
...
export async function readWorkspaceRegistryFile(
  ...
  try {
    ...
    warn?.({ path: registryPath, error: String(error) }, 'workspaces.json malformed; treating as empty');
    return emptyRegistryFile();
  }
  if (
