 ...
 *    state under `<MIRRICODE_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.mirricode-code`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("mirri-code (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `mirri-code (${options.serverName})`;
  }
