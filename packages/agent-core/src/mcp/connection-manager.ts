...
export class McpConnectionManager {
  ...
  async reconnect(name: string): Promise<void> {
    const entry = this.entries.get(name);
    if (entry === undefined) {
      throw new MirriError(ErrorCodes.MCP_SERVER_NOT_FOUND, `Unknown MCP server: ${name}`);
    }
    if (entry.config.enabled === false) {
      throw new MirriError(ErrorCodes.MCP_SERVER_DISABLED, `MCP server is disabled: ${name}`);
    }
