...
/**
 * Central mapping from capability → tool names that provide it.
 *
 * Sources:
 *  - Built-in / user tool `capabilities` field (from ExecutableTool metadata).
 ...
 *
 * Pure data structure; owned and populated by ToolManager. Not thread-safe;
 * the agent loop is single-tasked so this is fine.
 */
export class CapabilityRegistry {
  ...
  /**
   * Register a built-in / user tool's capability declarations. Both sources
   * are equivalent to the registry — they contribute local providers as
   * opposed to MCP-sourced ones from `applyIntegrations`.
   *
   * Reads `tool.capabilities` (may be undefined).
   */
  registerBuiltinTool(tool: ExecutableTool): void {
    if (tool.capabilities === undefined || tool.capabilities.length === 0) return;
    this.addTool(tool.name, tool.capabilities, 'builtin');
  }
