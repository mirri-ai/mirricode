 ...
 *     (via {@link addTask} / {@link removeTasks}) so that `mirri resume`
 ...
 * `MIRRICODE_CRON_NO_STALE=1` (bench / acceptance tests).
 *
...
export interface CronManagerOptions {
   ...
   * `resolveClockSources(process.env.MIRRICODE_CRON_CLOCK)` so production
   * picks up `MIRRICODE_CRON_CLOCK=file:...` automatically.
   ...
   * (1000ms unless `MIRRICODE_CRON_MANUAL_TICK=1`, which forces `null` here
...
export class CronManager {
   ...
   * later `mirri resume` can reload via {@link loadFromDisk}.
   */
  ...
  constructor(agent: Agent, opts: CronManagerOptions = {}) {
    ...
    this.clocks =
      opts.clocks ??
      resolveClockSources(process.env['MIRRICODE_CRON_CLOCK']) ??
      SYSTEM_CLOCKS;
    this.persistStore =
    ...
    this.scheduler = createCronScheduler({
      ...
      isKilled: () => process.env['MIRRICODE_DISABLE_CRON'] === '1',
      onFire: (task, ctx) => {
      ...
      // P1.8: `MIRRICODE_CRON_MANUAL_TICK=1` forces the scheduler into
      ...
      pollIntervalMs:
        process.env['MIRRICODE_CRON_MANUAL_TICK'] === '1'
          ? null
          : opts.pollIntervalMs,
    });
   ...
   * so a `mirri resume` does not coalesce-replay an already-delivered
   ...
   * `mirri resume`. No-op when persistence is not attached. Idempotent:
   ...
   * Stale judgment.
   *
   *   - `MIRRICODE_CRON_NO_STALE=1` short-circuits to false (bench).
  ...
  isStale(task: CronTask): boolean {
    if (process.env['MIRRICODE_CRON_NO_STALE'] === '1') return false;
   ...
   *
   * Gated on `MIRRICODE_CRON_MANUAL_TICK=1` for two reasons:
   *
   ...
   * Set `MIRRICODE_CRON_DEBUG=1` to surface the swallowed error to stderr —
   * mirrors `scheduler.ts`'s debugLog pattern so bench debugging can
   * see a bad tick.
   */
  private bindSigusr1(): void {
    if (process.platform === 'win32') return;
    if (process.env['MIRRICODE_CRON_MANUAL_TICK'] !== '1') return;
