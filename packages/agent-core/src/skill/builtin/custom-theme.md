---
name: custom-theme
description: Create or edit a mirri-code custom color theme — a JSON file under the resolved MIRRICODE_HOME data directory that recolors the TUI. Use when the user wants their own theme, asks for a specific palette or mood, or wants to tweak an existing custom theme's colors.
---

# Create a mirri-code custom theme (custom-theme)

Help the user design, write, and apply a custom color theme for the mirri-code TUI. A theme is a single JSON file; the TUI ships with `dark`, `light`, and `auto`, and any file the user adds becomes selectable alongside them.
...
- A theme lives at `<MIRRICODE_HOME>/themes/<name>.json`.
...
```
https://moonshotai.github.io/mirri-code/en/customization/themes.html
```
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<MIRRICODE_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
