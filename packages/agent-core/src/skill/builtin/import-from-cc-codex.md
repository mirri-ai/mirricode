...
Help them migrate selected local Claude Code and Codex assets into Mirri Code.
...
- Do **not** migrate `.agents` content. Mirri Code already supports `.agents`
...
- Tell the user to start a new session (for example `/new`) or restart Mirri Code
