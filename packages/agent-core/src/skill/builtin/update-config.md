---
name: update-config
description: Inspect or edit mirri-code's own config — `config.toml` (model, provider, permission, hooks) and `tui.toml` (theme, editor, notifications, auto-update). Use when the user asks what a setting does or wants to change one.
---

# Configure mirri-code (update-config)

Help the user inspect, change, and validate mirri-code's configuration files. The files are **TOML** with **snake_case** keys.
...
```
https://docs.mirricode.com/en/configuration/config-files.html
```
...
- Location: `<MIRRICODE_HOME>/config.toml` or `<MIRRICODE_HOME>/tui.toml`. For other scopes/files, defer to the official docs.
...
3. **Copy out a candidate (do not create from scratch)**: use **Bash** to copy the target verbatim — `cp config.toml config-new.toml` (same directory, `-new` suffix; for tui.toml, `cp tui.toml tui-new.toml`). **Leave the original untouched for now.**
   - Only when the target doesn't exist (nothing to copy) should you use **Write** to create a minimal skeleton candidate (e.g. just the comment line `# <MIRRICODE_HOME>/config.toml`).
...
5. **Validate the candidate** (see Capability 3, via `mirri doctor`). **If anything fails, keep Editing the candidate and re-validate, looping until it all passes.**
...
Use **`mirri doctor`** to validate the candidate you wrote — it doesn't start the TUI and doesn't modify any file; it runs mirri's own parser + schema (syntax and schema together), so it's the authoritative check. Pick the subcommand by which file you changed, and pass the **candidate** path explicitly:

- changed `config.toml` → `mirri doctor config <config-new.toml path>`
- changed `tui.toml` → `mirri doctor tui <tui-new.toml path>`
...
Then do two checks `mirri doctor` can't:
...
> To also check whether the currently **active** config is OK overall, run `mirri doctor` with no path (it checks the default `config.toml` + `tui.toml`, showing a missing one as skipped).
