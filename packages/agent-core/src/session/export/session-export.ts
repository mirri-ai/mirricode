...
export async function exportSessionDirectory(input: {
  ...
  if (sessionFiles.length === 0) {
    throw new MirriError(ErrorCodes.SESSION_EXPORT_NOT_FOUND, `Session "${input.summary.id}" has no exportable directory at "${sessionDir}"`, {
...
function defaultExportZipName(sessionId: string, now: Date): string {
  ...
  return `mirri-debug-${shortId}-${timestamp}.zip`;
}
