/**
 * MirriFetchURLProvider — host-side UrlFetcher.
 *
 * Flow:
 *   1. Try Mirri coding-fetch service (POST {url}, Bearer token from a
 *      narrow token provider, Accept: text/markdown, host-provided headers).
 *   2. Mirri 200 → return the body as `extracted` content (the
 *      service has already extracted the main page text on its side).
 *   3. Any Mirri failure — non-200, network error, or token
...
export interface MirriFetchURLProviderOptions {
...
export class MirriFetchURLProvider implements UrlFetcher {
  ...
  constructor(options: MirriFetchURLProviderOptions) {
  ...
  async fetch(url: string, options?: { toolCallId?: string }): Promise<UrlFetchResult> {
    try {
      const content = await this.fetchViaMirri(url, options?.toolCallId);
  ...
  private async fetchViaMirri(
    ...
    if (response.status !== 200) {
      ...
      throw new HttpFetchError(
        response.status,
        `Mirri fetch request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error('Mirri fetch service is not configured: missing API key or token provider.');
  }
}
