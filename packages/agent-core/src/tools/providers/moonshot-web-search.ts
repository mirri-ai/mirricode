/**
 * MirriWebSearchProvider — host-side `WebSearchProvider`.
 *
...
export interface MirriWebSearchProviderOptions {
...
interface MirriSearchResponse {
  search_results?: MirriSearchResult[];
}

export class MirriWebSearchProvider implements WebSearchProvider {
  ...
  constructor(options: MirriWebSearchProviderOptions) {
  ...
  async search(
    ...
    if (response.status === 401) {
      ...
      throw new Error(
        `Mirri search request failed: HTTP 401 (auth/unauthorized). ${detail}`.trim(),
      );
    }
    ...
    if (response.status !== 200) {
      const detail = await safeReadText(response);
      throw new Error(
        `Mirri search request failed: HTTP ${String(response.status)}. ${detail}`.trim(),
      );
    }

    const json = (await response.json()) as MirriSearchResponse;
  ...
  private async resolveApiKey(): Promise<string> {
    ...
    throw new Error(
      'Mirri search service is not configured: missing API key or token provider.',
    );
  }
}
