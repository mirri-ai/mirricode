 ...
 *
 * `wallNow` resolution is driven by the `MIRRICODE_CRON_CLOCK` env var; see
...
export interface ClockSources {
   ...
   * via `MIRRICODE_CRON_CLOCK`. Used for cron matching, `createdAt`, stale
   * judgment.
   */
  wallNow(): number;
 ...
 * whenever `MIRRICODE_CRON_CLOCK` is unset, set to `"system"`, or set to a
 ...
 * `process.env.MIRRICODE_CRON_CLOCK`).
 *
...
function debugInvalidSpec(spec: string, reason: string): void {
  ...
  // gated on MIRRICODE_CRON_DEBUG is enough — production is silent.
  if (process.env['MIRRICODE_CRON_DEBUG'] === '1') {
    process.stderr.write(
      `[cron/clock] invalid MIRRICODE_CRON_CLOCK spec ${JSON.stringify(spec)}: ${reason} — falling back to system clock\n`,
    );
  }
}
