 ...
 *
 *   MIRRI_SNAPSHOT_READER       'auto' (default) | 'legacy'
 *   MIRRI_SNAPSHOT_TIMEOUT_MS   integer ms hard ceiling on the auto path (default 4000)
 *   MIRRI_SNAPSHOT_CACHE_LIMIT  transcript LRU entries (default 32)
 */
...
export function loadSnapshotConfig(env: NodeJS.ProcessEnv = process.env): SnapshotConfig {
  const rawMode = env['MIRRI_SNAPSHOT_READER']?.trim().toLowerCase();
  const mode: SnapshotReaderMode = rawMode === 'legacy' ? 'legacy' : 'auto';
  return {
    mode,
    timeoutMs: parseInteger(env['MIRRI_SNAPSHOT_TIMEOUT_MS'], DEFAULT_TIMEOUT_MS, 100),
    cacheLimit: parseInteger(env['MIRRI_SNAPSHOT_CACHE_LIMIT'], DEFAULT_CACHE_LIMIT, 1),
  };
}
