 ...
 *     explicit whitelist (`MIRRICODE_CORS_ORIGINS`, no `*` wildcard — PLAN
...
const CORS_ALLOW_HEADERS = 'Content-Type, Authorization, X-Mirri-Client-Id, X-Mirri-Client-Name, X-Mirri-Client-Version, X-Mirri-Client-Ui-Mode';
...
/**
 * Parse `MIRRICODE_CORS_ORIGINS` into an allowlist.
 *
...
export function parseCorsOrigins(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['MIRRICODE_CORS_ORIGINS'];
  if (raw === undefined) {
    return [];
  }
  return raw
