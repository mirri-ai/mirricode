...
import { resolveMirriHome } from '@mirri-ai/agent-core-v2';
import { ulid } from 'ulid';

/** Default cadence for refreshing `heartbeat_at`. */
export const HEARTBEAT_INTERVAL_MS = 15_000;

export const DEFAULT_SERVER_DIR = join(resolveMirriHome(), 'server');
...
interface ServerInstanceDisk {
  ...
  started_at: number;
  heartbeat_at: number;
  // Wire name kept as `host_version`: external tooling (mirri-inspect's server
...
export interface InstanceRegistryOptions {
  /** Directory holding `<serverId>.json` files. Defaults to `<MIRRICODE_HOME>/server/instances`. */
...
/** Resolve the instances directory for a given home (or the default mirri home). */
 ...
 * talk to (e.g. the CLI's `server ps/kill` and the `mirri web` spawner).
 */
