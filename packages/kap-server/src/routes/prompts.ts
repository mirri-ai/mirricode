  ...
  const submitRoute = defineRoute(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // Ensure the agent has a model before the prompt is enqueued. The
        // main agent is created unbound (no Profile / Model) on first
        // request — `ensureReady()` validates a default model exists in config
        // but does not bind it. Without this, the loop enqueues the prompt,
        // creates a turn, and the LLM request silently fails (no model),
        // producing no output. If the caller passed a profile or model above,
        // the bind already happened; only fall back to the configured default
        // when the agent is still unbound.
        if (!resolved.profile.hasModel()) {
          const defaultModel = core.accessor.get(IConfigService).get<string>('defaultModel');
          if (defaultModel !== undefined && defaultModel !== '') {
            await resolved.profile.setModel(defaultModel);
          }
        }
  ...
  for (const part of content) {
    if (part.type === 'text') parts.push({ type: 'text', text: part.text });
      ...
      parts.push(match === null
        ...
        : { type: 'image', source: { kind: 'base64', media_type: match[1]!, data: match[2]!, id: part.imageUrl.id } });
      ...
      // the client.
      const mirriFile = parseMirriFileUrl(part.videoUrl.url);
      if (mirriFile !== undefined) {
        parts.push({ type: 'video', source: { kind: 'file', file_id: mirriFile.fileId } });
        continue;
      }
      const match = /^data:([^;]+);base64,(.*)$/.exec(part.videoUrl.url);
      parts.push(match === null
        ? { type: 'video', source: { kind: 'url', url: part.videoUrl.url, id: part.videoUrl.id } }
        : { type: 'video', source: { kind: 'base64', media_type: match[1]!, data: match[2]!, id: part.videoUrl.id } });
    }
  }
  return parts;
}
...
function contentToCoreParts(content: PromptSubmission['content']): ContentPart[] {
  ...
  for (const part of content) {
    ...
    else if (part.type === 'image' && part.source.kind === 'base64') parts.push({ type: 'image_url', imageUrl: { url: `data:${part.source.media_type};base64,${part.source.data}`, id: part.source.id } });
    else if (part.type === 'video' && part.source.kind === 'url') parts.push({ type: 'video_url', videoUrl: { url: part.source.url, id: part.source.id } });
    else if (part.type === 'video' && part.source.kind === 'base64') parts.push({ type: 'video_url', videoUrl: { url: `data:${part.source.media_type};base64,${part.source.data}`, id: part.source.id } });
  }
  return parts;
}
...
async function resolvePromptMediaFiles(
  ...
  for (const part of body.content) {
      ...
      if (compressed.changed) {
        ...
        content.push({
          ...
          source: {
            ...
            id: originalPath ?? undefined,
          },
        });
        changed = true;
      } else {
        // Even when the image needed no compression, persist the original so a
        // sub-agent can ReadMediaFile it when the bound model lacks image
        // input. Without this the capability-gap placeholder carries no path
        // and the LLM cannot delegate — the exact failure shown in the user's
        // screenshot ("I don't know where the image is").
        const dir = await resolveOriginalsDir();
        const originalPath = await persistOriginalImage(
          Buffer.from(part.source.data, 'base64'),
          canonicalMime,
    ...
    if (part.type === 'image') {
      ...
      if (compressed.changed) {
        const dir = await resolveOriginalsDir();
        originalPath = await persistOriginalImage(data, mediaType, { dir });
        content.push({
          type: 'text',
          text: buildImageCompressionCaption({
            original: {
    ...
    content.push({
      type: 'video',
      source: { kind: 'url', url: buildMirriFileUrl(file.meta.id, cachePath) },
    });
    changed = true;
  }
...
// This notice's exact shape is a client contract: mirri-web's messagesToTurns
