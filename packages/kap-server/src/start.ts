/**
 * Server bootstrap — wires `@mirri-ai/agent-core-v2` (DI × Scope engine) into
...
export interface ServerHostIdentity extends MirriHostIdentity {
...
export interface ServerStartOptions {
   ...
   * (User-Agent + `X-Msh-*` via `createMirriDefaultHeaders`), and the session
  ...
  /**
   * Directory of the built Mirri web UI (`dist-web`). When set, `GET /` and the
   ...
   * endpoint unintentionally; the CLI's `mirri web` host passes true.
   */
...
export async function startServer(opts: ServerStartOptions): Promise<RunningServer> {
  ...
  // one homeDir and consumers (the CLI's `server ps/kill`, `mirri web`, dev
  ...
  const { app: core } = bootstrap(
    {
      ...
      args: {
        ...
        requestHeaders: createMirriDefaultHeaders({ homeDir, ...opts.hostIdentity }),
  ...
  if (exposureClass !== 'loopback') {
    ...
    if (!passwordConfigured) {
      logger.warn(
        { host, exposureClass },
        'binding non-loopback host with token-only auth (no MIRRICODE_PASSWORD) — the bearer token printed in the startup banner is the only credential protecting this server',
      );
    }
  }
  ...
  async function registerOpenApi(): Promise<void> {
    ...
    await app.register(swagger, {
      openapi: {
        info: {
          title: 'Mirri Code Server API',
          description:
            'REST API for the Mirri Code local server. All JSON responses are wrapped in a uniform envelope `{ code, msg, data, request_id }`.',
  ...
  // never retried.
  //
  // There is no single-instance lock: a busy port may be a sibling mirri
 ...
 * busy port may be a sibling mirri instance. The `port + 1` walk then serves
