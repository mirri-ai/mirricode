...
import { ErrorCodes, Error2 } from '@mirri-ai/agent-core-v2';
...
const MIRRI_TO_PROTOCOL: Record<string, ErrorCode> = {
...
export function mapError(err: unknown, requestId: string): ReturnType<typeof errEnvelope> {
  if (err instanceof Error2) {
    const code = MIRRI_TO_PROTOCOL[err.code] ?? ErrorCode.INTERNAL_ERROR;
