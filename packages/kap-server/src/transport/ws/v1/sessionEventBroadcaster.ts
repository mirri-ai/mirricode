...
export class SessionEventBroadcaster {
  ...
  private onCoreEvent(event: GlobalEvent): void {
    if (event.type === 'event.session.created') {
      ...
      // mirri-web's Stop button (gated on session.status === 'running') never
 ...
 *
 * Exists so unchanged v1 consumers (mirri-code TUI / `mirri -p`, node-sdk) keep
 * working while v2-shaped consumers (mirri-web) keep the native event and ignore
