...
describe('server-v2 snapshot route enrichment', () => {
  it('attaches current_prompt_id to an in-flight turn from prompt active state', async () => {
    ...
    const previousReaderMode = process.env['MIRRI_SNAPSHOT_READER'];
    process.env['MIRRI_SNAPSHOT_READER'] = 'legacy';
    ...
    try {
      ...
      if (previousReaderMode === undefined) delete process.env['MIRRI_SNAPSHOT_READER'];
      else process.env['MIRRI_SNAPSHOT_READER'] = previousReaderMode;
    }
