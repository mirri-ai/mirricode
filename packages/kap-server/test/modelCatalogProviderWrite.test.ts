...
/** Same providers/models, but the global default model belongs to kimi provider. */
...
const MANAGED_TOML = [
  ...
  '',
  '[models."managed:mirri-code/kimi-k2"]',
...
describe('server-v2 /api/v1 provider write endpoints', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-provider-write-'));
  ...
  it('rejects deleting an OAuth-managed provider with 40003', async () => {
    ...
    const { body } = await deleteJson<unknown>('/api/v1/providers/managed%3Amirri-code');
    expect(body?.code).toBe(40003);
    expect(body?.msg).toContain('/oauth/logout');

    const providers = await getJson<{ items: Array<{ id: string }> }>('/api/v1/providers');
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['managed:mirri-code']);
  });
  ...
  it('rejects replacing an OAuth-managed provider with 40003', async () => {
    ...
    const { body } = await putJson<unknown>(
      '/api/v1/providers/managed%3Amirri-code',
      REPLACE_BODY,
    );
    expect(body.code).toBe(40003);
    expect(body.msg).toContain('/oauth/logout');
    ...
    expect(providers.body.data.items.map((p) => p.id)).toEqual(['managed:mirri-code']);
    const models = await getJson<{ items: Array<{ model: string }> }>('/api/v1/models');
    expect(models.body.data.items.map((m) => m.model)).toEqual(['managed:mirri-code/kimi-k2']);
  });
