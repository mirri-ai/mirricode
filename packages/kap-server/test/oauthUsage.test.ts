...
describe('server-v2 GET /api/v1/oauth/usage', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-oauth-usage-'));
  });
  ...
  it('passes through the error payload and forwards the provider query', async () => {
    ...
    expect(await getUsage('?provider=managed%3Amirri-code')).toEqual({
      kind: 'error',
      message: 'Authorization failed.',
      status: 401,
    });
    expect(getManagedUsage).toHaveBeenCalledWith('managed:mirri-code');
  });
});
...
describe('server-v2 GET /api/v1/oauth/userinfo', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-oauth-userinfo-'));
  });
  ...
  it('passes through the error payload and forwards the provider query', async () => {
    ...
    expect(await getUserInfo('?provider=managed%3Amirri-code')).toEqual({
      kind: 'error',
      message: 'Authorization failed.',
      status: 401,
    });
    expect(getManagedUserInfo).toHaveBeenCalledWith('managed:mirri-code');
  });
});
