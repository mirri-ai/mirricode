...
describe('formatHostErrorMessage', () => {
  it('includes the rejected host and allow guidance', () => {
    expect(formatHostErrorMessage('APP.Example.com:443')).toBe(
      "Invalid Host header: app.example.com; allow this host with MIRRICODE_ALLOWED_HOSTS=app.example.com or 'mirri web --allowed-host app.example.com'.",
    );
  });
});
...
describe('parseAllowedHosts', () => {
  it('splits, trims, and drops empties', () => {
    expect(parseAllowedHosts({ MIRRICODE_ALLOWED_HOSTS: ' a, .b.com, ' })).toEqual(['a', '.b.com']);
  });
...
describe('isHostCheckDisabled', () => {
  it('is true when set to "1"', () => {
    expect(isHostCheckDisabled({ MIRRICODE_DISABLE_HOST_CHECK: '1' })).toBe(true);
  });
...
describe('createHostCheck (onRequest hook)', () => {
  ...
  it('rejects a disallowed Host with the 40301 envelope', async () => {
    ...
    expect(body['msg']).toBe(
      "Invalid Host header: evil.com; allow this host with MIRRICODE_ALLOWED_HOSTS=evil.com or 'mirri web --allowed-host evil.com'.",
    );
...
describe('startServer allowedHosts — env + option merge', () => {
  const ENV_KEY = 'MIRRICODE_ALLOWED_HOSTS';
