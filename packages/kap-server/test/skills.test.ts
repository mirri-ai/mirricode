...
describe('server-v2 /api/v1 skills', () => {
  ...
  /** Seed a project skill bundle at `<root>/.mirri-code/skills/<name>/SKILL.md`. */
  async function seedProjectSkill(root: string, name: string): Promise<void> {
    const dir = join(root, '.mirri-code', 'skills', name);
    await mkdir(dir, { recursive: true });
    await writeFile(
      join(dir, 'SKILL.md'),
  ...
  describe('GET /api/v1/sessions/{sid}/skills', () => {
    ...
    it('does not expose the kimi-code-specific check-mirri-code-docs skill', async () => {
      const id = await createSession();
      const { body } = await getJson<{ skills: SkillWire[] }>(
        `/api/v1/sessions/${id}/skills`,
      );
      ...
      // v2 deliberately excludes the kimi-code-specific docs-checker skill;
      // the builtin set is the 6 mirri families (8 definitions) only.
      const docsSkill = skills.find((s) => s.name === 'check-mirri-code-docs');
      expect(docsSkill).toBeUndefined();
    });
  });
