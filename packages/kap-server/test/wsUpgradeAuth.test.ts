 ...
 * `mirri-code.bearer.<token>` subprotocol.
 */
...
describe('WS upgrade auth', () => {
  ...
  describe('/api/v1/ws', () => {
    ...
    it('accepts a valid bearer subprotocol and echoes it', async () => {
      const { ws, firstFrame } = await openConn(url(), {
        protocols: [`mirri-code.bearer.${TOKEN}`],
      });
      sockets.push(ws);
      expect(ws.protocol).toBe(`mirri-code.bearer.${TOKEN}`);
    ...
    it('rejects a wrong bearer token', async () => {
      await expectRejected(url(), { protocols: ['mirri-code.bearer.wrong'] });
    });
  ...
  it('rejects upgrades to a non-WS path', async () => {
    ...
    await expectRejected(badUrl, { protocols: [`mirri-code.bearer.${TOKEN}`] });
  });
});
