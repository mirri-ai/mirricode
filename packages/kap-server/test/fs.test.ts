...
describe('server-v2 /api/v1 fs routes', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-fs-home-'));
    work = await mkdtemp(join(tmpdir(), 'mirri-server-v2-fs-work-'));
  ...
  it('fs:search resolves a registered workspace id when no session exists', async () => {
    ...
    // Register the workspace without creating any session (the mirri-web
  ...
  it('rejects reads and downloads that escape the workspace through a symlink', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'mirri-server-v2-fs-outside-'));
    try {
  ...
  it('serves fs actions when the session cwd itself goes through a symlink', async () => {
    const link = join(tmpdir(), `mirri-server-v2-fs-cwd-link-${process.pid}`);
