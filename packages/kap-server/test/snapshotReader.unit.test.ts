 ...
 * `state.json` normalization, and `MIRRI_SNAPSHOT_*` config parsing without
...
async function makeFixtureAsync(opts?: { cacheLimit?: number }): Promise<Fixture> {
  const homeDir = await mkdtemp(join(tmpdir(), 'mirri-snapshot-reader-'));
...
describe('readWireRecords', () => {
  it('drops a torn final line but throws on mid-file corruption', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'mirri-wire-'));
    tmpDirs.push(dir);
...
describe('loadSnapshotConfig', () => {
  ...
  it('parses legacy mode and integer knobs with floors', () => {
    const c = loadSnapshotConfig({
      MIRRI_SNAPSHOT_READER: 'legacy',
      MIRRI_SNAPSHOT_TIMEOUT_MS: '2500',
      MIRRI_SNAPSHOT_CACHE_LIMIT: '0', // below min → default
    });
  ...
  it('falls back on non-numeric / sub-minimum timeout', () => {
    expect(loadSnapshotConfig({ MIRRI_SNAPSHOT_TIMEOUT_MS: 'abc' }).timeoutMs).toBe(4000);
    expect(loadSnapshotConfig({ MIRRI_SNAPSHOT_TIMEOUT_MS: '50' }).timeoutMs).toBe(4000);
  });
});
