...
describe('server-v2 /api/v1 model/provider catalog', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-model-catalog-'));
    ...
    process.env['MIRRICODE_MODEL_CATALOG_REFRESH_ON_START'] = '0';
    process.env['MIRRICODE_MODEL_CATALOG_REFRESH_INTERVAL_MS'] = '0';
  });
  ...
  afterEach(async () => {
    ...
    delete process.env['MIRRICODE_MODEL_CATALOG_REFRESH_ON_START'];
    delete process.env['MIRRICODE_MODEL_CATALOG_REFRESH_INTERVAL_MS'];
  });
  ...
  it('refreshes OAuth provider models through POST /providers:refresh_oauth', async () => {
    const refreshOAuthProviderModels = vi.fn(async () => ({
      changed: [
        { provider_id: 'managed:mirri-code', provider_name: 'Mirri Code', added: 1, removed: 0 },
      ],
      unchanged: [],
      failed: [],
    }));
  ...
  it('refreshes all provider models through POST /providers:refresh', async () => {
    const refreshProviderModels = vi.fn(async () => ({
      changed: [
        { provider_id: 'managed:mirri-code', provider_name: 'Mirri Code', added: 2, removed: 1 },
      ],
  ...
  it('refreshes a single provider through POST /providers/{id}:refresh', async () => {
    ...
    const { status, body } = await postJson('/api/v1/providers/managed%3Amirri-code:refresh', {});
    expect(status).toBe(200);
    expect(body.code).toBe(0);
    expect(refreshProviderModels).toHaveBeenCalledWith({ providerId: 'managed:mirri-code' });
  });
  ...
  it('deletes a model alias through POST /models/{id}:delete and records the tombstone', async () => {
    await boot(CATALOG_TOML);

    const { status, body } = await postJson<unknown>('/api/v1/models/turbo:delete', {});
    expect(status).toBe(200);
    expect(body.code).toBe(0);
    expect(body.data).toEqual({ deleted: true });

    // The alias is gone from the provider's model list…
    const provider = await getJson<{ models?: string[] }>('/api/v1/providers/kimi');
    expect(provider.body.code).toBe(0);
    expect(provider.body.data?.models).toEqual(['k2']);

    // …and the deletion is tombstoned on the provider config so catalog
    // re-imports never resurrect it. The kosongConfig bridge persists the
    // registry change asynchronously — poll briefly for it. (In-memory config
    // values are camelCase after the TOML transform; the file stores
    // `deleted_models`.)
    const cfg = server!.core.accessor.get(IConfigService);
    await cfg.ready;
    let deletedModels: string[] | undefined;
    for (let attempt = 0; attempt < 50; attempt++) {
      const providers = cfg.get<Record<string, { deletedModels?: string[] }>>('providers') ?? {};
      deletedModels = providers['kimi']?.deletedModels;
      if (deletedModels !== undefined) break;
      await new Promise((resolve) => setTimeout(resolve, 50));
    }
    expect(deletedModels).toContain('kimi-turbo');
  });

  it('maps an unknown model id on :delete to 40413', async () => {
    await boot(CATALOG_TOML);
