...
import type { Terminal } from '@mirri-ai/agent-core-v2/os/interface/terminal';
...
describe('server-v2 /api/v1/sessions/{sid}/terminals', () => {
  ...
  beforeEach(async () => {
    ...
    processes.length = 0;
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-term-home-'));
    work = await mkdtemp(join(tmpdir(), 'mirri-server-v2-term-work-'));
    await writeFile(
  ...
  it('creates terminals for multiple sessions using each session workspace cwd', async () => {
    const rootA = await mkdtemp(join(tmpdir(), 'mirri-server-v2-term-a-'));
    const rootB = await mkdtemp(join(tmpdir(), 'mirri-server-v2-term-b-'));
    try {
