 ...
 * Run: `pnpm --filter @mirri-ai/kap-server exec vitest run test/sessions.test.ts`.
 */
...
import { sessionWarningsResponseSchema } from '@mirri-ai/agent-core-v2/app/sessionLegacy/sessionProtocol';
...
describe('server-v2 /api/v1/sessions', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-sessions-'));
  ...
  it('downloads a ZIP with the supplied Web log and cleans up its temporary directory', async () => {
    ...
    expect(res.headers.get('content-disposition')).toBe(
      `attachment; filename="mirri-session-${id}.zip"`,
    );
    ...
    expect(entries.get('logs/mirri-web.jsonl')?.toString('utf8')).toBe(webLog);
    expect(manifest).toMatchObject({
      sessionId: id,
      kimiCodeVersion: TEST_HOST_IDENTITY.version,
      webLogPath: 'logs/mirri-web.jsonl',
    });
  ...
  it('bundles the on-disk desktop app log when the desktop flag is set', async () => {
    ...
    await writeFile(
      join(home as string, 'logs', 'mirri-code-desktop.log'),
    ...
    expect(entries.get('logs/mirri-desktop.log')?.toString('utf8')).toBe(
      '2026-07-27T00:00:00.000Z INFO  [renderer] hello\n',
    );
    expect(manifest.desktopLogPath).toBe('logs/mirri-desktop.log');
...
async function listExportTempDirs(sessionId: string): Promise<string[]> {
  const prefix = `mirri-session-export-${sessionId}-`;
...
describe('server-v2 /api/v1/sessions status context window', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-status-'));
    await writeFile(
