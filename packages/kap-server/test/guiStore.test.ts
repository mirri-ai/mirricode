...
describe('server-v2 gui store routes', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-server-v2-gui-store-'));
  ...
  it('quotes dotted keys in the persisted TOML and reads them back', async () => {
    const api = appOf(server as RunningServer);
    await setItem(api, 'mirri-web.theme', 'modern');

    const text = await readFile(join(home as string, 'gui.toml'), 'utf-8');
    expect(text).toContain('"mirri-web.theme" = "modern"');

    const env = envelopeOf<{ value: string | null }>((await getItem(api, 'mirri-web.theme')).json());
