 ...
 * Run: `pnpm --filter @mirri-ai/kap-server exec vitest run test/workspaceLayout.test.ts`.
 */
...
describe('local/local on-disk layout (byte compatibility)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'mirri-layout-home-'));
    workDir = await mkdtemp(join(tmpdir(), 'mirri-layout-work-'));
