...
describe('parseCorsOrigins', () => {
  it('splits, trims, and drops empties', () => {
    expect(parseCorsOrigins({ MIRRICODE_CORS_ORIGINS: ' https://a.com, https://b.com, ' })).toEqual([
      'https://a.com',
      'https://b.com',
    ]);
  });
...
describe('createOriginHook (onRequest hook)', () => {
  ...
  it('reflects Access-Control-Request-Headers in Allow-Headers for an allowed origin', async () => {
    const res = await app.inject({
      ...
      headers: {
        ...
        'access-control-request-headers': 'x-request-id, x-mirri-client-version, authorization',
      },
    });
    expect(res.statusCode).toBe(204);
    expect(res.headers['access-control-allow-origin']).toBe('https://foo.com');
    expect(res.headers['access-control-allow-headers']).toBe(
      'x-request-id, x-mirri-client-version, authorization',
    );
  });
  ...
  it('falls back to CORS_ALLOW_HEADERS for non-preflight responses', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.headers['access-control-allow-headers']).toBe(
      'Content-Type, Authorization, X-Mirri-Client-Id, X-Mirri-Client-Name, X-Mirri-Client-Version, X-Mirri-Client-Ui-Mode',
    );
  });
});
