...
type PromptContentPart =
  ...
  | {
      type: 'image';
      source: { kind: 'base64'; media_type: string; data: string; id?: string };
    };
...
describe('server-v2 /api/v1 prompts', () => {
  ...
  it('compresses uploaded image prompts into base64 image parts with a readback caption', async () => {
    ...
    // The persisted original path must land in `source.id` so a sub-agent can
    // ReadMediaFile it when the bound model lacks image input.
    expect(image.source.id).toBeDefined();
    expect(image.source.id).toContain('/media-originals/');
  });
  ...
  it('compresses inline base64 image prompts into session media-originals', async () => {
    ...
    // The persisted original path must land in `source.id` so a sub-agent can
    // ReadMediaFile it when the bound model lacks image input. Without this
    // the capability-gap placeholder carries no path and the LLM cannot
    // delegate.
    expect(image.source.id).toBeDefined();
    expect(image.source.id).toContain('/media-originals/');
  });

  it('persists an inline base64 image even when no compression is needed, carrying the path in source.id', async () => {
    // Regression: when an image is small enough that compression is a no-op,
    // the original must still be persisted to media-originals and the path
    // recorded in `source.id`. Without this, a model that lacks image input
    // gets a capability-gap placeholder with no file path and cannot delegate
    // to a multimodal sub-agent — the LLM says "I don't know where the image
    // is."
    const id = await createSession(home as string);
    await createMainAgent(id);
    const smallPng = solidPng(120, 60);
    ...
    if (image?.type !== 'image' || image.source.kind !== 'base64') {
      throw new Error('expected base64 image part');
    }
    expect(image.source.id).not.toBeUndefined();
    expect(image.source.id).toContain('/media-originals/');
    expect(await readFile(image.source.id!)).toEqual(smallPng);
  });

  it('persists an uploaded image file even when no compression is needed, carrying the path in source.id', async () => {
    // Regression: same as the inline case but for the file-upload path. When
    // compression is a no-op the original must still be persisted and the
    // path recorded in `source.id`.
    const id = await createSession(home as string);
    await createMainAgent(id);
    const smallPng = solidPng(120, 60);
    const form = new FormData();
  ...
  // Note: "abort an already-settled prompt returns 40402" is covered by the
  // "aborting an unknown prompt" test below — from abort()'s perspective a
  // settled prompt is no longer in active/pending, so it throws
  // PROMPT_NOT_FOUND just like an unknown id. We cannot reliably settle a
  // prompt here because the stub provider points at an unreachable port
  // (127.0.0.1:9999), which keeps the turn in a retrying active state.
  ...
  it('binds a discovered custom agent profile on the first prompt', async () => {
    // A user-level agent file under $MIRRICODE_HOME/agents is discovered into
