...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'mirri-server-v2-auth-token-'));
});
...
describe('password', () => {
  it('resolvePasswordHash returns undefined when env is unset or empty', async () => {
    ...
    expect(await resolvePasswordHash({ MIRRICODE_PASSWORD: '' })).toBeUndefined();
  });

  it('hashes a set password with bcrypt and verifies correctly', async () => {
    const passwordHash = await resolvePasswordHash({
      MIRRICODE_PASSWORD: 'correct-horse-battery-staple',
    });
