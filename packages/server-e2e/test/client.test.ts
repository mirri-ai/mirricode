 ...
 * `process.env.MIRRICODE_SERVER_URL ?? http://127.0.0.1:58627`.
 *
...
const BASE_URL = process.env['MIRRICODE_SERVER_URL'] ?? 'http://127.0.0.1:58627';
...
describe('DaemonClient session action helpers', () => {
  ...
  it('model catalog helpers call the catalog and action-suffix routes', async () => {
    ...
    const calls: FetchCall[] = [];
    const model = testModel({ model: 'mirri-code/kimi-for-coding' });
    ...
    expect(calls.map((call) => [call.init.method, call.url])).toEqual([
      ...
      ['POST', 'http://server.example.test/api/v1/models/mirri-code%2Fkimi-for-coding:set_default'],
