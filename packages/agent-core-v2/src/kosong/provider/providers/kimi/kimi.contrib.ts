 ...
 *     - request params: the `MIRRI_API_KEY` / `MIRRI_BASE_URL` endpoint
 ...
 *       schema dialect (`normalizeMirriToolSchema`);
 ...
 *       `classifyMirriQuotaError`, before the base's own classification would
 *       mint a retryable rate limit;
 *     - video upload: `uploadVideo` uploads through the Kimi files API
 *       (`MirriFiles`), memoized per trait context with a
...
import { classifyMirriQuotaError } from './kimi-errors';
import { MirriFiles } from './kimi-files';
import { normalizeMirriToolSchema } from './kimi-schema';

export const MIRRI_API_KEY_ENV = 'MIRRI_API_KEY';
export const MIRRI_BASE_URL_ENV = 'MIRRI_BASE_URL';
export const MIRRI_DEFAULT_BASE_URL = 'https://api.moonshot.ai/v1';
...
export interface MirriThinkingConfig {
...
export interface ExtraBody {
  thinking?: MirriThinkingConfig;
  [key: string]: unknown;
}

export function convertMirriTool(tool: Tool): OpenAIToolParam {
  ...
  return {
    ...
    function: {
      ...converted.function,
      parameters: normalizeMirriToolSchema(tool.parameters),
    },
  };
}
...
const filesByContext = new WeakMap<TraitContext, MirriFiles>();
...
function resolveFiles(ctx: TraitContext): MirriFiles {
  let files = filesByContext.get(ctx);
  if (files === undefined) {
    files = new MirriFiles({
      apiKey: ctx.config.apiKey ?? firstEnv(MIRRI_API_KEY_ENV),
      baseUrl: ctx.config.baseUrl ?? firstEnv(MIRRI_BASE_URL_ENV) ?? MIRRI_DEFAULT_BASE_URL,
      defaultHeaders:
...
export const kimiOpenAITrait: ProtocolTrait = {
  ...
  endpoint: () => ({
    apiKeyEnv: MIRRI_API_KEY_ENV,
    baseUrlEnv: MIRRI_BASE_URL_ENV,
    defaultBaseUrl: MIRRI_DEFAULT_BASE_URL,
  }),

  convertError: (error) => classifyMirriQuotaError(error),

  cacheKey: (key) => ({ prompt_cache_key: key }),

  withThinking: (effort, options, generationKwargs) => {
    const thinking: MirriThinkingConfig =
      effort === 'off'
        ? { type: 'disabled' }
        : effort === 'on'
          ? { type: 'enabled' }
  ...
  convertTool: (tool) => convertMirriTool(tool),
  ...
  convertMessage: (message, converted) => {
    ...
    if (message.tools !== undefined && message.tools.length > 0) {
      converted['tools'] = message.tools.map((tool) => convertMirriTool(tool));
    }
