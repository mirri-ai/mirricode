...
export interface MirriUploadOptions {
  auth?: ProviderRequestAuth;
  signal?: AbortSignal;
}

export interface MirriFilesOptions {
  apiKey?: string;
  baseUrl: string;
...
export class MirriFiles {
  ...
  constructor(options: MirriFilesOptions) {
  ...
  async uploadVideo(
    ...
    if (typeof input === 'string') {
      ...
      if (mimeType === undefined || !mimeType.startsWith('video/')) {
        throw new ChatProviderError(
          `MirriFiles.uploadVideo: file extension does not indicate a video type: ${filename}`,
        );
      }
    ...
    try {
      ...
      throw convertOpenAIError(error, classifyMirriQuotaError);
    }
    ...
    return resolveAuthBackedClient(
      ...
      (a) => {
        ...
        return new OpenAIClient({
          apiKey: requireProviderApiKey('MirriFiles.uploadVideo', a, this._apiKey),
