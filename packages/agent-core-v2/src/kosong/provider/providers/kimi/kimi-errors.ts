...
const MIRRI_QUOTA_EXHAUSTED_ERROR_CODES = new Set(['exceeded_current_quota_error']);

const MIRRI_QUOTA_EXHAUSTED_MESSAGE_PATTERNS = [
...
export function classifyMirriQuotaError(error: unknown): APIProviderQuotaExhaustedError | undefined {
  ...
  const structuredHit = collectErrorCodes(error).some((code) =>
    MIRRI_QUOTA_EXHAUSTED_ERROR_CODES.has(code),
  );
  const lowerMessage = message.toLowerCase();
  const wordingHit = MIRRI_QUOTA_EXHAUSTED_MESSAGE_PATTERNS.some((pattern) =>
