...
import { parseMirriCodeCustomHeaders } from '@mirri-ai/v2-oauth';
...
export class ModelCatalog extends Disposable implements IModelCatalog {
  ...
  getById(modelId: string): Model | undefined {
    try {
      return this.entry(modelId).model;
    } catch {
      return undefined;
    }
  }
...
export function resolveOutboundHeaders(
  ...
  return { ...parseMirriCodeCustomHeaders(), ...hostLayer, ...customHeaders };
}
...
function resolveModelCapabilities(
  ...
  // When the protocol catalog could not identify the model (detected is
  // UNKNOWN_CAPABILITY) and the user declared no capabilities of their own,
  // the resolved capability stays "unknown" — the media-capability projector
  // must pass media through to the provider rather than preemptively stripping
  // based on default-false flags. We detect this by checking the UNKNOWN marker
  // on `detected`; if present AND no declared capabilities override any modality,
  // propagate the unknown status so `isUnknownCapability` still returns true.
  // The spread operator does NOT copy the non-enumerable symbol marker, so we
  // re-attach it explicitly via Object.defineProperty.
  const isDetectedUnknown = isUnknownCapability(detected);
  const hasDeclaredOverride =
    declared.has('image_in') ||
    declared.has('video_in') ||
    declared.has('audio_in') ||
    declared.has('thinking') ||
    declared.has('always_thinking') ||
    declared.has('tool_use') ||
    declared.has('dynamically_loaded_tools');
  if (isDetectedUnknown && !hasDeclaredOverride) {
    // Keep max_context_tokens from config (callers that gate on context length
    // still need it), but preserve the UNKNOWN marker so downstream
    // `isUnknownCapability` checks still return true. The spread operator does
    // NOT copy the non-enumerable symbol marker, so we re-attach it explicitly.
    const resolved: ModelCapability = {
      ...detected,
      max_context_tokens: maxContextSize,
      max_input_tokens: maxInputSize,
    };
    return Object.defineProperty(resolved, UNKNOWN_CAPABILITY_MARKER, { value: true }) as ModelCapability;
  }
  return {
