 ...
 * two shapes:
 *   - want data    → `get(id)` / `getById(id)`  → the pure-data Model;
...
export interface IModelCatalog {
  ...
  get(id: string): Model;
  /**
   * Look up a Model by its `modelId` (the `[models.<id>]` config key).
   * Returns `undefined` when no model is configured for that id. This is the
   * sole lookup path — callers must resolve the canonical modelId upstream.
   */
