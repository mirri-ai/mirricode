...
registerAgentProfile({
  ...
  tools: AGENT_TOOLS,
  // No `subagents` allowlist: the default agent can spawn any profile in the
  // catalog, so user-defined subagents (e.g. ~/.mirri-code/agents/media-reader.yaml)
  // are discoverable without code changes. A profile that wants to restrict
  // its children declares its own `subagents` list.
