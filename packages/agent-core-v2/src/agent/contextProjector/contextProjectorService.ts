...
import { buildMediaPlaceholder, type MediaKind } from '#/agent/messagePipeline/mediaPlaceholder';
import {
...
/**
 * Map a degradable media part's wire type to the `MediaKind` used by the
 * shared placeholder builder. The wire types (`image_url` / `audio_url` /
 * `video_url`) carry the same modality labels as `MediaKind` minus the
 * `_url` suffix.
 */
function mediaKindOfDegradablePart(part: DegradableMediaPart): MediaKind {
  switch (part.type) {
    case 'image_url':
      return 'image';
    case 'audio_url':
...
/**
 * The persisted file path attached to a degradable media part's container,
 * when one was recorded. Only filesystem-style paths are recognised so a
 * protocol identifier such as `ms://file-1` is never mistaken for a file the
 * sub-agent can open. Mirrors the `id` semantics of the capability-gap
 * projector so a degraded/stripped placeholder can carry the same recovery
 * handle.
 */
function persistedPathOfDegradablePart(part: DegradableMediaPart): string | undefined {
  const id = mediaContainer(part).id;
...
function degradedPlaceholderFor(part: DegradableMediaPart): string {
  return buildMediaPlaceholder({
    kind: mediaKindOfDegradablePart(part),
    reason: 'size_limit',
    filePath: persistedPathOfDegradablePart(part),
  });
}

function strippedPlaceholderFor(part: DegradableMediaPart): string {
  return buildMediaPlaceholder({
    kind: mediaKindOfDegradablePart(part),
...
type DegradableMediaPart = Extract<
  ContentPart,
  { readonly type: 'image_url' | 'audio_url' | 'video_url' }
>;
...
const DEGRADABLE_PART_TYPES = new Set<string>(['image_url', 'audio_url', 'video_url']);

function isDegradableMediaPart(
  part: ContentPart,
): part is DegradableMediaPart {
  return DEGRADABLE_PART_TYPES.has(part.type);
}
...
export function stripMediaPartsBySnapshot(
  ...
  const result = messages.map((message) => {
    ...
    const content = message.content.map((part): ContentPart => {
      ...
      changed = true;
      messageChanged = true;
      return { type: 'text', text: strippedPlaceholderFor(part) };
    });
...
export function degradeOlderMediaParts(
  ...
  return messages.map((message) => {
    ...
    const content = message.content.map((part): ContentPart => {
      ...
      toDegrade -= 1;
      return { type: 'text', text: degradedPlaceholderFor(part) };
    });
