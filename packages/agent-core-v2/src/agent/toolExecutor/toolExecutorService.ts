...
import type { ToolInputDisplay } from '@mirri-ai/protocol';
...
export class AgentToolExecutorService implements IAgentToolExecutorService {
  ...
  private async prepareToolCall(
    ...
    const effectiveArgs = decision?.rewrittenArgs ?? call.args;

    // When the hook rewrites args, re-resolve execution with the new args
    // so that display fields, description, and the execute closure all
    // reflect the rewritten input.
    let effectiveExecution: RunnableToolExecution = execution;
    if (decision?.rewrittenArgs !== undefined) {
      try {
        const reResolved = await call.tool.resolveExecution(effectiveArgs);
        if (reResolved.isError !== true) {
          effectiveExecution = reResolved;
        }
      } catch {
        // If re-resolution fails, fall back to the original execution.
      }
    }

    const effectiveDisplayFields = decision?.rewrittenArgs !== undefined
      ? toolCallDisplayFieldsFromExecution(effectiveExecution)
      : displayFields;
    ...
    this.dispatchToolCall(call, effectiveArgs, options, effectiveDisplayFields);

    return {
      task: {
        accesses: effectiveExecution.accesses ?? ToolAccesses.all(),
        execute: async (taskSignal) =>
          this.runSingleExecution(call, effectiveExecution, executionMetadata, options, taskSignal),
      },
      stopBatchAfterThis: effectiveExecution.stopBatchAfterThis,
    };
  }
