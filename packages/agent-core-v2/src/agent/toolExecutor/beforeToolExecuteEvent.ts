...
export class BeforeToolExecuteEventImpl implements BeforeToolExecuteEvent {
  ...
  private _rewrittenArgs: unknown;
  ...
  rewriteArgs(newArgs: unknown): void {
    this.assertOpen('rewriteArgs');
    if (this._rewrittenArgs === undefined) {
      this._rewrittenArgs = newArgs;
    }
  }
  ...
  get rewrittenArgs(): unknown {
    return this._rewrittenArgs;
  }
...
export class BeforeToolExecuteEmitter extends Emitter<BeforeToolExecuteEvent> {
  async fireBeforeExecute(
    ...
    for (const entry of Array.from(this._listeners)) {
      ...
      if (event.finalAllowed) {
        return event.rewrittenArgs === undefined
          ? undefined
          : { rewrittenArgs: event.rewrittenArgs };
      }
    ...
    let rewrittenArgs = event.rewrittenArgs;
    for (const factory of event.pendingVetos) {
      ...
      rewrittenArgs ??= decision?.rewrittenArgs;
    }
    if (passMetadata === undefined && rewrittenArgs === undefined) return undefined;
    return {
      ...(passMetadata !== undefined ? { executionMetadata: passMetadata } : undefined),
      ...(rewrittenArgs !== undefined ? { rewrittenArgs } : undefined),
    };
  }
}
