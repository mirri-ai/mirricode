...
export interface BeforeExecuteDecision {
  ...
  readonly rewrittenArgs?: unknown;
}
...
export interface BeforeToolExecuteEvent extends ResolvedToolExecutionHookContext {
  ...
  /**
   * Replace the tool arguments with new values. The first call wins;
   * subsequent calls are silently ignored. This is used by external
   * PreToolUse hooks (gated by the `hook-command-rewrite` flag) to
   * transparently rewrite tool arguments before execution.
   */
