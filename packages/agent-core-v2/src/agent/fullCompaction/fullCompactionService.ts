...
import { ErrorCodes, Error2, isCodedError, isError2, toMirriErrorPayload, unwrapErrorCause } from "#/errors";
...
export class AgentFullCompactionService extends Disposable implements IAgentFullCompactionService {
  ...
  private async compactionRound(
    ...
    try {
      ...
      // Collect any hook_additional_context messages injected by onWillCompact
      // hooks (e.g. PreCompact additionalContext) so the summarizer sees them.
      const currentHistory = this.context.get();
...
function historySafeToCompact(
  ...
  return current.slice(original.length).every(
    (message) => isRealUserInput(message) || isHookAdditionalContext(message),
  );
}
