...
function resultFromExitCode(exitCode: number, stdout: string, stderr: string): HookResult {
  ...
  if (structured?.action === 'block') {
    // Block via permissionDecision:deny carries additionalContext but NOT updatedInput
    return {
      action: 'block',
  ...
  // Allow path carries both updatedInput and additionalContext
  return allowResult({
...
function structuredOutput(stdout: string): StructuredOutputResult | undefined {
  ...
  try {
    ...
    const result: StructuredOutputResult = {
    ...
    // Block via deny: carries additionalContext but discards updatedInput
    return {
      action: 'block',
