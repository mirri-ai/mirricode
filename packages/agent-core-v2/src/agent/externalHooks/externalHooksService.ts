...
import { toMirriErrorPayload } from '#/errors';
...
export class AgentExternalHooksService extends Disposable implements IAgentExternalHooksService {
  ...
  private registerToolHooks(toolExecutor: IAgentToolExecutorService): void {
    this._register(
      toolExecutor.onBeforeExecuteTool(async (event) => {
        const blockReason = await this.runPreToolUse(event);
        if (blockReason !== undefined) {
          event.veto(denyToolExecution(blockReason));
        }
      }),
    );
    this._register(
  ...
  private async runPreToolUse(event: BeforeToolExecuteEvent): Promise<string | undefined> {
    event.signal.throwIfAborted();
    const toolInput = isPlainRecord(event.args) ? event.args : {};
    const results = await this.runner.trigger('PreToolUse', {
      matcherValue: event.toolCall.name,
      signal: event.signal,
      sessionId: this.sessionContext.sessionId,
      inputData: {
    ...
    if (block !== undefined) {
      // Block: still inject additionalContext from deny-decision results
      injectHookAdditionalContext(this.context, results, 'PreToolUse', { includeBlock: true });
      return block.reason;
    }

    // Allow path: inject additionalContext and apply updatedInput when flag is enabled
    injectHookAdditionalContext(this.context, results, 'PreToolUse');

    if (this.flags.enabled(HOOK_COMMAND_REWRITE_FLAG_ID)) {
      const modified = results.find((r) => r.updatedInput !== undefined);
      if (modified !== undefined) {
        event.rewriteArgs(modified.updatedInput);
      }
    }
  ...
  private notifyPostToolUse(ctx: ToolDidExecuteContext): void {
    ...
    this.fireAndForget(
      isError ? 'PostToolUseFailure' : 'PostToolUse',
      {
        ...
        error: isError ? toMirriErrorPayload(output) : undefined,
  ...
  private async runPreCompact(ctx: FullCompactionTask): Promise<void> {
    ...
    signal.throwIfAborted();
    const results = await this.runner.trigger('PreCompact', {
    ...
    signal.throwIfAborted();
    injectHookAdditionalContext(this.context, results, 'PreCompact');
  }
