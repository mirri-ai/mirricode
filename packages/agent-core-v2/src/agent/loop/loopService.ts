...
import { BugIndicatingError, ErrorCodes, Error2, isError2, toMirriErrorPayload } from '#/errors';
...
export class AgentLoopService extends Disposable implements IAgentLoopService {
  ...
  private async runTurn(
    ...
    try {
      ...
      if (result !== undefined) {
        const error = result.type === 'failed' ? toMirriErrorPayload(result.error) : undefined;
  ...
  private async executeStepTools(
    ...
    const providerFinishReason = response.providerFinishReason ?? 'completed';
    if (response.message.toolCalls.length === 0) {
      return providerFinishReason === 'tool_calls' ? 'other' : providerFinishReason;
    }
    ...
    // Running them would be unsafe: truncated arguments could cause
    // side-effects. Record each call with sanitized arguments (unparseable
    // JSON becomes {}) and immediately close it with a synthetic error
    // result, so the exchange stays wire-valid and the model learns the
    // calls never ran. Mirrors v1's recordUnexecutedToolCalls.
    if (isStreamBreakFinishReason(providerFinishReason)) {
      this.recordUnexecutedToolCalls(turnId, currentStep, stepUuid, response);
    ...
    const finishReason = stopTurn ? 'completed' : 'tool_calls';
    return finishReason;
  }

  /**
   * Record tool-call declarations that the step never executed: the provider
   * stream broke off (paused / overloaded / token limit), so running the
   * calls — whose arguments may be truncated mid-stream — would be unsafe.
   * Each call is recorded with sanitized arguments (unparseable JSON, e.g.
   * truncated by an interrupted stream, becomes `{}`) and immediately closed
   * with a synthetic error result, so the exchange stays wire-valid and the
   * model learns the calls never ran.
   */
  private recordUnexecutedToolCalls(
    turnId: number,
    currentStep: number,
    stepUuid: string,
    response: AgentLLMRequestFinish,
...
/**
 * Finish reasons that indicate the provider stream broke off mid-response.
 * When tool calls are present alongside these reasons, the calls must not be
 * executed — their arguments may be truncated — and should be recorded as
 * unexecuted instead.
 */
function isStreamBreakFinishReason(reason: FinishReason): boolean {
  return reason === 'paused' || reason === 'other' || reason === 'truncated';
}
...
/**
 * Parse tool-call arguments from the raw JSON string the provider sends.
 * Unparseable JSON (e.g. truncated by an interrupted stream) becomes `{}`,
 * matching v1's `parseToolCallArguments` behaviour.
 */
function parseToolCallArguments(raw: string | null): unknown {
