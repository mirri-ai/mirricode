...
import { ICapabilityResolverService } from '#/agent/capability/capabilityResolver';
import {
  ActiveToolsModel,
  configUpdate,
  profileBind,
...
export class AgentProfileService extends Disposable implements IAgentProfileService {
  ...
  constructor(
    ...
    @ICapabilityResolverService private readonly capabilityResolver: ICapabilityResolverService,
  ) {
    super();
  ...
  async bind(input: BindAgentInput): Promise<void> {
    ...
    const alias = input.model ?? profile.defaultModel ?? this.config.get<string>('defaultModel');
    ...
    this.activeToolNamesOverlay = undefined;
    const resolvedToolNames = this.capabilityResolver.augmentToolNames(
      profile.tools,
    ...
    this.wire.dispatch(profileBind({
      ...
      systemPrompt,
      activeToolNames: resolvedToolNames,
  ...
  useProfile(profile: ResolvedAgentProfile, context: SystemPromptContext): void {
    ...
    this.setActiveTools(resolvedToolNames);
  }
  ...
  data(): ProfileData {
    ...
    return {
      ...
      capabilitiesRequired: this.activeProfile?.capabilitiesRequired !== undefined
        ? [...this.activeProfile.capabilitiesRequired]
        : undefined,
    };
  }
  ...
  /** Collect all currently known tool names (contributions + runtime registry). */
  private availableToolNames(): ReadonlySet<string> {
    const names = new Set<string>();
    for (const contribution of getAgentToolContributions()) names.add(contribution.options.name);
    for (const ref of this.toolRegistry.listReferences()) names.add(ref.name);
    return names;
  }
}
