...
export const imageSourceSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    data: z.string().min(1),
    // Persisted filesystem path of the original (pre-compression) bytes.
    // Set by the ingestion layer when it persists the original so downstream
    // capability adaptation can hand the path to a sub-agent.
    id: z.string().min(1).optional(),
  }),
