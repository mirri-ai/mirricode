  ...
  activate(): Promise<void> {
    ...
    this.instantiationService.invokeFunction((accessor) => {
      for (const { id, options } of getAgentToolContributions()) {
        ...
        // Register the tool's declared capabilities into the capability resolver
        // so that profiles declaring capabilitiesRequired can discover this tool.
        if (options.capabilities !== undefined && options.capabilities.length > 0) {
          this.capabilityResolver.registerBuiltinToolCapabilities(options.name, options.capabilities);
        }
      }
    });
