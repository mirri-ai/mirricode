...
interface ManagedTask {
  ...
  /**
   * Set by `dispose()` when force-killing a *detached* task at session close.
   * The settle path reads this to persist `lost` (orphaned by session close)
   * instead of `killed` (user-stopped), so a resumed session sees the task as
   * "was running, no live process" — matching `markLoadedTasksLost`'s
   * projection rather than fighting it with a late `killed` write.
   */
  sessionClosedOrphan: boolean;
}
...
export class AgentTaskService extends Disposable implements IAgentTaskService {
  ...
  override dispose(): void {
    if (!this.keepAliveOnExit()) {
      for (const entry of this.tasks.values()) {
        ...
        // A detached task killed at session close is an *orphan*, not a
        // user-stopped task: its status on disk must be `lost` so a resumed
        // session (or a concurrent engine reading the same dir) sees the
        // task as "was running, no live process" rather than "user killed
        // it". Mark the entry so the settle path writes `lost`.
        if (this.isDetached(entry)) {
          entry.sessionClosedOrphan = true;
        }
  ...
  private async settleTask(
    ...
    // A detached task killed at session close is an *orphan*: persist `lost`
    // (not `killed`) so a resumed session's `markLoadedTasksLost` projection
    // wins rather than fighting a late `killed` write from this settle.
    if (entry.sessionClosedOrphan && settlement.status === 'killed') {
      entry.status = 'lost';
    } else {
