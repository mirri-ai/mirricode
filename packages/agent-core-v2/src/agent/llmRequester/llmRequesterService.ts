...
import { IMessagePipelineService, type RequestProjection } from '#/agent/messagePipeline/messagePipeline';
...
export class AgentLLMRequesterService implements IAgentLLMRequesterService {
  ...
  constructor(
    ...
    @IMessagePipelineService private readonly messagePipeline: IMessagePipelineService,
  ...
  private async runRequest(
    ...
    const run = async (projection: RequestProjection): Promise<AgentLLMRequestFinish> => {
      ...
      // Run the message pipeline: video reference resolution followed by
      // model-capability media adaptation. Each step is a registered
      // `MessageProjectionStep`; the pipeline executes them in order and
      // returns the final transformed array. See `messagePipelineService.ts`
      // and `messagePipelineSteps.ts` for the builtin steps.
      const adaptedMessages = await this.messagePipeline.run(
        projected.messages,
        request.model,
        request.requester,
        projection,
      ...
      const adaptedInput = { ...projected, messages: adaptedMessages };
      const fields =
        projection === 'normal'
          ? request.logFields
      ...
      const logInput: LLMRequestLogInput = {
        ...
        systemPrompt: adaptedInput.systemPrompt,
        tools: adaptedInput.tools,
        messages: adaptedInput.messages,
        fields,
      };
      ...
      for await (const event of request.requester.request(adaptedInput, signal, {
        ...request.params,
