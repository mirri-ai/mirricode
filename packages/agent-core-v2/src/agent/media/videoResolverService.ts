...
import { type MirriFileRef, isMirriFileUrl, parseMirriFileUrl } from './mirriFileUrl';
...
export class AgentVideoResolverService implements IAgentVideoResolverService {
  ...
  async resolve(
    ...
    if (!messages.some(hasMirriFileVideoPart)) return messages;

    let changed = false;
    const out: Message[] = [];
    for (const message of messages) {
      if (!hasMirriFileVideoPart(message)) {
        out.push(message);
        continue;
      }
      ...
      for (const part of message.content) {
        const ref =
          part.type === 'video_url' ? parseMirriFileUrl(part.videoUrl.url) : undefined;
...
function hasMirriFileVideoPart(message: Message): boolean {
  return message.content.some(
    (part) => part.type === 'video_url' && isMirriFileUrl(part.videoUrl.url),
  );
}

function tag(ref: MirriFileRef): ContentPart {
