...
- Project-local: `<cwd>/.mirri-code/mcp.json`. Use for Kimi-specific
...
or the `MIRRI_MCP_STARTUP_TIMEOUT_MS` / `MIRRI_MCP_TOOL_TIMEOUT_MS` env vars;
...
3. **Write and tell them how to reload MCP servers.** Preserve unrelated
   ...
   restart `mirri-code` for the change to take effect.
