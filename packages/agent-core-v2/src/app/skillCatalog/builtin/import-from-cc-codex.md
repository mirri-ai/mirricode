...
- Only write under Kimi Code targets:
  ...
  - Project-local MCP: `<cwd>/.mirri-code/mcp.json`, because Kimi reads the
...
- User-level source -> user-global MCP target (`$MIRRICODE_HOME/mcp.json` or
  `~/.mirri-code/mcp.json`).
- Project-level source -> project-local Kimi MCP target (`<cwd>/.mirri-code/mcp.json`). If `<cwd>` is not the project root, call this out in the preview so the user understands when Kimi will load it.
