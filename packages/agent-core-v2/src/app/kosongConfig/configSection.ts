 ...
 * constant and its zod schema, plus the env bindings / write-path strips and
 * the snake_case ↔ camelCase TOML transforms. Where kosong owns a pure type
 * (`providers` / `models` / `thinking`), the schema is re-derived from it and
 * pinned by an `AssertExact` assertion (schema ≡ type at compile time);
 * `modelCatalog` has no kosong-side type — its schema derives locally.
...
export const providersEnvBindings = envBindings(ProvidersSectionSchema, {
  [ENV_MODEL_PROVIDER_KEY]: envBindings(ProviderConfigSchema, {
    apiKey: 'MIRRICODE_MODEL_API_KEY',
    type: 'MIRRICODE_MODEL_PROVIDER_TYPE',
    baseUrl: 'MIRRICODE_MODEL_BASE_URL',
  }),
});
...
export const thinkingEnvBindings = envBindings(ThinkingConfigSchema, {
  forcedEffort: 'MIRRICODE_MODEL_THINKING_EFFORT',
});
