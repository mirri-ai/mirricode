/**
 * `kosongConfig` domain — `MIRRICODE_MODEL_*` effective-config overlay.
 *
 * When `MIRRICODE_MODEL_NAME` is set, synthesizes one model id (bound to the
 * reserved `__mirri_env__` provider whose schema kosong owns) from the
 * `MIRRICODE_MODEL_*` environment variables and overlays it onto the resolved
 ...
 * provider-definition registry, not from a hardcoded vendor table — for the
 * default 'kimi' provider type that is the `MIRRI_BASE_URL` →
 * `https://api.moonshot.ai/v1` chain declared by the vendor's traits.
 */
...
export const mirriModelEnvOverlay: ConfigEffectiveOverlay = {
  apply(effective, getEnv, validate) {
    const model = trimmed(getEnv('MIRRICODE_MODEL_NAME'));
    const temperature = parseFloatEnv(
      getEnv('MIRRICODE_MODEL_TEMPERATURE'),
      'MIRRICODE_MODEL_TEMPERATURE',
    );
    const topP = parseFloatEnv(getEnv('MIRRICODE_MODEL_TOP_P'), 'MIRRICODE_MODEL_TOP_P');
    const thinkingKeep = trimmed(getEnv('MIRRICODE_MODEL_THINKING_KEEP'));
    const maxCompletionTokens =
      parseCompletionTokens(getEnv('MIRRICODE_MODEL_MAX_COMPLETION_TOKENS')) ??
      parseCompletionTokens(getEnv('MIRRICODE_MODEL_MAX_TOKENS'));
    ...
    const maxContextRaw = trimmed(getEnv('MIRRICODE_MODEL_MAX_CONTEXT_SIZE'));
    const maxContextSize =
      maxContextRaw === undefined
        ? DEFAULT_MAX_CONTEXT_SIZE
        : parsePositiveInt(maxContextRaw, 'MIRRICODE_MODEL_MAX_CONTEXT_SIZE');

    const maxOutputRaw = trimmed(getEnv('MIRRICODE_MODEL_MAX_OUTPUT_SIZE'));
    const maxOutputSize =
      maxOutputRaw === undefined
        ? undefined
        : parsePositiveInt(maxOutputRaw, 'MIRRICODE_MODEL_MAX_OUTPUT_SIZE');
    const capabilities = parseCapabilities(getEnv('MIRRICODE_MODEL_CAPABILITIES')) ?? DEFAULT_CAPABILITIES;
    const displayName = trimmed(getEnv('MIRRICODE_MODEL_DISPLAY_NAME'));
    const reasoningKey = trimmed(getEnv('MIRRICODE_MODEL_REASONING_KEY'));
    const adaptiveThinking = parseBooleanVar(
      getEnv('MIRRICODE_MODEL_ADAPTIVE_THINKING'),
    ...
    // Default to 'kimi' provider type — this is a kosong provider registry ID
    // (like 'anthropic' or 'openai'), not a product name.
    const providerType =
    ...
    if (envProvider['type'] === undefined) providerPatch['type'] = 'kimi'; // kosong provider type ID
...
registerConfigOverlay(mirriModelEnvOverlay);
