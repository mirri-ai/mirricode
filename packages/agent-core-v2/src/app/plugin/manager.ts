    ...
    try {
      ...
      if (managedCopy !== undefined) {
        try {
          ...
          // eslint-disable-next-line preserve-caught-error -- AggregateError carries cause in the 3rd arg
    ...
    for (const record of this.records.values()) {
      ...
      for (const hook of record.manifest.hooks ?? []) {
        out.push({
          ...
          cwd: record.root,
          env: {
            MIRRICODE_HOME: this.kimiHomeDir,
            MIRRI_PLUGIN_ROOT: record.root,
          },
        });
      }
    }
...
const MIRRI_NODE_FALLBACK_SUBCOMMAND = '__plugin_run_node';
...
function withPluginMcpRuntime(
  ...
  if (config.command === 'node' && isMirriNativeBinary()) {
    return {
      ...config,
      command: process.execPath,
      args: [MIRRI_NODE_FALLBACK_SUBCOMMAND, ...(config.args ?? [])],
      cwd: config.cwd ?? pluginRoot,
      env,
    };
  }
