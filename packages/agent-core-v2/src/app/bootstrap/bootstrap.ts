...
export interface HostArgs {
   ...
   * `X-Msh-*`, built by the host through `createMirriDefaultHeaders`).
...
export interface IBootstrapOptions {
  ...
  readonly clientIdentity: MirriHostIdentity;
...
export function resolveBootstrapOptions(input: BootstrapInput): IBootstrapOptions {
  ...
  const homeDir = resolveMirriHome(input.homeDir, env, osHomeDir);
...
export function resolveMirriHome(
  homeDir?: string,
  env: NodeJS.ProcessEnv = process.env,
  osHomeDir: string = homedir(),
): string {
  return homeDir ?? env['MIRRICODE_HOME'] ?? join(osHomeDir, '.mirri-code');
}
