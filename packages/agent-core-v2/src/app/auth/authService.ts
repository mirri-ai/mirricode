 ...
 * (provided by `OAuthToolkitService` over `@mirri-ai/v2-oauth`,
...
export class OAuthService extends Disposable implements IOAuthService {
  ...
  async startLogin(provider = MIRRICODE_PROVIDER_NAME): Promise<OAuthFlowStart> {
  ...
  getFlow(provider = MIRRICODE_PROVIDER_NAME): OAuthFlowSnapshot | undefined {
  ...
  cancelLogin(provider = MIRRICODE_PROVIDER_NAME): Promise<OAuthLoginCancelResponse> {
    const state = this.flows.get(provider);
  ...
  async logout(provider = MIRRICODE_PROVIDER_NAME): Promise<OAuthLogoutResponse> {
    const oauthRef =
  ...
  async status(provider = MIRRICODE_PROVIDER_NAME): Promise<AuthStatus> {
  ...
  getManagedUsage(provider = MIRRICODE_PROVIDER_NAME): Promise<AuthManagedUsageResult> {
  ...
  getManagedUserInfo(provider = MIRRICODE_PROVIDER_NAME): Promise<AuthManagedUserInfoResult> {
    const configured = this.providerService.get(provider);
    const auth = resolveMirriCodeRuntimeAuth({
      configuredBaseUrl: configured?.baseUrl,
      configuredOAuthRef: configured?.oauth,
  ...
  private async doRefreshOAuthProviderModels(): Promise<RefreshOAuthProviderModelsResponse> {
    ...
    const provider = current.providers[MIRRICODE_PROVIDER_NAME];
    ...
    try {
      ...
      const tokenProvider = this.resolveTokenProvider(MIRRICODE_PROVIDER_NAME, auth.oauthRef);
      ...
      restoreProviderAliases(
        next,
        preserveUserProviderAliases(current, MIRRICODE_PROVIDER_NAME, refreshedAliasKeys),
      );
      restoreDefaultSelection(next, current.defaultModel, current.thinking?.enabled);
      clampDanglingDefault(next);

      if (providerModelsEqual(current, next, MIRRICODE_PROVIDER_NAME, refreshedAliasKeys)) {
        unchanged.push(MIRRICODE_PROVIDER_NAME);
      } else {
        ...
        changed.push({
          provider_id: MIRRICODE_PROVIDER_NAME,
  ...
  private readUserConfigShape(): ManagedMirriConfigShape {
    const providers =
    ...
    const services =
      this.config.inspect<ManagedMirriConfigShape['services']>(SERVICES_SECTION).userValue;
    const defaultModel = this.config.inspect<string>(DEFAULT_MODEL_SECTION).userValue;
    const thinking =
      this.config.inspect<ManagedMirriConfigShape['thinking']>(THINKING_SECTION).userValue;
    return {
      providers: { ...providers } as ManagedMirriConfigShape['providers'],
      models: { ...models } as ManagedMirriConfigShape['models'],
  ...
  private resolveLoginAuth(provider: string): {
    ...
    if (provider !== MIRRICODE_PROVIDER_NAME) {
  ...
  private resolveRuntimeOAuthRef(provider: string, oauthRef?: OAuthRef): OAuthRef | undefined {
    if (provider !== MIRRICODE_PROVIDER_NAME) return oauthRef;
    const config = this.providerService.get(provider);
  ...
  private async finalizeAuthentication(state: FlowState): Promise<void> {
    try {
      ...
      if (state.status !== 'pending') return;
      if (state.provider === MIRRICODE_PROVIDER_NAME) {
  ...
  private async provisionProvider(
    ...
    if (oauthRef === undefined && provider !== MIRRICODE_PROVIDER_NAME) return;
    const baseUrl =
  ...
  private async deprovisionProvider(provider: string): Promise<void> {
    if (provider !== MIRRICODE_PROVIDER_NAME) return;
    const next = structuredClone(this.readUserConfigShape());
    const cleanup = clearManagedMirriCodeConfig(next);
    if (
...
function restoreProviderAliases(
  ...
  config.models = {
    ...config.models,
    ...aliases,
  } as ManagedMirriConfigShape['models'];
}
...
class OAuthToolkitService extends MirriOAuthToolkit implements IOAuthToolkit {
  declare readonly _serviceBrand: undefined;
