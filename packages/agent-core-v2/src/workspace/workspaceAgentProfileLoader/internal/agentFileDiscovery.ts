...
/**
 * Accepted agent-file extensions. `.md` (YAML frontmatter + Markdown body) is
 * the native v2 format; `.yml`/`.yaml` are supported for v1 compatibility —
 * v1 stored profiles as pure YAML files, and users migrating from v1 expect
 * these to be discovered. YAML-only files use the same frontmatter schema
 * (the body is the system prompt, same as `.md`).
 */
const AGENT_FILE_EXTENSIONS = ['.md', '.yml', '.yaml'] as const;
  ...
  async function walk(dirPath: string, root: AgentFileRoot, depth: number): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (!isAgentFileName(entry) || !(await isFilePath(fs, entryPath))) continue;
...
/**
 * Check whether two file paths reside in the same directory. Used to detect
 * same-name agent files with different extensions (e.g. `reviewer.md` and
 * `reviewer.yaml`) so the discovery can warn about redundant duplicates.
 */
