 ...
 *
 * Builds the `LoggingConfig` from `MIRRI_LOG_*` environment variables plus
...
export function resolveGlobalLogPath(homeDir: string): string {
  return join(homeDir, 'logs', 'mirri-code.log');
}

export function resolveSessionLogPath(sessionDir: string): string {
  return join(sessionDir, 'logs', 'mirri-code.log');
}

export function resolveLoggingConfig(input: ResolveLoggingInput): LoggingConfig {
  const env = input.env;
  return {
    level: parseLevel(env['MIRRI_LOG_LEVEL']) ?? DEFAULT_LOG_LEVEL,
    globalLogPath: resolveGlobalLogPath(input.homeDir),
    globalMaxBytes: parsePositiveInt(env['MIRRI_LOG_GLOBAL_MAX_BYTES']) ?? DEFAULT_GLOBAL_MAX_BYTES,
    globalFiles: parsePositiveInt(env['MIRRI_LOG_GLOBAL_FILES']) ?? DEFAULT_GLOBAL_FILES,
    sessionMaxBytes:
      parsePositiveInt(env['MIRRI_LOG_SESSION_MAX_BYTES']) ?? DEFAULT_SESSION_MAX_BYTES,
    sessionFiles: parsePositiveInt(env['MIRRI_LOG_SESSION_FILES']) ?? DEFAULT_SESSION_FILES,
  };
}
