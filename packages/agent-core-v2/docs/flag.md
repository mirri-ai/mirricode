...
- `src/flag/flagService.ts` — `FlagService` impl + `MASTER_ENV` (`MIRRICODE_EXPERIMENTAL_FLAG`) + `EXPERIMENTAL_SECTION` (`experimental`); reads definitions from `IFlagRegistry`; self-registers at App scope.
...
1. L1 master env `MIRRICODE_EXPERIMENTAL_FLAG` truthy → every flag on.
2. L2 per-feature `def.env` (e.g. `MIRRICODE_EXPERIMENTAL_MY_FEATURE`) → forces on/off.
...
export const myFeatureFlag: FlagDefinitionInput = {
  ...
  title: 'My feature',
  description: '...',
  env: 'MIRRICODE_EXPERIMENTAL_MY_FEATURE',
  default: false,
  surface: 'both',
};
...
- `env` must start with `MIRRICODE_EXPERIMENTAL_`, be unique, and not equal `MIRRICODE_EXPERIMENTAL_FLAG`.
