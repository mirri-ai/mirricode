...
const V1 = ['@mirri-ai', 'agent-core'].join('/');

describe('check-import-boundaries', () => {
  it('flags a direct import of v1 (@mirri-ai/agent-core)', () => {
    const violations = checkSource(
      `import { MirriCore } from '${V1}';`,
      at('loop', 'loop.ts'),
    );
  ...
  it('allows sibling-package imports outside kosong', () => {
    const violations = checkSource(
      `import { something } from '@mirri-ai/kaos';`,
      at('log', 'log.ts'),
    );
    expect(violations).toHaveLength(0);
  });
  ...
  it('resolves the package self-reference as an intra-v2 import', () => {
    const violations = checkSource(
      `import { Foo } from '@mirri-ai/agent-core-v2/kosong/provider/provider';`,
      atKosong('protocol', 'protocol.ts'),
    );
    expect(violations).toHaveLength(1);
    expect(violations[0]?.message).toMatch(/kosong layer violation/);
