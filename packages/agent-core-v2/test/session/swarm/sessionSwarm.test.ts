...
describe('resolveSwarmMaxConcurrency', () => {
  ...
  it('returns undefined for empty or whitespace-only values', () => {
    expect(
      resolveSwarmMaxConcurrency({ MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY: '' }),
    ).toBeUndefined();
    expect(
      resolveSwarmMaxConcurrency({ MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY: '   ' }),
    ).toBeUndefined();
  ...
  it('throws for non-positive, non-integer, or non-numeric values', () => {
    for (const raw of ['0', '-1', '2.5', 'abc']) {
      expect(() =>
        resolveSwarmMaxConcurrency({ MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY: raw }),
      ).toThrow(/MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY.*positive integer/);
    }
  });

  it('returns the integer for a positive integer value', () => {
    expect(resolveSwarmMaxConcurrency({ MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY: '3' })).toBe(3);
    expect(resolveSwarmMaxConcurrency({ MIRRICODE_AGENT_SWARM_MAX_CONCURRENCY: ' 8 ' })).toBe(8);
  });
});
    ...
    createAgent.mockImplementationOnce((opts: CreateAgentOptions = {}) => {
      ...
      const handle = agentHandle(
        ...
        eventBus,
        {
          ...
          modelAlias: opts.binding?.model ?? 'mirri-test',
