...
import { Event, type Event as MirriEvent } from '#/_base/event';
...
import { DEFAULT_AGENT_PROFILE_NAME, type AgentProfile } from '#/app/agentProfileCatalog/agentProfileCatalog';
...
function modelCatalogResolving(...aliases: readonly string[]): IModelCatalog {
  return {
    ...
    getById: (alias: string) => (aliases.includes(alias) ? ({ id: alias } as Model) : undefined),
...
function createAgentLifecycleStub(options: AgentLifecycleStubOptions = {}): AgentLifecycleStub {
  ...
  lifecycle = {
    ...
    onDidStopAgentTask: Event.None as MirriEvent<AgentTaskStopHookContext>,
    onDidCreate: Event.None as MirriEvent<IAgentScopeHandle>,
    onDidDispose: Event.None as MirriEvent<string>,
...
describe('SubagentToolInputSchema', () => {
  ...
  it('exposes the model parameter as a free-form string in the JSON schema', () => {
    const properties = agentSchemaProperties<{ type?: string; description?: string }>();

    expect(properties['model']).toBeDefined();
    expect(properties['model']?.type).toBe('string');
    expect(properties['model']?.description).toContain('Model alias to override the subagent default');
  });
...
describe('Agent tool description', () => {
  ...
  it('exposes user-defined subagent profiles alongside builtins when the default profile has no allowlist', () => {
    // Given the default "agent" profile declares no `subagents` allowlist,
    // every profile in the catalog (including user-defined ones like
    // "media-reader") must be visible in the Agent tool description.
    // This is the core of custom-subagent discoverability: a profile the
    // user dropped into ~/.mirri-code/agents/ must show up without code changes.
    const builtinAgent: AgentProfile = {
      name: DEFAULT_AGENT_PROFILE_NAME,
      description: 'Default Mirri Code agent',
      // No `subagents` field — allowlist is undefined, so all profiles are visible.
      systemPrompt: () => 'agent',
    };
    const coder: AgentProfile = {
      name: 'coder',
    ...
    const mediaReader: AgentProfile = {
      name: 'media-reader',
      description: 'Read media library, can read image files',
      defaultModel: 'media',
      capabilitiesRequired: ['media.image', 'media.video'],
      systemPrompt: () => 'media-reader',
    };
    const profiles = [builtinAgent, coder, mediaReader];
    const catalog: ISessionAgentProfileCatalog = {
      _serviceBrand: undefined,
      ready: Promise.resolve(),
      onDidChange: Event.None as ISessionAgentProfileCatalog['onDidChange'],
      get: (name) => profiles.find((profile) => profile.name === name),
      getDefault: () => builtinAgent,
      list: () => profiles,
      inspect: () => undefined,
      load: async () => {},
      reload: async () => {},
    ...
    expect(description).toContain('- media-reader: Read media library, can read image files');
  });

  it('lists curated model choices when the models config carries descriptions', () => {
    ctx = createTestAgent(
      configServices(() => ({
        ...MOCK_MODEL_CONFIG,
        models: {
    ...
    expect(agentDescription()).toContain(
      '- mock-model — Mock model for tests, 977k context',
    );
  });

  it('omits the models section when no curated models are configured', () => {
    ctx = createTestAgent();
  ...
  it('always advertises the model parameter as a free-form string', () => {
    ctx = createTestAgent();

    const properties = agentParameters()['properties'] as Record<string, { type?: string; enum?: string[] }>;
...
describe('Agent tool execution contract', () => {
  ...
  it('resolves the caller binding by default when the tool call omits model', async () => {
  ...
  it('resolves an explicit model alias through the catalog', async () => {
    const lifecycle = createAgentLifecycleStub({ createAgentIds: ['agent-child'] });
    const context = createAgentToolContext(lifecycle);
  ...
  it('surfaces an invalid model alias as an error to the LLM', async () => {
    const lifecycle = createAgentLifecycleStub();
    const context = createAgentToolContext(lifecycle);
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('Model "provider/missing" is not configured');
    expect(lifecycle.create).not.toHaveBeenCalled();
  });
...
describe('AgentSwarmToolInputSchema', () => {
  ...
  it('exposes subagent_type, resume_agent_ids, and model parameters', () => {
    ...
    expect(properties['model']?.description).toContain(
      'Model alias applied to all item-spawned subagents',
    );
...
describe('AgentSwarm tool description', () => {
  ...
  it('lists curated models when the models config carries descriptions', () => {
    ctx = createTestAgent(
      configServices(() => ({
        ...MOCK_MODEL_CONFIG,
        models: {
  ...
  it('always advertises the model parameter as a free-form string', () => {
    ...
    const properties = agentSwarmParameters()['properties'] as Record<string, { type?: string; enum?: string[] }>;
...
describe('AgentSwarm tool execution contract', () => {
  ...
  it('threads an explicit model alias through spawn task bindings', async () => {
    const runSwarm = vi.fn(
      async (
  ...
  it('surfaces an invalid model alias as an error to the swarm caller', async () => {
    const runSwarm = vi.fn(
      async (
        args: SessionSwarmRunArgs,
      ): Promise<readonly SessionSwarmRunResult[]> => {
...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, CreateGoal, CronCreate, CronDelete, CronList, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Lookup, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Write
        messages:
    ...
    `);
      ...
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, CreateGoal, CronCreate, CronDelete, CronList, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Write
        messages:
          <last>
