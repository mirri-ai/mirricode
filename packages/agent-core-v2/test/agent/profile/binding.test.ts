...
describe('AgentProfileService.bind', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-bind-home-'));
  });
  ...
  it('binds a profile + model atomically and becomes runnable', async () => {
    ...
    expect(svc.getSystemPrompt()).toContain('Mirri Code CLI');
  });
  ...
  it('persists the complete binding in one journal record', async () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      thinkingEffort: 'on',
      systemPrompt: expect.stringContaining('Mirri Code CLI'),
  ...
  it('refreshes the system prompt from the session cwd after a default bind', async () => {
    const workDir = await mkdtemp(join(tmpdir(), 'mirri-bind-work-'));
    try {
  ...
  it('rejects an unsupported thinking effort atomically before first bind', async () => {
    ...
    expect(svc.data().profileName).toBeUndefined();
    await svc.bind({ profile: DEFAULT_AGENT_PROFILE_NAME, model: 'mirri-code/kimi-for-coding' });
    expect(svc.data().profileName).toBe(DEFAULT_AGENT_PROFILE_NAME);
  });
...
describe('AgentToolPolicyService tool denylist', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-deny-home-'));
  });
...
describe('AgentToolPolicyService global [tools] config', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-tools-config-home-'));
  });
...
describe('AgentToolPolicyService.setSessionDisabledTools', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-session-deny-home-'));
  });
...
describe('AgentToolPolicyService executor enforcement', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-executor-policy-home-'));
  });
...
describe('AgentProfileService tool-pattern warnings', () => {
  ...
  beforeEach(async () => {
    homeDir = await mkdtemp(join(tmpdir(), 'mirri-tool-pattern-home-'));
  });
