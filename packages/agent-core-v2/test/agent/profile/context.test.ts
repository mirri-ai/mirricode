...
beforeEach(async () => {
  homeDir = await mkdtemp(join(tmpdir(), 'mirri-agents-home-'));
  workDir = await mkdtemp(join(tmpdir(), 'mirri-agents-work-'));
  extraDirs = [];
  fs = createFs();
});
...
describe('loadAgentsMd user-level discovery', () => {
  it('loads user-level branded and generic files before project-level', async () => {
    ...
    await writeFile(join(homeDir, '.mirri-code', 'AGENTS.md'), 'user branded', 'utf-8');
  ...
  it('does not load the same file twice when the work dir is the home dir', async () => {
    await mkdir(join(homeDir, '.mirri-code'), { recursive: true });
    await writeFile(join(homeDir, '.mirri-code', 'AGENTS.md'), 'home branded', 'utf-8');
...
describe('loadAgentsMd unreadable paths', () => {
  it('warns when an instruction file exists but is a dangling symlink', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'mirri-agents-brand-'));
...
describe('loadAgentsMd brand home (MIRRICODE_HOME)', () => {
  let brandHome: string;

  beforeEach(async () => {
    brandHome = await mkdtemp(join(tmpdir(), 'mirri-agents-brand-'));
  });
  ...
  it('ignores the real-home .mirri-code/AGENTS.md when the brand home is elsewhere', async () => {
    await writeFile(join(brandHome, 'AGENTS.md'), 'brand wins', 'utf-8');
    await mkdir(join(homeDir, '.mirri-code'), { recursive: true });
    await writeFile(join(homeDir, '.mirri-code', 'AGENTS.md'), 'stale real-home brand', 'utf-8');
  ...
  it('falls back to the real-home .mirri-code/AGENTS.md when no brand home is given', async () => {
    await mkdir(join(homeDir, '.mirri-code'), { recursive: true });
    await writeFile(join(homeDir, '.mirri-code', 'AGENTS.md'), 'fallback branded', 'utf-8');
...
describe('loadAgentsMd nested project hierarchy', () => {
  it('loads AGENTS.md from the project root down to the cwd in root→leaf order', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'mirri-agents-project-'));
...
describe('prepareSystemPromptContext additional directories', () => {
  it('includes additional directory listings without loading their AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'mirri-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDir = await mkdtemp(join(tmpdir(), 'mirri-agents-extra-'));
  ...
  it('loads user-level AGENTS.md once and skips additional directory AGENTS.md', async () => {
    const brandHome = await mkdtemp(join(tmpdir(), 'mirri-agents-empty-brand-'));
    extraDirs.push(brandHome);
    const extraDirA = await mkdtemp(join(tmpdir(), 'mirri-agents-extra-a-'));
    const extraDirB = await mkdtemp(join(tmpdir(), 'mirri-agents-extra-b-'));
