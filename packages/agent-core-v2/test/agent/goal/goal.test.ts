 ...
 * Run: `pnpm --filter @mirri-ai/agent-core-v2 exec vitest run test/agent/goal/goal.test.ts`.
 */
...
import { ErrorCodes, Error2, errorInfo, toMirriErrorPayload } from '#/errors';
...
function endTurn(
  ...
  const error = result.error !== undefined ? toMirriErrorPayload(result.error) : undefined;
  eventBus.publish({
    type: 'turn.ended',
    turnId: turn.id,
