...
describe('task tool descriptions', () => {
  ...
  it('matches the v1 model-facing contract exactly', () => {
    ...
    // TaskOutputTool intentionally diverged from v1 in B1: it is now always
    // non-blocking (no block/timeout parameters) and mentions AskUserQuestion.
    // Verify name and description parity only; parameters are checked separately.
    const taskOutput = new TaskOutputTool(tasks);
    const v1TaskOutput = new V1TaskOutputTool({} as never);
    expect(taskOutput.name).toBe(v1TaskOutput.name);
    expect(taskOutput.description).toBe(v1TaskOutput.description);
