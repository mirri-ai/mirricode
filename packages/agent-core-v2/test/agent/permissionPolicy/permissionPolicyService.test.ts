...
describe('AgentPermissionPolicyService chain', () => {
  ...
  it('deny-all overrides all other policies when enabled', async () => {
    mode = 'yolo';
    const svc = service();
    const policyService = svc as AgentPermissionPolicyService;

    policyService.denyAll.enable('Tool calls are disabled for side questions.');

    await expect(evaluate({
      toolName: 'Bash',
      args: { command: 'echo hello', timeout: 60 },
    })).resolves.toMatchObject({
      policyName: 'deny-all',
      result: {
        kind: 'deny',
        message: 'Tool calls are disabled for side questions.',
      },
    });

    policyService.denyAll.disable();

    // After disabling, yolo-mode-approve should take over again.
    await expect(evaluate({
      toolName: 'Bash',
      args: { command: 'echo hello', timeout: 60 },
    })).resolves.toMatchObject({
...
describe('AgentPermissionPolicyService git cwd write approval', () => {
  ...
  beforeEach(async () => {
    disposables = new DisposableStore();
    mode = 'manual';
    workspaceDir = await mkdtemp(join(tmpdir(), 'mirri-permission-git-'));
  ...
  it('approves Edit on an additionalDir path in manual mode', async () => {
    const extraDir = await mkdtemp(join(tmpdir(), 'mirri-permission-extra-'));
