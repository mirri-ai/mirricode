...
describe('runHook process runner', () => {
  ...
  it('parses additionalContext from hookSpecificOutput in structured JSON', async () => {
    const result = await runHook(
      hostProcess,
      nodeCommand(
        'process.stdout.write(JSON.stringify({ hookSpecificOutput: { additionalContext: "remember this context" } }));',
      ),
      {},
      { timeout: 5 },
    );
  ...
  it('includes additionalContext in block results from structured JSON', async () => {
    const result = await runHook(
      hostProcess,
      nodeCommand(
        'process.stdout.write(JSON.stringify({ hookSpecificOutput: { permissionDecision: "deny", permissionDecisionReason: "nope", additionalContext: "context even when blocked" } }));',
      ),
      {},
      { timeout: 5 },
    );
    ...
    expect(result.additionalContext).toBe('context even when blocked');
  });
});
