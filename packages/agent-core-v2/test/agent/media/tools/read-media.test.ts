...
describe('ReadMediaFileTool', () => {
  ...
  it('errors when reading an image without image input capability', async () => {
    ...
    expect(result.output).toContain('Dispatch a subagent');
  });

  it('errors when reading a video without video input capability, hinting to dispatch a subagent', async () => {
    const result = await execute(
      makeTool(
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('does not support video input');
    expect(result.output).toContain('Dispatch a subagent');
  });
