 ...
 * Run: `pnpm --filter @mirri-ai/agent-core-v2 exec vitest run test/agent/plan/planGuard.test.ts`.
 */
...
describe('AgentPlanService plan-guard listener', () => {
  ...
  describe('guard', () => {
    ...
    it.each(['Read', 'Glob', 'Grep', 'Bash', 'CronList'] as const)(
    ...
    it('does not block Bash in plan mode — it defers to the normal permission chain', async () => {
      await enterPlan();
      const decision = await run(hookContext('Bash', { args: { command: 'ls' } }));
