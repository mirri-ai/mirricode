...
describe('McpConnectionManager', () => {
  ...
  it('starts stdio servers in stdioCwd when config.cwd is omitted', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'mirri-mcp-manager-cwd-'));
    ...
    try {
      await cm.connectAll({
        crashy: {
          ...
          env: { MIRRI_TEST_MCP_EXIT_AFTER_MS: '500', MIRRI_TEST_MCP_STDERR: 'fatal: out of memory' },
    ...
    try {
      await cm.connectAll({
        nope: {
          ...
          env: { MIRRI_TEST_MCP_STDERR: 'fatal: missing API token MIRRI_X' },
          startupTimeoutMs: 4_000,
        },
      });
      const entry = cm.get('nope');
      expect(entry?.status).toBe('failed');
      expect(entry?.error).toContain('fatal: missing API token MIRRI_X');
    } finally {
      await cm.shutdown();
    }
  }, 10000);
