...
describe('resolveLoggingConfig', () => {
  it('uses defaults when env is empty', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/home/mirri', env: {} });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalLogPath).toBe('/home/mirri/logs/mirri-code.log');
  ...
  it('reads level and sizes from env', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        MIRRI_LOG_LEVEL: 'debug',
        MIRRI_LOG_GLOBAL_MAX_BYTES: '1024',
        MIRRI_LOG_GLOBAL_FILES: '7',
        MIRRI_LOG_SESSION_MAX_BYTES: '2048',
        MIRRI_LOG_SESSION_FILES: '4',
      },
    });
  ...
  it('ignores invalid level and non-positive sizes', () => {
    const cfg = resolveLoggingConfig({
      homeDir: '/h',
      env: {
        MIRRI_LOG_LEVEL: 'verbose',
        MIRRI_LOG_GLOBAL_MAX_BYTES: '-5',
        MIRRI_LOG_GLOBAL_FILES: 'abc',
      },
    });
    expect(cfg.level).toBe(DEFAULT_LOG_LEVEL);
    expect(cfg.globalMaxBytes).toBe(DEFAULT_GLOBAL_MAX_BYTES);
  ...
  it('resolves the log path regardless of env', () => {
    ...
    expect(cfg.globalLogPath).toBe('/h/logs/mirri-code.log');
  });
});

describe('path resolution', () => {
  it('resolves the global log path under homeDir/logs', () => {
    expect(resolveGlobalLogPath('/home/mirri')).toBe('/home/mirri/logs/mirri-code.log');
  });

  it('resolves the session log path under sessionDir/logs', () => {
    expect(resolveSessionLogPath('/sessions/s1')).toBe('/sessions/s1/logs/mirri-code.log');
  });
});

describe('logSeed', () => {
  it('seeds ILogOptions into a App scope', () => {
    const cfg = resolveLoggingConfig({ homeDir: '/h', env: { MIRRI_LOG_LEVEL: 'warn' } });
    const host = createScopedTestHost(logSeed(cfg));
    const opts = host.app.accessor.get(ILogOptions);
    expect(opts.level).toBe('warn');
    expect(opts.globalLogPath).toBe('/h/logs/mirri-code.log');
    host.dispose();
  });
});
