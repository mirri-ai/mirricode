...
export interface TestAgentOptions {
  ...
  | undefined;
  readonly initialConfig?: Partial<MirriConfig> | undefined;
...
export function configServices(readConfig: () => MirriConfig): TestAgentServiceOverride {
...
class ConfigBackedModelCatalog extends ModelCatalog {
  ...
  override getById(modelId: string): Model | undefined {
    this.syncRegistriesFromConfig();
    return super.getById(modelId);
  }
}
...
export class AgentTestContext {
  ...
  constructor(overrides: readonly TestAgentServiceOverride[] = [], options: TestAgentOptions = {}) {
    ...
    const appSeeds = collectScopeSeed(
      [
        (reg) => {
          for (const [id, value] of bootstrapSeed({
            homeDir: '/tmp/mirri-code-agent-app-v2-test',
            cwd: this.cwd,
          ...
          reg.defineInstance(
            ...
            {
              level: 'off',
              globalLogPath: '/tmp/mirri-code-agent-app-v2-test/logs/mirri-code.log',
      ...
      extra: collectScopeSeed(
        [
          (reg) => {
            ...
            reg.defineDescriptor(ICapabilityResolverService, new SyncDescriptor(CapabilityResolverService));
            reg.defineDescriptor(
              IAgentLLMRequesterService,
  ...
  async expectResumeMatches(): Promise<void> {
    ...
    await this.drainWirePersistence();
    const configSnapshot = structuredClone(this.get(IConfigService).getAll() as MirriConfig);
...
function isTaskOriginLike(value: unknown): value is {
  ...
  return origin['kind'] === 'background_task' &&
...
function applyTestAgentOptionsToConfig(config: MirriConfig, options: TestAgentOptions): MirriConfig {
...
function configWithEnvOverrides(config: MirriConfig): MirriConfig {
  const maxCompletionTokens =
    parseEnvCompletionTokens(process.env['MIRRICODE_MODEL_MAX_COMPLETION_TOKENS']) ??
    parseEnvCompletionTokens(process.env['MIRRICODE_MODEL_MAX_TOKENS']);
  const temperature = parseEnvFloat(process.env['MIRRICODE_MODEL_TEMPERATURE']);
  const topP = parseEnvFloat(process.env['MIRRICODE_MODEL_TOP_P']);
  const forcedEffort = process.env['MIRRICODE_MODEL_THINKING_EFFORT']?.trim();
  const thinkingKeep = process.env['MIRRICODE_MODEL_THINKING_KEEP']?.trim();
...
function cronEnvOverrides(base: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  setBoolean('debug', 'MIRRI_CRON_DEBUG');
  setBoolean('noJitter', 'MIRRI_CRON_NO_JITTER');
  setBoolean('noStale', 'MIRRI_CRON_NO_STALE');
  setBoolean('disabled', 'MIRRI_DISABLE_CRON');
  setBoolean('manualTick', 'MIRRI_CRON_MANUAL_TICK');
  const pollIntervalMs = parseEnvCronPollIntervalMs(process.env['MIRRI_CRON_POLL_INTERVAL_MS']);
  if (pollIntervalMs !== undefined) {
    next['pollIntervalMs'] = pollIntervalMs;
    changed = true;
  }
  if (process.env['MIRRI_CRON_CLOCK'] !== undefined) {
    next['clock'] = process.env['MIRRI_CRON_CLOCK'];
    changed = true;
  }
...
function providerConfigForAlias(provider: TestProviderConfig): MirriConfig['providers'][string] {
  return {
    type: provider.type,
