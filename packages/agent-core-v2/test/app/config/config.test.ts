 ...
 * boundaries. Run with `pnpm --filter @mirri-ai/agent-core-v2 exec vitest run
...
describe('ConfigService env overlay (live)', () => {
  it('re-applies env bindings on every get()', async () => {
    const env: Record<string, string> = { MIRRI_DISABLE_CRON: '0' };
  ...
  it('keeps the Kimi effort force separate from the configured effort', async () => {
    const env: Record<string, string> = { MIRRICODE_MODEL_THINKING_EFFORT: 'max' };
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/mirri-cfg', env));
  ...
  it('strips the Kimi effort force before persisting thinking config', async () => {
    ...
    const ix = disposables.add(new TestInstantiationService());
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/mirri-cfg'));
    ix.stub(IFileSystemStorageService, new InMemoryStorageService());
    ix.set(IAtomicTomlDocumentStore, new SyncDescriptor(TomlAtomicDocumentStore));
    ix.set(IConfigRegistry, new SyncDescriptor(ConfigRegistry));
    ix.set(IConfigService, new SyncDescriptor(ConfigService));
  ...
  it('deletes a scalar section on replace(undefined) — set(undefined) cannot', async () => {
    ...
    await config.replace('defaultModel', 'mirri-code/kimi-k2');
    expect(config.get<string>('defaultModel')).toBe('mirri-code/kimi-k2');
...
describe('services config section env bindings', () => {
  ...
  it('resolves moonshot_search / moonshot_fetch fields from MIRRI_WEB_* env vars', async () => {
...
describe('image config section', () => {
  ...
  it('re-applies image env bindings on every get() and ignores invalid env', async () => {
    ...
    env['MIRRI_IMAGE_MAX_EDGE_PX'] = 'abc';
    env['MIRRI_IMAGE_READ_BYTE_BUDGET'] = '-1';
    expect(config.get<ImageConfig>(IMAGE_SECTION)).toEqual({});

    env['MIRRI_IMAGE_MAX_EDGE_PX'] = '1500';
    env['MIRRI_IMAGE_READ_BYTE_BUDGET'] = '131072';
    expect(config.get<ImageConfig>(IMAGE_SECTION)).toEqual({
      maxEdgePx: 1500,
      readByteBudget: 131072,
    });

    env['MIRRI_IMAGE_MAX_EDGE_PX'] = '2500';
  ...
  it('restores env-owned fields to the raw value on set() while the env var is set', async () => {
    const env: Record<string, string> = { 'MIRRI_IMAGE_MAX_EDGE_PX': '1500' };
    const disposables = new DisposableStore();
    const ix = disposables.add(new TestInstantiationService());
...
describe('ConfigService thinking effort max migration', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'mirri-v2-cfg-migrate-'));
  });
...
describe('ConfigService replaceSections', () => {
  ...
  async function createSectionsConfig(toml = SEED_TOML) {
    ...
    await storage.write('', 'config.toml', new TextEncoder().encode(toml));
    ix.stub(ILogService, stubLog());
    ix.stub(IBootstrapService, stubBootstrap('/tmp/mirri-cfg-replace-sections'));
    ix.stub(IFileSystemStorageService, storage);
    ix.set(IAtomicTomlDocumentStore, new SyncDescriptor(TomlAtomicDocumentStore));
    ix.set(IConfigRegistry, new SyncDescriptor(ConfigRegistry));
    ix.set(IConfigService, new SyncDescriptor(ConfigService));
