 ...
 * Run with `pnpm --filter @mirri-ai/agent-core-v2 exec vitest run
...
async function isolatedTmpdir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), 'mirri-isolated-tmp-'));
...
async function makeMirriHome(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), 'mirri-home-'));
}
...
async function makePlugin(
  ...
  await writeFile(path.join(root, 'mirri.plugin.json'), JSON.stringify(manifest), 'utf8');
  return realpath(root);
}
...
describe('PluginManager consumption plane', () => {
  ...
  it('pluginSkillRoots() excludes plugins in error state', async () => {
    ...
    await writeFile(
      path.join(await managedPluginRoot(manager, 'demo'), 'mirri.plugin.json'),
      '{ not json',
      'utf8',
    );
    ...
    expect(manager.enabledMcpServers()).toEqual(
      expect.objectContaining({
        'plugin-demo:finance': expect.objectContaining({
          command: 'finance-mcp',
          cwd: managedRoot,
          env: expect.objectContaining({ MIRRICODE_HOME: home, MIRRI_PLUGIN_ROOT: managedRoot }),
        }),
  ...
  it('reload() picks up edits to the managed plugin copy', async () => {
    ...
    const managedRoot = await managedPluginRoot(manager, 'demo');
    await writeFile(
      path.join(managedRoot, 'mirri.plugin.json'),
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        timeout: 10,
        cwd: installedRoot,
        env: { MIRRICODE_HOME: home, MIRRI_PLUGIN_ROOT: installedRoot },
      },
    ]);
  });
  ...
  it('install() from /tree/<tag-shaped-ref> pins the resolved commit', async () => {
    ...
    await writeFile(
      path.join(sourceRoot, 'mirri.plugin.json'),
  ...
  it('install() from github URL overwrites an existing zip-url install (CDN migration)', async () => {
    ...
    await writeFile(
      path.join(cdnSource, 'mirri.plugin.json'),
    ...
    await writeFile(
      path.join(ghSource, 'mirri.plugin.json'),
