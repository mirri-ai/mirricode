 ...
 * Run: pnpm --filter @mirri-ai/agent-core-v2 exec vitest run test/app/plugin/manager.test.ts
 */
...
describe('PluginManager', () => {
    ...
    expect(manager.enabledHooks()).toEqual([
      {
        ...
        command: 'echo stop',
        cwd: root,
        env: { MIRRICODE_HOME: home, MIRRI_PLUGIN_ROOT: root },
      },
    ]);
  ...
  it('installs a local-path plugin into the managed root', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'mirri.plugin.json'), JSON.stringify({ name: 'other' }), 'utf8');
      const manager = new PluginManager({ kimiHomeDir: home });
  ...
  it('installs a zip-url plugin', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'mirri.plugin.json'), JSON.stringify({ name: 'zip-plugin' }), 'utf8');
  ...
  it('installs a github plugin through codeload', async () => {
    ...
    try {
      await writeFile(join(sourceRoot, 'mirri.plugin.json'), JSON.stringify({ name: 'github-plugin' }), 'utf8');
      execFileSync('zip', ['-qr', zipPath, '.'], { cwd: sourceRoot });
