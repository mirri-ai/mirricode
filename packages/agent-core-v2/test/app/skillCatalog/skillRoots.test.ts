...
describe('skillRoots', () => {
  ...
  describe('projectRoots', () => {
    it('resolves the brand .mirri-code/skills directory at the .git root', async () => {
      await markGitRoot();
      await mkdir(join(root, '.mirri-code/skills/commit'), { recursive: true });

      const roots = await projectRoots(root);

      expect(roots.some((r) => r.path.endsWith('.mirri-code/skills') && r.source === 'project')).toBe(
        true,
      );
    });
    ...
    it('falls back to the generic .agents/skills directory', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.mirri-code/skills'))).toBe(false);
    });
    ...
    it('walks up from a child directory to the .git root', async () => {
      ...
      expect(roots.some((r) => r.path.endsWith('.mirri-code/skills'))).toBe(true);
    });
