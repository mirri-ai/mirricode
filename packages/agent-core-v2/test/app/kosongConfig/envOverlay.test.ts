/**
 * `app/kosongConfig` envOverlay tests — the `MIRRICODE_MODEL_*` effective overlay:
 *
 *  - with `MIRRICODE_MODEL_NAME` set it synthesizes the reserved env model +
 ...
 *    reads): the Kimi chain yields `MIRRI_BASE_URL` →
...
import { ENV_MODEL_ALIAS_KEY, mirriModelEnvOverlay } from '#/app/kosongConfig/envOverlay';

type Env = Record<string, string>;

function apply(effective: Record<string, unknown>, env: Env): readonly string[] {
  return mirriModelEnvOverlay.apply(effective, (name) => env[name], (_domain, value) => value);
}

describe('mirriModelEnvOverlay.apply', () => {
  it('does nothing with no MIRRICODE_MODEL_* env', () => {
    const effective: Record<string, unknown> = {};
    expect(apply(effective, {})).toEqual([]);
    expect(effective).toEqual({});
  });

  it('applies only modelOverrides when MIRRICODE_MODEL_NAME is unset', () => {
    const effective: Record<string, unknown> = {};
    const changed = apply(effective, {
      MIRRICODE_MODEL_TEMPERATURE: '0.7',
      MIRRICODE_MODEL_TOP_P: '0.95',
      MIRRICODE_MODEL_THINKING_KEEP: 'all',
      MIRRICODE_MODEL_MAX_COMPLETION_TOKENS: '8192',
    });
  ...
  it('synthesizes the env model, selects it, and defaults the provider through the registry', () => {
    const effective: Record<string, unknown> = {};
    const changed = apply(effective, { MIRRICODE_MODEL_NAME: 'kimi-k2-custom' });
  ...
  it('honors the vendor endpoint env chain for the default baseUrl', () => {
    const effective: Record<string, unknown> = {};
    apply(effective, {
      MIRRICODE_MODEL_NAME: 'kimi-k2-custom',
      MIRRI_BASE_URL: 'https://kimi-proxy.example.test/v1',
    });
    expect((effective['providers'] as Record<string, unknown>)[ENV_MODEL_PROVIDER_KEY]).toEqual({
      type: 'kimi',
  ...
  it('keeps an existing env-provider type and baseUrl untouched', () => {
    ...
    const changed = apply(effective, { MIRRICODE_MODEL_NAME: 'my-model' });
  ...
  it('parses the optional model fields and validates their shapes', () => {
    const effective: Record<string, unknown> = {};
    apply(effective, {
      MIRRICODE_MODEL_NAME: 'my-model',
      MIRRICODE_MODEL_MAX_CONTEXT_SIZE: '131072',
      MIRRICODE_MODEL_MAX_OUTPUT_SIZE: '4096',
      MIRRICODE_MODEL_CAPABILITIES: 'image_in, tool_use',
      MIRRICODE_MODEL_DISPLAY_NAME: 'Mine',
      MIRRICODE_MODEL_REASONING_KEY: 'reasoning_content',
    ...
    expect(() => apply({}, { MIRRICODE_MODEL_NAME: 'm', MIRRICODE_MODEL_MAX_CONTEXT_SIZE: 'abc' })).toThrowError(
      /MIRRICODE_MODEL_MAX_CONTEXT_SIZE must be a positive integer/,
    );
    expect(() => apply({}, { MIRRICODE_MODEL_TEMPERATURE: 'hot' })).toThrowError(
      /MIRRICODE_MODEL_TEMPERATURE must be a number/,
    );
  });
});

describe('mirriModelEnvOverlay.strip', () => {
  it('removes the synthesized values on the write path', () => {
    const strip = mirriModelEnvOverlay.strip!;
    expect(
