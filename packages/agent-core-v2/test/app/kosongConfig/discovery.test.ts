...
async function createHost(
  ...
  const host = createScopedTestHost([
    ...
    [
      IBootstrapService,
      stubBootstrap('/tmp/mirri-home', {}, { requestHeaders: { 'User-Agent': 'mirri-test/1.0' } }),
    ],
  ]);
...
describe('refreshProviderModels write behavior', () => {
    ...
    try {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ 'User-Agent': 'mirri-test/1.0' }),
        }),
      );
    } finally {
      host.dispose();
  ...
  it('refreshes a hand-configured API-key provider at the managed endpoint', async () => {
    ...
    vi.stubEnv('MIRRICODE_BASE_URL', baseUrl);
    const fetchMock = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
