...
describe('sessionExport', () => {
  ...
  it('exports a v2 session directory with per-agent wire activity and optional global log', async () => {
    ...
    await writeFile(join(sessionDir, 'logs', 'mirri-code.log'), '{"msg":"session"}\n', 'utf-8');
    await writeFile(
    ...
    const globalLogPath = join(tmp, 'logs', 'mirri-code.log');
    ...
    expect(result.manifest).toMatchObject({
      ...
      sessionLogPath: 'logs/mirri-code.log',
      globalLogPath: 'logs/global/mirri-code.log',
    });
  });
  ...
  it('keeps the session log bound when it rotates as wire scanning starts', async () => {
    ...
    const logPath = join(sessionDir, 'logs', 'mirri-code.log');
    ...
    try {
      ...
      expect(result.manifest.sessionLogPath).toBe('logs/mirri-code.log');
      expect(result.entries).toContain('logs/mirri-code.log');
      await expect(readZipEntry(outputPath, 'logs/mirri-code.log')).resolves.toEqual(log);
    } finally {
  ...
  it('keeps the global log bound when it rotates as wire scanning starts', async () => {
    ...
    try {
      ...
      expect(rotated).toBe(true);
      expect(result.manifest.globalLogPath).toBe('logs/global/mirri-code.log');
      expect(result.entries).toContain('logs/global/mirri-code.log');
      await expect(readZipEntry(outputPath, 'logs/global/mirri-code.log')).resolves.toEqual(log);
    } finally {
      fsOpenHook.afterOpen = undefined;
    }
  });
  ...
  it('closes pre-opened logs when manifest creation fails before writer ownership', async () => {
    ...
    const sessionLogPath = join(sessionDir, 'logs', 'mirri-code.log');
    const globalLogPath = join(tmp, 'logs', 'mirri-code.log');
    const wirePath = join(sessionDir, 'agents', 'main', 'wire.jsonl');
  ...
  it('omits the optional global log when the configured file is missing', async () => {
    ...
    expect(result.entries).not.toContain('logs/global/mirri-code.log');
  });
  ...
  it('closes a pre-opened source when it conflicts with the output path', async () => {
    ...
    await expect(
      writeExportZip({
        ...
        sessionDir: tmp,
        sessionFiles: [],
        extraEntries: [{ source, target: 'logs/global/mirri-code.log' }],
      }),
    ).rejects.toMatchObject({ code: 'session.export_output_conflict' });
  ...
  it('archives a bound session log after its path is rotated', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'session-export-test-'));
    const logPath = join(tmp, 'logs', 'mirri-code.log');
    const rotatedPath = `${logPath}.1`;
    ...
    ).resolves.toContain('logs/mirri-code.log');
    await expect(readZipEntry(outputPath, 'logs/mirri-code.log')).resolves.toEqual(original);
    expect(closeCalls).toBe(1);
  });
  ...
  it('includes a bounded Web log in the exported archive', async () => {
    ...
    expect(result.entries).toContain('logs/mirri-web.jsonl');
    expect(result.manifest.webLogPath).toBe('logs/mirri-web.jsonl');
    await expect(readZipEntry(outputPath, 'logs/mirri-web.jsonl')).resolves.toEqual(
  ...
  it('includes the desktop app log when given', async () => {
    ...
    await mkdir(sessionDir, { recursive: true });
    await writeFile(join(sessionDir, 'state.json'), '{}\n', 'utf-8');
    const desktopLogPath = join(tmp, 'logs', 'mirri-code-desktop.log');
    await mkdir(join(tmp, 'logs'), { recursive: true });
    ...
    expect(result.entries).toContain('logs/mirri-desktop.log');
    expect(result.manifest.desktopLogPath).toBe('logs/mirri-desktop.log');
    await expect(readZipEntry(outputPath, 'logs/mirri-desktop.log')).resolves.toEqual(
  ...
  it('skips a missing desktop app log silently', async () => {
    ...
    expect(result.entries).not.toContain('logs/mirri-desktop.log');
