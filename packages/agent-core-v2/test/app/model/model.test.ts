 ...
 * persistence wrapper), and the `MIRRICODE_MODEL_*` env overlay.
 *
...
import { mirriModelEnvOverlay, ENV_MODEL_ALIAS_KEY } from '#/app/kosongConfig/envOverlay';
import {
...
describe('mirriModelEnvOverlay', () => {
  it('does nothing when MIRRICODE_MODEL_NAME is absent', () => {
    const effective = {
      models: {
    ...
    const result = applyMirriModelEnvOverlay({}, effective);

    expect(result.changed).toEqual([]);
    expect(result.effective).toEqual(effective);
  });

  it('applies request overrides when MIRRICODE_MODEL_NAME is absent', () => {
    const { changed, effective } = applyMirriModelEnvOverlay({
      MIRRICODE_MODEL_TEMPERATURE: '0.3',
  ...
  it('omits baseUrl for openai so the base SDK default applies at construction', () => {
    const { effective } = applyMirriModelEnvOverlay(
  ...
  it('maps extended model metadata and request overrides', () => {
    const { changed, effective } = applyMirriModelEnvOverlay({
      MIRRICODE_MODEL_NAME: 'env-model',
      MIRRICODE_MODEL_MAX_CONTEXT_SIZE: '1000000',
      MIRRICODE_MODEL_MAX_OUTPUT_SIZE: '8192',
      MIRRICODE_MODEL_CAPABILITIES: 'Image_In, thinking , tool_use',
      MIRRICODE_MODEL_DISPLAY_NAME: 'Custom Model',
      MIRRICODE_MODEL_REASONING_KEY: 'reasoning',
      MIRRICODE_MODEL_ADAPTIVE_THINKING: 'true',
      MIRRICODE_MODEL_TEMPERATURE: '0.3',
      MIRRICODE_MODEL_TOP_P: ' 0.95 ',
      MIRRICODE_MODEL_THINKING_KEEP: 'all',
      MIRRICODE_MODEL_MAX_COMPLETION_TOKENS: '4096',
      MIRRICODE_MODEL_MAX_TOKENS: '2048',
    });
  ...
  it('falls back to legacy MIRRICODE_MODEL_MAX_TOKENS for completion overrides', () => {
    const { effective } = applyMirriModelEnvOverlay({
      MIRRICODE_MODEL_NAME: 'env-model',
      MIRRICODE_MODEL_MAX_TOKENS: '2048',
    });
  ...
  it.each([
    ['MIRRICODE_MODEL_MAX_CONTEXT_SIZE', '0'],
    ['MIRRICODE_MODEL_MAX_CONTEXT_SIZE', '1.5'],
    ['MIRRICODE_MODEL_MAX_OUTPUT_SIZE', 'nope'],
    ['MIRRICODE_MODEL_ADAPTIVE_THINKING', 'maybe'],
    ['MIRRICODE_MODEL_TEMPERATURE', 'abc'],
    ['MIRRICODE_MODEL_TEMPERATURE', '1.2.3'],
    ['MIRRICODE_MODEL_TOP_P', 'NaN'],
  ])('throws config.invalid for invalid %s=%s', (key, value) => {
    expectConfigInvalid(() =>
      applyMirriModelEnvOverlay({ MIRRICODE_MODEL_NAME: 'env-model', [key]: value }),
    );
  });

  it('strips env-only model values before write-back', () => {
    expect(
      mirriModelEnvOverlay.strip?.(
        'models',
        {
    ...
    expect(
      mirriModelEnvOverlay.strip?.('defaultModel', ENV_MODEL_ALIAS_KEY, {
        default_model: 'user',
      }),
    ).toBe('user');
    expect(mirriModelEnvOverlay.strip?.('modelOverrides', { temperature: 0.3 }, {})).toBeUndefined();
  });

  it('self-registers into ConfigRegistry without ModelService instantiation', () => {
    const freshRegistry = new ConfigRegistry();
    expect(freshRegistry.listEffectiveOverlays()).toContain(mirriModelEnvOverlay);
  });
});
