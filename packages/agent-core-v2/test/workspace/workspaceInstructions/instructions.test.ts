...
describe('WorkspaceInstructionsService', () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), 'mirri-instructions-work-'));
    osHomeDir = mkdtempSync(join(tmpdir(), 'mirri-instructions-os-'));
    brandHomeDir = mkdtempSync(join(tmpdir(), 'mirri-instructions-brand-'));
