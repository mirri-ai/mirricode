 ...
 * - a persisted `addDir` writes `.mirri-code/local.toml` and refreshes every
...
describe('workspace add-dir (handler chain)', () => {
  ...
  async function makeProjectRoot(): Promise<string> {
    const root = await makeRoot('mirri-add-dir-proj-');
  ...
  it('persists addDir to local.toml and every session of the workspace sees the dir', async () => {
    const homeDir = await makeRoot('mirri-add-dir-home-');
    const root = await makeProjectRoot();
    const extra = await makeRoot('mirri-add-dir-extra-');
    ...
    expect(result.configPath).toBe(join(root, '.mirri-code', 'local.toml'));
    expect(result.additionalDirs).toEqual([extra]);
    // local.toml written on disk.
    const toml = await readFile(join(root, '.mirri-code', 'local.toml'), 'utf8');
  ...
  it('persist: false stays in handler memory, never touches the disk, and is shared', async () => {
    ...
    await expect(readFile(join(root, '.mirri-code', 'local.toml'), 'utf8')).rejects.toThrow();
  ...
  it('refreshes live session views when another process edits local.toml', async () => {
    ...
    // External write (another process, an editor, `mirri` in a second CLI).
    await mkdir(join(root, '.mirri-code'), { recursive: true });
    const writeLocalToml = () =>
      writeFile(join(root, '.mirri-code', 'local.toml'), `[workspace]\nadditional_dir = ["${extra}"]\n`);
    ...
    // Under high parallel CI load, FSEvents delivery can take much longer
    // than the debounce; allow 30s to avoid false flakes.
  ...
  it('registers the additional-directory sets into the workspace state container', async () => {
    const homeDir = await makeRoot('mirri-add-dir-home-');
    const root = await makeProjectRoot();
    const persisted = await makeRoot('mirri-add-dir-persisted-');
    const ephemeral = await makeRoot('mirri-add-dir-ephemeral-');
    const host = buildHost(homeDir);
    const handler = await host.app.accessor.get(IWorkspaceLifecycleService).handlerFor({ root });
