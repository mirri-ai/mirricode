...
describe('workspace resource sharing (handler chain)', () => {
    ...
    host = createScopedTestHost([
      ...
      stubPair(IProjectLocalConfigService, {
        ...
        readAdditionalDirs: (workDir: string) =>
          Promise.resolve({
            projectRoot: workDir,
            configPath: `${workDir}/.mirri-code/local.toml`,
            additionalDirs: [],
          }),
  ...
  it('runs one shared MCP manager and one initial connect for two concurrent sessions', async () => {
    const root = await makeRoot('mirri-ws-mcp-');
    await mkdir(join(root, '.mirri-code'), { recursive: true });
    await writeFile(
      join(root, '.mirri-code', 'mcp.json'),
      JSON.stringify({
        mcpServers: {
    ...
    const home = await makeRoot('mirri-ws-mcp-home-');
  ...
  it('does not rescan skills when a second session of the workspace lists them', async () => {
    const root = await makeRoot('mirri-ws-skill-');
    await mkdir(join(root, '.mirri-code', 'skills'), { recursive: true });
    const home = await makeRoot('mirri-ws-skill-home-');
  ...
  it('refreshes a live session skill list when .agents/skills changes on disk', async () => {
    const root = await makeRoot('mirri-ws-watch-');
    const home = await makeRoot('mirri-ws-watch-home-');
    ...
    await vi.waitFor(
      ...
      // rescan: under high parallel load the FSEvents delivery can take much
      // longer than expected. Allow 60 s to avoid false flakes.
