...
describe('WorkspaceMcpConfigService', () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), 'mirri-workspace-mcp-config-cwd-'));
    homeDir = mkdtempSync(join(tmpdir(), 'mirri-workspace-mcp-config-home-'));
