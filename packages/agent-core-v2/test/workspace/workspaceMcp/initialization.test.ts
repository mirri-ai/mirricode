...
describe('Workspace MCP initialization', () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), 'mirri-session-mcp-cwd-'));
    homeDir = mkdtempSync(join(tmpdir(), 'mirri-session-mcp-home-'));
  ...
  it('times out tool calls using the session MCP timeout preference', async () => {
    await writeProjectMcpJson(cwd, {
      slowTool: {
        ...
        env: { MIRRI_TEST_MCP_TOOL_DELAY_MS: '300' },
      },
    });
...
async function writeProjectMcpJson(
  ...
  await mkdir(join(cwd, '.mirri-code'), { recursive: true });
  await writeFile(
    join(cwd, '.mirri-code', 'mcp.json'),
