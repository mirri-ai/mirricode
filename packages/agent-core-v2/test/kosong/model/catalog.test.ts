...
const HOST_HEADERS = { 'User-Agent': 'mirri-test/1.0', 'X-Msh-Device-Id': 'device-1' };
...
beforeEach(() => {
  savedCustomHeaders = process.env['MIRRICODE_CUSTOM_HEADERS'];
  delete process.env['MIRRICODE_CUSTOM_HEADERS'];
});

afterEach(() => {
  if (savedCustomHeaders === undefined) delete process.env['MIRRICODE_CUSTOM_HEADERS'];
  else process.env['MIRRICODE_CUSTOM_HEADERS'] = savedCustomHeaders;
});
...
describe('Model assembly (pure data)', () => {
  ...
  it('forwards only the User-Agent to vendors without a full hostHeaders declaration', () => {
    ...
    try {
      ...
      expect(model.headers).toEqual({ 'User-Agent': 'mirri-test/1.0' });
    } finally {
      host.dispose();
    }
  });
  ...
  it('resolves provider env-bag credentials and endpoints through the registry', () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: { type: 'kimi', env: { MIRRI_API_KEY: 'env-token', MIRRI_BASE_URL: 'https://kimi-env.example.test/v1' } },
  ...
  it('getById resolves a model by its config id', () => {
    const { host, catalog } = createHost({
      ...kimiSections,
      models: {
    ...
    try {
      // getById takes the modelId (the `[models.<id>]` config key) and returns
      // the assembled Model, or undefined when no model is configured for
      // that id. There is no fuzzy alias matching — callers resolve the id
      // upstream.
      expect(catalog.getById('k1')?.id).toBe('k1');
      expect(catalog.getById('k2')?.id).toBe('k2');
      expect(catalog.getById('kimi-k2')).toBeUndefined();
      expect(catalog.getById('shared-name')).toBeUndefined();
      expect(catalog.getById('unknown')).toBeUndefined();
    } finally {
      host.dispose();
    }
  });
  ...
  it('attributes env-bag credentials and endpoints by env-var name', () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: {
          type: 'kimi',
          env: { MIRRI_API_KEY: 'env-token', MIRRI_BASE_URL: 'https://kimi-env.example.test/v1' },
        },
      },
    ...
    try {
      ...
      expect(view.sources['resolved.auth']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('MIRRI_API_KEY'),
      });
      expect(view.sources['resolved.baseUrl']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('MIRRI_BASE_URL'),
      });
...
describe('ModelCatalog enumeration', () => {
  ...
  it('detects env-bag credentials through the vendor endpoint declarations', async () => {
    const { host, catalog } = createHost({
      providers: {
        kimi: { type: 'kimi', env: { MIRRI_API_KEY: 'kimi-env-key' } },
