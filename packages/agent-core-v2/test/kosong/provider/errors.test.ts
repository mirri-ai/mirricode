...
import { classifyMirriQuotaError } from '#/kosong/provider/providers/kimi/kimi-errors';
...
describe('classifyMirriQuotaError (Kimi trait classifier)', () => {
  it('classifies a structured exceeded_current_quota_error body as quota-exhausted', () => {
    const error = classifyMirriQuotaError(
  ...
  it.each([QUOTA_MESSAGE, TOKEN_QUOTA_MESSAGE])(
    ...
    (message) => {
      const error = classifyMirriQuotaError(moonshotQuota429(message));
      expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
      expect(isRetryableGenerateError(error)).toBe(false);
    },
  );
  ...
  it.each(['Too many requests', 'your token quota per minute was exceeded'])(
    ...
    (message) => {
      expect(classifyMirriQuotaError(moonshotQuota429(message))).toBeUndefined();
    },
  );

  it('answers undefined for non-429 and non-SDK shapes', () => {
    expect(
      classifyMirriQuotaError(new OpenAIAPIError(403, undefined, QUOTA_MESSAGE, new Headers())),
    ).toBeUndefined();
    expect(classifyMirriQuotaError(new Error(QUOTA_MESSAGE))).toBeUndefined();
    expect(classifyMirriQuotaError(undefined)).toBeUndefined();
  });
  ...
  it('classifies the Anthropic SDK error shape (body nested under .error)', () => {
    ...
    const error = classifyMirriQuotaError(source);
    expect(error).toBeInstanceOf(APIProviderQuotaExhaustedError);
    expect(isRetryableGenerateError(error)).toBe(false);
  });
