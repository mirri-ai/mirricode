 ...
 *  - `MirriFiles`: an upload failure classifies through
 *    `classifyMirriQuotaError`, so a Moonshot quota 429 from the files API
...
import { MirriFiles } from '#/kosong/provider/providers/kimi/kimi-files';
import {
  convertMirriTool,
  kimiAnthropicTrait,
  kimiOpenAITrait,
...
describe('kimiOpenAITrait.convertTool', () => {
  ...
  it('normalizes the schema dialect of regular tools', () => {
    ...
    expect(convertMirriTool(tool)).toEqual({
      type: 'function',
      function: {
        name: 'read_file',
        description: 'read',
...
describe('kimiOpenAITrait request params', () => {
  it('declares the MIRRI_API_KEY / MIRRI_BASE_URL fallback chain and default base URL', () => {
    expect(call(kimiOpenAITrait.endpoint, context)).toEqual({
      apiKeyEnv: 'MIRRI_API_KEY',
      baseUrlEnv: 'MIRRI_BASE_URL',
...
describe('MirriFiles upload error conversion', () => {
