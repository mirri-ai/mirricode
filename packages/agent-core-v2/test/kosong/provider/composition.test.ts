...
describe('apiKey env suppression (probe 1)', () => {
  ...
  it('uses the MIRRI_API_KEY env fallback when composing kimi', () => {
    process.env['MIRRI_API_KEY'] = 'sk-kimi-from-env';
...
describe('resolveProviderEndpoint', () => {
  it('resolves the kimi endpoint chain from process.env', () => {
    process.env['MIRRI_API_KEY'] = 'sk-kimi-env';
  ...
  it('reads a caller-supplied env bag instead of process.env', () => {
    process.env['MIRRI_API_KEY'] = 'sk-kimi-env';
    expect(resolveProviderEndpoint('kimi', { MIRRI_BASE_URL: 'https://example.com/v1' })).toEqual({
