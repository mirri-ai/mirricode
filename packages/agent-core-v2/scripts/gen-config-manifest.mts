 ...
 *
 * Usage:
 *   pnpm --filter @mirri-ai/agent-core-v2 gen:config-manifest          # write the file
 *   pnpm --filter @mirri-ai/agent-core-v2 gen:config-manifest --check  # freshness check (CI-style)
 *
...
export async function buildConfigManifest(): Promise<string> {
  ...
  const out: string[] = [
    ...
    '# Regenerate with: pnpm --filter @mirri-ai/agent-core-v2 gen:config-manifest',
    '#',
