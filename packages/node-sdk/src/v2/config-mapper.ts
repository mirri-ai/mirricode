 ...
 * `inspect().userValue` / `diagnostics()`) onto the v1 `MirriConfig` /
 * `ConfigDiagnostics` shapes the SDK contract returns.
 *
 * Why a mapping layer exists: v1 loads config.toml as ONE zod-validated
 * document (`MirriConfigSchema`), while v2 registers one config section per
 ...
 */
import type { ConfigDiagnostics, MirriConfig } from '#/types';

/**
 * Every top-level `MirriConfig` field except `raw` (a v1 write-path
 * implementation detail with no v2 counterpart). Each entry is both the v1
 * field name and the v2 config domain name.
 */
const MIRRI_CONFIG_DOMAINS = [
  'providers',
  'defaultProvider',
  'defaultModel',
  'models',
 ...
 * plus registered section defaults). Domains v2 knows but v1 does not are
 * dropped, mirroring how v1's schema strips unknown top-level keys.
 */
export function resolvedConfigToMirriConfig(resolved: Record<string, unknown>): MirriConfig {
  const config: Record<string, unknown> = {};
  for (const domain of MIRRI_CONFIG_DOMAINS) {
    const value = resolved[domain];
    if (value !== undefined) {
      config[domain] = value;
    }
  }
  return config as MirriConfig;
}
 ...
 * so the message texts are the warnings — v2-native callers keep the full
 * structure through the klient facade.
 */
...
/** The writes needed to reproduce v1 `removeMirriProvider` semantics. */
 ...
 * pointers when they dangle. The v2 engine's own provider delete only
 * clears the default-provider pointer, so the SDK replays the full v1
 * cascade through the config facade.
 */
...
/**
 * Apply the v1 remove-provider cascade to a whole `MirriConfig` in memory (no
 * persistence): drop the provider entry, every model pointing at it, and the
 * default pointers when they dangle. Hosts that stage the removal build on
 * this — the same role the v2 engine's provider-refresh path plays for its
 * own view.
 */
export function removeProviderFromConfig(config: MirriConfig, providerId: string): MirriConfig {
  ...
  return {
    ...config,
    providers: plan.providers as MirriConfig['providers'],
    models: plan.models as MirriConfig['models'],
