 ...
 * payloads are already v1-protocol-shaped — the same shapes v1 emits through
 * its own event stream (both sides inherit the same wire vocabulary, see
 * `agent-core-v2/src/agent/loop/turnEvents.ts` and
 * `packages/protocol/src/events.ts`). What the v2 bus does not carry is the
 ...
 * v1-visible fact the v2 engine publishes on the process-global `IEventService`
 * (`session.meta.updated`) is unwrapped from its `{type, payload}` envelope.
 */
import type { Event } from '@mirri-ai/agent-core';
import type { DomainEvent } from '@mirri-ai/agent-core-v2';
 ...
 * `task.*` (see `agent-core-v2/src/agent/task/taskOps.ts`). The payloads are
 * field-identical ports — the v1 SDK client only ever saw the legacy ones.
 */
