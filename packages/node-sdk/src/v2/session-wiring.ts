 ...
 *
 * One wiring instance per live session scope, created by `MirriRpcClientV2`
...
import { ContextSizeModel } from '@mirri-ai/agent-core-v2';
...
/**
 * The v2 approval payload — a superset of v1's `ApprovalRequest`: the extra
 * id/sessionId/agentId fields are stripped when the handler is fed.
 */
...
/** The v2 user-tool execution payload. */
...
export class SessionEventWiring {
  ...
  private attachAgent(agent: IAgentScopeHandle): void {
    ...
    this.agentSubscriptions.set(
      ...
      agent.accessor.get(IEventBus).subscribe((event) => {
        const enriched =
          event.type === 'agent.status.updated' ? withStatusSnapshot(agent, event as DomainEvent) : event;
  ...
  private async bridgeApproval(interaction: Interaction): Promise<void> {
    ...
    try {
      ...
      // request died with it, and `decide` no-ops on an unknown id anyway.
    }
  }
...
/**
 * v2 emits agent status in independent slices, and the model slice rides only
 * the bind-time emission — for a subagent that reaches the client before
 * `subagent.spawned` and is dropped there, so subagent cards never learn the
 * model. Fold a consistent usage + context + model snapshot into every status
 * event at this edge, restoring the v1 combined-payload contract regardless
 * of slice timing. Mirrors kap-server's `readLegacyStatus` projection (the
 * two v1-facing edges must agree); the SDK keeps its own copy because the
 * server package does not export the projection as a public subpath.
 */
function withStatusSnapshot(agent: IAgentScopeHandle, event: DomainEvent): DomainEvent {
  ...
  // Live (measured + estimated) context size — mirror of kap-server's
  // `readLegacyStatus`: the measured total is the better reading while a
  // post-step fold leaves the live estimate shorter than the measured prefix.
