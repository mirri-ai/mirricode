...
`,
  ...
  it('provisions SDK config using an existing Mirri OAuth token', async () => {
    await new FileTokenStorage(join(homeDir, 'credentials')).save('mirri-code', freshToken());
...
`,
    ...
    vi.stubEnv('MIRRICODE_BASE_URL', envBaseUrl);
    vi.stubEnv('MIRRICODE_OAUTH_HOST', envOauthHost);
    const fetchMock = vi.fn<FetchMock>(async (input, init) => {
...
`,
    ...
    await expect(
      new FileTokenStorage(join(homeDir, 'credentials')).load('mirri-code'),
    ).resolves.toBeUndefined();

    const text = await readFile(join(homeDir, 'config.toml'), 'utf-8');
    expect(text).not.toContain('managed:mirri-code');
    expect(text).not.toContain('mirri-code/kimi-for-coding');
...
`,
    ...
    await expect(storage.load('mirri-code')).resolves.toMatchObject({
      accessToken: 'oauth-access-token',
    });
  });
