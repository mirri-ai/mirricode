...
import { createMirriHarness, log } from '#/index';
...
describe('Local logging — harness integration', () => {
  it('writes session-tagged entries to session log only and untagged entries to global', async () => {
    ...
    const globalPath = join(homeDir, 'logs', 'mirri-code.log');
    const sessionLogPath = join(summary.sessionDir, 'logs', 'mirri-code.log');

    // Trigger an export — this flushes both global and session via MirriCore
  ...
  it('default export bundles session log only; no globalLogPath in manifest', async () => {
    ...
    expect(entries.has('logs/mirri-code.log')).toBe(true);
    expect(entries.has('logs/global/mirri-code.log')).toBe(false);
    expect(entries.get('logs/mirri-code.log')!.toString('utf-8')).toContain(
    ...
    const manifest = JSON.parse(entries.get('manifest.json')!.toString('utf-8')) as Record<
      ...
      unknown
    >;
    expect(manifest['sessionLogPath']).toBe('logs/mirri-code.log');
  ...
  it('default export works when no session log file exists', async () => {
    ...
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/mirri-code.log')).toBe(false);
  ...
  it('default export includes rotated session log files without requiring active mirri-code.log', async () => {
    ...
    try {
      ...
      const sessionLogEntries = [...entries.keys()].filter(
        (entry) => entry === 'logs/mirri-code.log' || entry.startsWith('logs/mirri-code.log.'),
      );
      expect(sessionLogEntries.length).toBeGreaterThan(0);
      expect(sessionLogEntries).toContain('logs/mirri-code.log.1');
      expect(entries.has('logs/global/mirri-code.log')).toBe(false);
      if (entries.has('logs/mirri-code.log')) {
        expect(result.manifest.sessionLogPath).toBe('logs/mirri-code.log');
      } else {
        expect(result.manifest.sessionLogPath).toBeUndefined();
      }
  ...
  it('--include-global-log bundles global active and sets manifest field', async () => {
    ...
    const entries = readZipEntries(zipBuf);
    expect(entries.has('agents/main/wire.jsonl')).toBe(true);
    expect(entries.has('logs/global/mirri-code.log')).toBe(true);
    ...
    expect(entries.get('logs/global/mirri-code.log')!.toString('utf-8')).toContain(
      'untagged probe',
    );
  });
  ...
  it('--include-global-log bundles the active root global log path', async () => {
    ...
    const entries = readZipEntries(await readFile(result.zipPath));
    const globalLog = entries.get('logs/global/mirri-code.log')!.toString('utf-8');
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'mirri-code.log'));
  ...
  it('logs export flush failures without failing the export', async () => {
    ...
    try {
      ...
      const entries = readZipEntries(await readFile(result.zipPath));
      const sessionLog = entries.get('logs/mirri-code.log')!.toString('utf-8');
      const globalLog = entries.get('logs/global/mirri-code.log')!.toString('utf-8');
  ...
  it('uses the latest harness homeDir for global diagnostic logging', async () => {
    ...
    const firstLog = await readOptionalFile(join(firstHome, 'logs', 'mirri-code.log'));
    const secondLog = await readFile(join(secondHome, 'logs', 'mirri-code.log'), 'utf-8');
  ...
  it('checks that an empty session log directory does not get a log file', async () => {
    ...
    try {
      ...
      expect(logsDir).not.toContain('mirri-code.log');
    } finally {
      restoreLogEnv(env);
    }
  });
