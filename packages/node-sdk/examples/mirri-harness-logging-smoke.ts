...
const TEST_HOME = join(homedir(), '.mirri-code-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'mirri-code.log' || file.startsWith('mirri-code.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('mirri-code.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('mirri-code.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['MIRRICODE_HOME'] = TEST_HOME;
  process.env['MIRRICODE_LOG_LEVEL'] = 'warn';
  process.env['MIRRICODE_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['MIRRICODE_LOG_SESSION_FILES'] = '2';
  process.env['MIRRICODE_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['MIRRICODE_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = createMirriHarness({
    identity: { userAgentProduct: 'mirri-code-cli', version: '0.1.1' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        'Open these files:',
        '  - logs/mirri-code.log',
        '  - logs/mirri-code.log.1',
        `  - ${summary.sessionDir}/logs/mirri-code.log`,
        `  - ${summary.sessionDir}/logs/mirri-code.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - mirri-code.log.1 exists',
        '  - mirri-code.log.2 does not exist',
        '',
        'Export evidence:',
