 ...
 *   - Windows resolves Git Bash via `MIRRICODE_SHELL_PATH`, `git.exe` on PATH
...
describe('detectEnvironment', () => {
  ...
  it('uses MIRRICODE_SHELL_PATH override when set and the file exists', async () => {
    const env = await detectEnvironment(
      stubDeps({
        platform: 'win32',
        env: { MIRRICODE_SHELL_PATH: 'D:\\custom\\bash.exe' },
