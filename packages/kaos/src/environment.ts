 ...
 * user-facing install hint. Set `MIRRICODE_SHELL_PATH` to override.
 */
...
async function locateWindowsGitBash(deps: EnvironmentDeps): Promise<string> {
  ...
  const override = deps.env['MIRRICODE_SHELL_PATH']?.trim();
  ...
  throw new KaosShellNotFoundError(
    `Git Bash was not found on this Windows host. Install Git for Windows from https://gitforwindows.org/ or set MIRRICODE_SHELL_PATH to a bash.exe. Checked: ${checked.join(', ')}.`,
  );
}
