// bench/import-mirri-code.js
//
// Import all workspaces + sessions from ~/.mirri-code into minidb and build a
// full-text index over the session content, then measure import + search speed.
//
// Run:  node bench/import-mirri-code.js [--data ~/.mirri-code] [--out <dir>]
...
const OUT = path.resolve(arg('out', path.join(os.tmpdir(), 'minidb-mirri-code-' + Date.now())));
...
async function main() {
  ...
  const byWs = db.findEq('byWorkspace', 'mirri-code-dev-1');
  console.log(`  index lookup (workspace=mirri-code-dev-1) -> ${byWs.length} in ${(performance.now() - w0).toFixed(2)} ms`);
