...
//
// "Without minidb" search baseline over the SAME real ~/.mirri-code sessions
// used by import-mirri-code.ts. Loads every session's extracted text into an
...
// range). Paired with import-mirri-code.ts, this shows what minidb's indexes
// buy you on real data.
//
// Run:  node --import tsx bench/search-baseline.ts [--data ~/.mirri-code]
...
const DATA = path.resolve(arg('data', path.join(os.homedir(), '.mirri-code')));
const FULL = argv.includes('--full'); // also index full tool results (matches import-mirri-code --full)
...
async function main() {
  ...
  {
    const { value: res, ms } = med(() => docs.filter((d) => d.workspaceName === 'mirri-code-dev-1'));
    console.log(`  naive lookup (workspace=mirri-code-dev-1) -> ${res.length} in ${ms.toFixed(2)} ms`);
  }
