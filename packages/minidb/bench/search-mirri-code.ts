// bench/search-mirri-code.js
//
// Import ~/.mirri-code sessions (useful extraction) and full-text-search for a
// query, printing hits with context snippets.
//
// Run:  node bench/search-mirri-code.js <query>  [--full]
...
if (!query) {
  console.error('usage: node bench/search-mirri-code.js <query> [--full]');
  process.exit(1);
}

const DATA = path.join(os.homedir(), '.mirri-code');
