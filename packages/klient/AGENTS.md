...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live
  suites (moved from server-e2e). They skip unless `MIRRI_SERVER_URL` points
...
- `pnpm --filter @mirri-ai/klient test` — all Vitest suites (unit +
  conformance + e2e; live cases skip without their env).
- `MIRRI_SERVER_URL=http://127.0.0.1:58627 pnpm --filter @mirri-ai/klient test`
  — include the live legacy cases against a running server.
- `pnpm --filter @mirri-ai/klient docker:e2e` — docker e2e; the run
  derives its runner name/namespace from the current workspace to avoid
  cross-workspace conflicts.
- `pnpm --filter @mirri-ai/klient typecheck` / `pnpm smoke` (in-process
  smoke over the memory transport; see `examples/smoke.ts`).
- `pnpm --filter @mirri-ai/klient smoke:boundary` — ModelRequester boundary
  probe: pings every model configured in the real `~/.mirri-code/config.toml`
...
- `pnpm --filter @mirri-ai/klient smoke:select-tools` — select_tools
  ...
  `examples/mirri-select-tools.ts`).
