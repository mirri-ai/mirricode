...
const M_ANTHROPIC_UNKNOWN = 'matrix-anthropic-unknown';
const M_GOOGLE = 'matrix-google';

const MIRRI_PROVIDER = 'matrix-kimi-provider';
...
beforeAll(async () => {
  ...
  await klient.global.kosong.addProvider(MIRRI_PROVIDER, {
    type: 'kimi',
  ...
  // M_KIMI needs a `provider` reference to MIRRI_PROVIDER so the engine
  ...
  // An Anthropic-protocol model whose name is NOT in the anthropic base's
  // capability catalog (non-`claude-*` prefix). Its capability resolves to
  // UNKNOWN, so the media-capability projector passes media through and the
  // provider's own conversion runs — used by the "illegal base64 video mime
  // throws in conversion" test, which needs the conversion to execute.
  await klient.global.kosong.addProvider({
    id: M_ANTHROPIC_UNKNOWN,
    model: 'test-anthropic-uncatalogued',
    protocol: 'anthropic',
    baseUrl: `${baseUrl}/v1`,
    auth: { method: 'api-key', apiKey: 'test-key' },
    maxContextSize: 262_144,
...
describe('video blocks', () => {
  it('video_url with an http URL passes through verbatim on every provider', async () => {
    // Each provider must carry the video part into the wire request body in
    // its native shape. Models are chosen so the bound model has `video_in:
    // true` (or is unknown), so the media-capability projector does not strip
    // the video before it reaches the provider conversion.
    const cases: Array<{
      label: string;
      model: string;
      reply: MockReply;
  ...
  it('kimi (composed): ReadMediaFile on a video uploads via the files API (uploadVideo trait)', async () => {
    ...
    // The MirriFiles client POSTs multipart form data to {baseUrl}/files.
  ...
  it('anthropic: illegal base64 video mime throws in conversion; retryable, so stepRetry re-runs (l2)', async () => {
    ...
    // Uses M_ANTHROPIC_UNKNOWN (uncatalogued model name) so the capability
    // resolves to UNKNOWN — the media-capability projector passes the video
    // through, the provider's own conversion runs, and it throws on the
    // illegal WMV MIME type before any HTTP request leaves.
    await klient.global.config.set({ domain: 'loopControl', patch: { maxRetriesPerStep: 2 } });
    try {
      const ctx = await newCase(M_ANTHROPIC_UNKNOWN, 'anthropic-video-mime');
