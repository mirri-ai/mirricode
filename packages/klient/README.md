...
import { createKlient } from '@mirri-ai/klient/memory';   // or '/ipc'
...
|---|---|---|
| `@mirri-ai/klient/ipc` | `{ socketPath, token? }` | same socket |
| `@mirri-ai/klient/memory` | `{ scope }` (a bootstrapped engine app scope) | direct emitter/bus subscription |
...
- `test/e2e/legacy/` + `test/e2e/harness/` — the legacy `/api/v1` live suites
  and their client harness (skip unless `MIRRI_SERVER_URL` is set; the v1
...
readings through a real prompt (requires `MIRRI_EXAMPLE_MODEL` +
`MIRRI_EXAMPLE_API_KEY`).
