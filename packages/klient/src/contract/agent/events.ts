...
export const turnEndedEventSchema = z.object({
  ...
  turnId: z.number(),
  reason: z.enum(['completed', 'cancelled', 'failed', 'filtered', 'blocked']),
  /** Protocol `MirriErrorPayload` — mirrored as `unknown`. */
...
/** `error` payloads carry the full `MirriErrorPayload`; kept loose. */
