 ...
 *
 * Env: MIRRI_MODEL_NAME is unset for the run (it would pin `defaultModel` and
...
import { IKosongConfigService } from '@mirri-ai/agent-core-v2/app/kosongConfig/kosongConfig';
...
const pinnedModelEnv = process.env['MIRRI_MODEL_NAME'];
delete process.env['MIRRI_MODEL_NAME'];
try {
  await main();
} catch (error) {
  console.error(error);
  ...
  if (pinnedModelEnv !== undefined) process.env['MIRRI_MODEL_NAME'] = pinnedModelEnv;
}
