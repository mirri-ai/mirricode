 ...
 * Kimi home (`MIRRICODE_HOME` or `~/.mirri-code`), resolves `IModelCatalog`
 ...
 *   MIRRI_BOUNDARY_MODELS  — comma-separated model ids to ping (default: all)
 *   MIRRI_BOUNDARY_SKIP_LIVE — set to `1` to skip part 1 (no real API calls)
 */
...
import { isError2 } from '@mirri-ai/agent-core-v2/_base/errors/errors';
...
} from '@mirri-ai/agent-core-v2/kosong/contract/errors';
import type { ToolCall } from '@mirri-ai/agent-core-v2/kosong/contract/message';
...
async function probeRealConfig(): Promise<void> {
  ...
  try {
    ...
    const filter = process.env['MIRRI_BOUNDARY_MODELS']?.split(',').map((s) => s.trim());
...
async function main(): Promise<void> {
  if (process.env['MIRRI_BOUNDARY_SKIP_LIVE'] !== '1') {
