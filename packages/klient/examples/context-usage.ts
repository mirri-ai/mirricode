 ...
 *   MIRRI_EXAMPLE_MODEL=... MIRRI_EXAMPLE_API_KEY=... \
 ...
 *
 * Env:
 *   MIRRI_EXAMPLE_MODEL      — gateway model id to seed (required)
 *   MIRRI_EXAMPLE_API_KEY    — API key for the seeded model (required)
 *   MIRRI_EXAMPLE_BASE_URL   — optional gateway base URL for the seeded model
 *   MIRRI_EXAMPLE_PROTOCOL   — optional wire protocol for the seeded model (default `openai`)
 */
...
async function main(): Promise<void> {
  const seedModel = process.env['MIRRI_EXAMPLE_MODEL'];
  const seedKey = process.env['MIRRI_EXAMPLE_API_KEY'];
  if (seedModel === undefined || seedKey === undefined) {
    throw new Error('MIRRI_EXAMPLE_MODEL and MIRRI_EXAMPLE_API_KEY are required (see header)');
  }
  ...
  try {
    ...
    await klient.global.kosong.addProvider({
      id: SEEDED_MODEL_ID,
      model: seedModel,
      protocol: (process.env['MIRRI_EXAMPLE_PROTOCOL'] ?? 'openai'),
      baseUrl: process.env['MIRRI_EXAMPLE_BASE_URL'] ?? 'http://127.0.0.1:1',
