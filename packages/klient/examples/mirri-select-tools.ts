 ...
 *     --import ../../build/register-raw-text-loader.mjs examples/mirri-select-tools.ts
 *
 * Env:
 *   MIRRICODE_HOME              — default `~/.mirri-code`
 *   MIRRI_SELECT_TOOLS_MODELS    — comma-separated model ids for the live parts (default: all kimi-type)
 *   MIRRI_SELECT_TOOLS_SKIP_LIVE — set to `1` to skip part B (no real API calls)
 *   MIRRI_SELECT_TOOLS_TAP       — set to `1` to run part C instead of B: route
...
import { bootstrap, logSeed, resolveLoggingConfig } from '@mirri-ai/agent-core-v2';
import { IConfigService } from '@mirri-ai/agent-core-v2/app/config/config';
import { renderLoadableToolsAnnouncement } from '@mirri-ai/agent-core-v2/agent/toolSelect/dynamicTools';
import { UNKNOWN_CAPABILITY } from '@mirri-ai/agent-core-v2/kosong/contract/capability';
import type { Message } from '@mirri-ai/agent-core-v2/kosong/contract/message';
import type { Tool } from '@mirri-ai/agent-core-v2/kosong/contract/tool';
import type { AuthProvider, Model } from '@mirri-ai/agent-core-v2/kosong/model/catalog';
import { IModelCatalog } from '@mirri-ai/agent-core-v2/kosong/model/catalog';
import type {
  ModelRequestInput,
  ModelRequester,
} from '@mirri-ai/agent-core-v2/kosong/model/modelRequester';
import { ModelRequesterImpl } from '@mirri-ai/agent-core-v2/kosong/model/modelRequesterImpl';
import { IProtocolAdapterRegistry } from '@mirri-ai/agent-core-v2/kosong/protocol/protocol';
import { ProtocolAdapterRegistry } from '@mirri-ai/agent-core-v2/kosong/provider/protocolAdapterRegistry';
...
async function probeLiveMirriProviders(): Promise<void> {
  const homeDir = process.env['MIRRICODE_HOME'] ?? join(homedir(), '.mirri-code');
  ...
  try {
    ...
    const filter = process.env['MIRRI_SELECT_TOOLS_MODELS']?.split(',').map((s) => s.trim());
...
async function main(): Promise<void> {
  ...
  if (process.env['MIRRI_SELECT_TOOLS_TAP'] === '1') {
    await probeTappedContext();
  } else if (process.env['MIRRI_SELECT_TOOLS_SKIP_LIVE'] !== '1') {
    await probeLiveMirriProviders();
  }
  console.log('\nselect-tools: OK');
}
