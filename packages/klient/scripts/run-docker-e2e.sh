...
RUN_ID="${MIRRI_SERVER_E2E_RUN_ID:-${workspace_slug}-${workspace_hash}}"

BASE_IMAGE="${MIRRI_SERVER_E2E_BASE_IMAGE:-mirri-server-e2e-base:${RUN_ID}}"
IMAGE="${MIRRI_SERVER_E2E_IMAGE:-mirri-server-e2e:${RUN_ID}}"
CONTAINER="${MIRRI_SERVER_E2E_CONTAINER:-mirri-server-e2e-${RUN_ID}}"
STATE_ROOT="${MIRRI_SERVER_E2E_STATE_ROOT:-${HOME}/.mirri-code-server-dev}"
PORT="${MIRRI_SERVER_E2E_PORT:-58627}"

MIRRI_HOME_HOST="${MIRRI_SERVER_E2E_MIRRI_HOME_HOST:-${STATE_ROOT}/docker-e2e/${RUN_ID}/mirri-code-home}"
MIRRI_HOME_CONTAINER="/data/docker-e2e/mirri-code-home"
SEED_HOME_HOST="${MIRRI_SERVER_E2E_SEED_MIRRI_HOME_HOST:-${STATE_ROOT}/mirri-home/mirri-code-home}"

if [[ -n "${MIRRI_SERVER_E2E_REPORT_DIR_HOST:-}" ]]; then
  REPORT_DIR_HOST="${MIRRI_SERVER_E2E_REPORT_DIR_HOST}"
  REPORT_ROOT_HOST="$(dirname -- "${REPORT_DIR_HOST}")"
  REPORT_DIR_NAME="$(basename -- "${REPORT_DIR_HOST}")"
else
  REPORT_ROOT_HOST="${MIRRI_SERVER_E2E_REPORT_ROOT_HOST:-${STATE_ROOT}/server-e2e-reports/docker/${RUN_ID}}"
...
workspace_node_modules=(
  ...
  "pkg_kap-server:/workspace/mirricode/packages/kap-server/node_modules"
  "pkg_klient:/workspace/mirricode/packages/klient/node_modules"
...
if [[ "${MIRRI_SERVER_E2E_SKIP_BUILD:-0}" != "1" ]]; then
...
mkdir -p "${MIRRICODE_HOME}/server" "${MIRRI_SERVER_E2E_REPORT_DIR}" "${TMPDIR}" /data/server-e2e-reports/docker
rm -f "${MIRRICODE_HOME}/server/lock"

if [[ ! -e /workspace/mirricode/node_modules/.modules.yaml || ! -e /workspace/mirricode/packages/klient/node_modules/ws ]]; then
...
echo "[server-e2e:docker] starting server on container-local ${MIRRI_SERVER_URL}"
pnpm dev:server -- \
  --host 127.0.0.1 \
  --port "${MIRRI_SERVER_E2E_PORT}" \
  --log-level debug \
  --debug-endpoints \
  >"${server_log}" 2>&1 &
server_pid=$!
...
for attempt in $(seq 1 90); do
  if curl -fsS "${MIRRI_SERVER_URL}/api/v1/meta" >/tmp/server-meta.json 2>/tmp/server-curl.err; then
    ready=1
...
cd /workspace/mirricode/packages/klient
pnpm test
EOS
...
docker_args=(
  ...
  --init
  --name "${CONTAINER}"
  --workdir /workspace/mirricode/packages/klient
  --env "MIRRICODE_HOME=${MIRRI_HOME_CONTAINER}"
  --env "MIRRI_SERVER_E2E_PORT=${PORT}"
  --env "MIRRI_SERVER_URL=http://127.0.0.1:${PORT}"
  --env "MIRRI_SERVER_E2E_REPORT_DIR=${REPORT_DIR_CONTAINER}"
