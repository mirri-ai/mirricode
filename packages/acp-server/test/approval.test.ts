...
// `ApprovalRequest` lives in the approval domain module (the public barrel
// re-exports only `ISessionApprovalService`) — see `src/approval.ts`.
import type { ApprovalRequest } from '@mirri-ai/agent-core-v2/session/approval/approval';
import type { ToolInputDisplay } from '@mirri-ai/protocol';

// Local alias so the test body reads like the pre-rename surface.
type SessionApprovalRequest = ApprovalRequest;
