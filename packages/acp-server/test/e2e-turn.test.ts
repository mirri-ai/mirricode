...
import { getLiveSessionById, IAgentLifecycleService, IEventBus } from '@mirri-ai/agent-core-v2';
...
describe('acp-server real prompt turn (scripted LLM)', () => {
  ...
  it('attaches locations to a file tool call and its terminal update', async () => {
    ...
    // No streaming-args deltas exist on this facade, so the wire `tool_call`
    // CREATE fires at `tool.call.started`: `in_progress`, and its locations
    // derive directly from the full args at that point.
    const createdNotification = await c.waitForSessionUpdate('tool_call', 10_000);
    expect(updateOf(createdNotification)?.status).toBe('in_progress');
    expect(updateOf(createdNotification)?.locations?.[0]?.path).toBe(filePath);
  ...
  it('creates the tool_call with the full stringified args at started (no streaming-args deltas on this facade)', async () => {
    const c = await boot();
    scripted!.mockNextResponse({
      type: 'function',
      id: 'call_1',
      name: 'Bash',
      arguments: JSON.stringify({ command: 'echo delta_stream' }),
    });
    scripted!.mockNextText('done');
    c.onRequest('session/request_permission', () => ({
      outcome: { outcome: 'selected', optionId: 'approve_once' },
    ...
    // The CREATE is the only tool_call notification: `in_progress` with the
    // canonical metadata and the full stringified args, followed directly by
    // the terminal completed update (no delta/upgrade chain exists here).
    const create = updates.find((u) => u?.sessionUpdate === 'tool_call');
    expect(create?.status).toBe('in_progress');
    // The engine attaches a human-readable description to the Bash invocation.
    expect(create?.title).toContain('echo delta_stream');
    expect(create?.kind).toBe('execute');
    expect(create?.rawInput).toEqual({ command: 'echo delta_stream' });
    expect(textOf(create)).toContain('echo delta_stream');
  ...
  it('does not forward unregistered engine event kinds (tool.progress) to the client', async () => {
    const c = await boot();
    ...
    // Publish progress events onto the main agent's event bus (the same
    // channel a progress-reporting tool uses). The facade does not stream
    // `tool.progress` (KLIENT-GAP), so neither entry may cross the wire.
    ...
    // Neither the stdout bytes nor the status text refreshed the card.
    expect(updates.some((u) => u?.title === 'Still working…')).toBe(false);
...
describe('acp-server builtin slash commands (local execution, no LLM turn)', () => {
  ...
  it('/mcp answers with a notice that the MCP surface is unavailable (KLIENT-GAP) and never calls the model', async () => {
    const c = await boot();
    ...
    expect(stopReason).toBe('end_turn');
    expect(chunk).toContain('MCP server status is not available through this ACP host.');
    expect(scripted!.callCount()).toBe(0);
  }, 30_000);

  it('/mcp on a session without servers answers with the same notice and never calls the model', async () => {
    const c = await boot();
    const sessionId = await newSession(c);
  ...
  it('/compact answers with a notice that compaction is unavailable (KLIENT-GAP) and never calls the model', async () => {
    const c = await boot();
    const sessionId = await newSession(c);

    const { chunk, stopReason } = await runSlash(c, sessionId, '/compact keep it short');
    expect(stopReason).toBe('end_turn');
    expect(chunk).toContain('Manual context compaction is not available through this ACP host.');
    expect(scripted!.callCount()).toBe(0);
  }, 30_000);

  it('/compact on any history still never calls the model', async () => {
    const c = await boot();
    const sessionId = await newSession(c);
