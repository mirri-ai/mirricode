...
import type { SessionSummary } from '@mirri-ai/klient';
...
describe('acp-server session lifecycle', () => {
  ...
  it(
    'session/delete is not supported and reports invalid_params (engine has no delete API)',
    async () => {
      const c = await boot();
      const created = (await c.send('session/new', { cwd: homeDir, mcpServers: [] })) as {
        sessionId: string;
      ...
      // The port trims engine session-delete (no facade/engine API exists) —
      // the request is answered with a structured invalid_params error.
  ...
  it(
    'session/new ignores ACP mcpServers (engine MCP servers are workspace-scoped) and never adds an overlay entry',
    async () => {
      const c = await boot();
      ...
      // Engine-side assertion: no ephemeral session overlay server appears —
      // the mcpServers request was dropped with a warning.
      const entries = await sessionMcpEntries(c, created.sessionId);
      expect(entries.find((e) => e.name === 'mock')).toBeUndefined();
    },
    30_000,
  );

  it(
    'session/load ignores ACP mcpServers (engine MCP servers are workspace-scoped)',
    async () => {
      const c = await boot();
      const created = (await c.send('session/new', { cwd: homeDir, mcpServers: [] })) as {
        sessionId: string;
      ...
      await c.send('session/load', {
        ...
        cwd: homeDir,
        mcpServers: [
          { name: 'mock', command: process.execPath, args: ['--version'], env: [] },
        ],
      });
