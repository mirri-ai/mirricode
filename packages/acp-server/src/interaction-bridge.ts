...
// `ApprovalRequest` / `ApprovalResponse` are exported only through the
// `./*` subpath of the engine (the public barrel re-exports just
// `ISessionApprovalService`) — see `approval.ts` for the same note.
import type {
  ApprovalRequest,
  ApprovalResponse,
} from '@mirri-ai/agent-core-v2/session/approval/approval';
import type { IDisposable, SessionHandle } from '@mirri-ai/klient';
