...
// The engine's public barrel exports only `ISessionApprovalService`; the
// `ApprovalRequest` / `ApprovalResponse` models are imported from the domain
// module directly (package exports expose `./*` subpaths).
import type {
  ApprovalRequest,
  ApprovalResponse,
...
export function permissionResponseToApprovalResponse(
  ...
  switch (optionId) {
    ...
    // fallthrough — Legacy Python kimi-cli (< v0.9.0) used 'approve' as the
    // allow-once optionId. Keep accepting it so custom ACP clients built
    // against the old SDK are not silently rejected.
    case 'approve':
      return { decision: 'approved' };
    case APPROVE_ALWAYS_OPTION_ID:
    // fallthrough — legacy 'approve_for_session' alias, same rationale.
