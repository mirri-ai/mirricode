...
/**
 * Map a turn-end reason to an ACP `stopReason`.
 *
 ...
 * `filtered`  → `refusal`: the engine reports the provider's content-filter
 *   rejection as its own terminal reason (protocol `TurnEndReason`).
 * `blocked`   → `refusal`: a prompt hook blocked the turn before the model
 *   ran (the klient's `turn.ended` schema retains this legacy reason). ACP has
 *   no separate hook-blocked terminal state, so reuse the refusal channel.
 */
