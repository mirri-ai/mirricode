/**
 * acp-server bootstrap — wires `@mirri-ai/agent-core-v2` (the DI × Scope
 * engine) into an ACP (Agent Client Protocol) stdio server.
 *
 * Composition root: `bootstrap()` builds the App `Scope`; a `@mirri-ai/
...
import type { Klient } from '@mirri-ai/klient';
...
export async function runAcpServerWithStream(
  ...
  const homeDir = resolveMirriHome(opts.homeDir);
  ...
  // `MIRRICODE_PLATFORM` from `@mirri-ai/v2-oauth`, which this package does
  // not depend on.
  const { app: core } = bootstrap(
    {
      ...
      configPath,
      clientIdentity: {
        productName: opts.agentInfo?.name ?? 'mirri-code-acp',
        version: opts.agentInfo?.version ?? '0.0.0',
        platform: 'mirri_code_cli',
      },
    },
  ...
  server = new AcpServer(client, klient, acpConnection, {
    ...
    // Engine-side auth readiness probe for the gate's primary check ("can the
    // default model authenticate"): scope-resolved here because the klient
    // facade surfaces no `auth.ensureReady`.
    probeAuth: () => core.accessor.get(IAuthSummaryService).ensureReady(),
  ...
  const close = async (): Promise<void> => {
    ...
    closePromise = (async () => {
      ...
      // shutdown never races a pending drain against teardown (and doesn't
      // drop the last persisted ops). Best-effort: a flush failure must not
      // block disposal.
      try {
      ...
      // Port note: the kimi build additionally drained a session-index mirror
      // and a miniDB query store here; the mirri engine replaced those with a
      // SQLite-backed `ISessionIndex` (no async mirror drains to await) —
      // `core.dispose()` settles them synchronously, matching kap-server's
      // shutdown order (`core.dispose()` + service-specific drains only).
      core.dispose();
    })();
    return closePromise;
  };
