 ...
 *     lives in `apps/mirri-code` and is intentionally out-of-scope for
 *     the adapter; see STATUS for the registry-gap note).
 *   - `plan` session_update derived from the mirri-code TodoList
 *     `display.kind === 'todo_list'` payload attached to a
 *     `tool.call.started` event.
 *
 * Status mapping is the mirri-code TodoStatus → ACP PlanEntryStatus
