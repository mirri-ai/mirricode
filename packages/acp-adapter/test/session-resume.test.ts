...
function makeHarness(opts: {
  ...
  return {
    ...
    // these to assemble configOptions. `mirri-coder` opts in to thinking
    // via `capabilities: ['thinking']`; `kimi-plain` stays off.
    getConfig: async () => ({
      providers: {},
      defaultModel: 'mirri-coder',
      models: makeModelsMap([
        { id: 'mirri-coder', name: 'Mirri Coder', thinkingSupported: true },
...
describe('AcpServer.resumeSession', () => {
  ...
  it('returns configOptions matching the resumed session model + mode + thinking', async () => {
    ...
    expect(modelOpt!.currentValue).toBe('mirri-coder');
