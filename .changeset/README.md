...
| --- | --- | --- |
| `@mirri-ai/mirri-code` | `apps/mirri-code` | CLI / TUI application — provides the `kimi` command after install |
| `@mirri-ai/mirri-code-sdk` | `packages/node-sdk` | Public TypeScript SDK |
...
Version impact from internal dependencies must be judged manually. The published artifacts for CLI and SDK bundle internal workspace packages into the artifact itself; runtime `dependencies` of published packages must not include any `@mirri-ai/*` internal workspace packages.
...
| --- | --- |
| Only modifies TUI behavior in `@mirri-ai/mirri-code` | Add `patch` / `minor` / `major` to `@mirri-ai/mirri-code` |
| Only modifies internal packages, no user-visible change in SDK / CLI | Usually no changeset needed |
| Internal package fix changes the CLI user experience | Add a changeset to `@mirri-ai/mirri-code` describing the user-visible fix |
| Internal package adds a new capability exposed by the SDK | Add a changeset to `@mirri-ai/mirri-code-sdk` |
...
1. Open each publishable package's page on the npm website, e.g. `https://www.npmjs.com/package/@mirri-ai/mirri-code`.
...
| Field | Value |
| --- | --- |
| GitHub Organization | `MirriAI` |
| GitHub Repository | `mirri-code` |
...
- If a change in an underlying internal package alters user-visible behavior or public API of a publishable package, add a changeset to the affected publishable package. For example, when a bug fixed in `@mirri-ai/agent-core` resolves an issue CLI users encounter, add a changeset to `@mirri-ai/mirri-code` describing the user-visible fix.
- `@mirri-ai/mirri-code` is the official CLI package name; after a global install it provides the `kimi` command.
